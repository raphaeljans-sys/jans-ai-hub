...
**Pfad**, nicht in doppelten eckigen Klammern (Wiki-Link-Syntax). Grund: jene KB führt **keinen `wiki/`-Ordner** (sie ist als
`catalog/` plus Abfragewerkzeug gebaut), und `wissen/tools/wiki-konsistenz.sh` sucht Linkziele in
`wiki/` und `destillate/`. Ein Wiki-Link auf diesen KB-Namen erzeugte deshalb bei **jedem**
