---
title: 2205 WB Guggerbach — Zentrum Guggerbach Davos, Wohnungsneubau Generation 65+ (Kennwort Juli)
status: draft
last_updated: 2026-07-12
sources: [2205-guggerbach_jurybericht.txt, 2205-guggerbach_programm.txt, 2205-guggerbach_projekttext.txt]
links: []
---

# 2205 WB Guggerbach — Zentrum Guggerbach Davos

## 1. Stammdaten

- **Verfahren:** Einstufiger, anonymer Projektwettbewerb im **selektiven Verfahren** mit Folgeauftrag, nach Ordnung SIA 142 (Ausgabe 2009); IVöB, Submissionsgesetz und Submissionsverordnung des Kantons Graubünden; PQ-Ausschreibung öffentlich auf simap.ch ab 14. Februar 2022 (Jurybericht S. 5, 8; Programm S. 28: Genehmigung 29. März 2022, SIA-142/143-Konformitätsprüfung nachpubliziert).
- **Auslober/Veranstalterin:** Stiftung Zentrum Guggerbach Davos, Obere Strasse 20, 7270 Davos Platz; Wettbewerbsbetreuung/Sekretariat: Fanzun AG, Birmensdorferstrasse 108, 8003 Zürich (Ansprechpartner Günther Bischof, Marcel Schlandt) (Jurybericht S. 4).
- **Bauaufgabe:** Ergänzung des bestehenden Alterszentrums (Generation 65+, Wohnen/Betreuung/Pflege) durch einen **Wohnungsneubau mit ca. zwanzig 3½-Zimmer-Wohnungen mit gehobenem Standard** auf Parzelle 110 (Platzstrasse 6, Davos Platz) plus eine neue Ankunfts- und Verteilzone («Knotenpunkt»), die Bestandsbauten und Neubau verbindet und Platz für ein neues Bistro und halböffentliche Nutzungen bietet (Jurybericht S. 3; Programm S. 22: Kataster-Nr. 110/111).
- **Jahr:** 2022 — Programm 5. April 2022 (Dateiname 220405); Ortsbegehung 7. April 2022; Abgabe Pläne 15. Juli 2022, Abgabe Modell 29. Juli 2022; Jurierung 15./19. August 2022; Bericht/Ausstellung 24./25. September 2022 (Jurybericht S. 8); Jurybericht datiert 22. September 2022 (Dateiname/PDF-Metadaten).
- **Präqualifikation:** 12 Teams selektioniert — die 10 bestbewerteten Bewerbungen plus 2 Nachwuchsbüros. Das JANS-Team wurde als **Nachwuchsteam Nr. 07** selektioniert: Atelier Arpagaus Sommer Zarn mit Archobau AG, B3 Kolb AG, Collenberg Energietechnik AG (Jurybericht S. 6–7). PQ-Unterlagen: `/Users/raphaeljans/Library/CloudStorage/OneDrive-FreigegebeneBibliotheken–JANS/AR - 02 Wettbewerbe/2205 WB Guggerbach/12_PQ/` (Entscheid: `12_PQ/07_Entscheid/Entscheid Präqualifikation Projektwettbewerb Zentrum Guggerbach Davos.pdf`).
- **Kennwort:** **Juli** (Jurybericht S. 49, Beitrag 7.8 mit Verfasserzuordnung Atelier Arpagaus Sommer Zarn). Hinweis: Im eigenen Verfassernachweis-Formular (`08_Abgabe/A05_Verfassernachweis_ausgefüllt.pdf`) ist das Kennwort-Feld im Textextrakt leer; die Zuordnung Kennwort↔Team ist über den Jurybericht belegt.
- **Team (Jurybericht S. 49; Verfassernachweis `08_Abgabe/A05_Verfassernachweis_ausgefüllt.pdf`):**
  - Architektur und Städtebau: Atelier Arpagaus Sommer Zarn — Entwurfsverfasser Andrea Marco Zarn; Mitarbeit **Raphael Jans**, Ramon Arpagaus, Mario Sommer.
  - Tragwerksplanung: B3 Kolb AG — Ivan Brühwiler.
  - HLK-Planung: Collenberg Energietechnik AG — Remo Collenberg.
  - Baumanagement: ARCHOBAU AG.
- **Resultat (belegt):** **Kein Rang, kein Preis.** Von 12 präqualifizierten Teams reichten 11 einen Beitrag ein; alle 11 wurden zur Beurteilung zugelassen (Jurybericht S. 13). «Juli» schied im **1. Beurteilungsrundgang** aus (Jurybericht S. 14). Rangierung: 1. Sonnenblume (CHF 60'000), 2. Compages (CHF 50'000), 3. Tetris (CHF 30'000), 4. Kranich (CHF 20'000, Nachwuchsbüro Nik Werenfels) (Jurybericht S. 15); Preissumme total CHF 160'000, vollumfänglich ausgerichtet, keine Ankäufe (Jurybericht S. 6, 14).
- **Siegerprojekt:** «Sonnenblume», Frei & Ehrensperger Architekten BSA (Julian Ringler, Colin Cruse), Tragwerk Liesch Ingenieure AG, HLK Züst Haustechnik AG; einstimmig zur Weiterbearbeitung empfohlen (Jurybericht S. 15–17).

## 2. Programm-Soll

- **Raumprogramm** (Programm S. 27, Tabelle 2): Wohnen 2'371 m2 NF; Bistro & Begegnungsräume 267 m2; Fitnessbereich 214 m2; weitere Betriebsräume 100 m2; Aussenbereich mit Infrastruktur 45 m2; Einstellhalle min. 30 PP. Summe Nutzflächen Neubau & Knotenpunkt 2'997 m2; Konstruktions-/Verkehrs-/Funktionsflächen 1'076 m2; **Geschossfläche Neubau & Knotenpunkt 4'073 m2**.
- **Wohnungen** (Programm S. 22): 3½-Zimmer-Wohnungen mit exklusivem Charakter; programmatisches Minimum **20 Wohnungen und 5 Studios** (gemäss Machbarkeitsstudie); zwei Individualräume plus Wohnraum; Küche zur Selbstversorgung zwingend; hohe räumliche Qualität, bewusster Umgang mit Aussicht und Besonnung.
- **Weitere Muss-Vorgaben** (Programm S. 22): Tiefgarage mit mindestens 30 zusätzlichen PP unter Nutzung des stark abfallenden Geländes; unterirdische Anbindung von Haus D an die bestehende Passerelle zwischen Haus A und Promenade zwingend.
- **Abgabeforderungen** (Programm S. 16): Situationsplan 1:500; Grundrisse, Ansichten und wesentliche Schnitte M 1:200 (mit gewachsenem und projektiertem Terrain); beispielhafter Wohnungsgrundriss M 1:50; Umgebungsmodell M 1:500 mit Einsatz; Planformat **A0 quer (84 x 120 cm)** verbindlich. Anonymität: alle Unterlagen mit Kennwort und Vermerk «Zentrum Guggerbach Davos» (Programm S. 12).
- **Beurteilungskriterien** (gleichgewichtet; Programm S. 8, identisch Jurybericht S. 8): Architektur · städtebauliche Setzung und Erschliessung · Einhaltung des Raumprogramms · Wirtschaftlichkeit (Investitions- und Lebenszykluskosten) · Ökologie und Nachhaltigkeit.

## 3. Eigener Beitrag (Kennwort «Juli»)

- **Typologie (Taxonomie):** **Ensemble/Mehrkoerper** — zwei in Aussenform und Grundriss identische, regelmässig polygonale, sechsgeschossige Solitäre, zueinander leicht ausgedreht in die Osthanglage gesetzt (eigener Projekttext `05_Texte/Guggerbach.docx`, Abschnitte «Städtebau»/«Architektur»; Jurybericht S. 50: «zwei Baukörper …, die in der Form identisch sind»). Erschliessung: **Spänner** — innenliegende, kompakte Erschliessungsanlage, **zwei Wohnungen pro Geschoss** in fünf Obergeschossen (Zweispänner), maximale Fassadenfläche den Wohnungen zugeschlagen (Projekttext, Abschnitt «Architektur»).
- **Setzung/Städtebau:** Zwei Körper mit minimalem Fussabdruck im parkartigen Grünstreifen an der Platzstrasse; durchlässige Gesamtanlage, Ensemble mit den Bestandsbauten (Häuser A und D), Dachkante beider Volumen auf gleicher Höhe belassen (Projekttext «Städtebau»/«Freiraumkonzept»; Jurybericht S. 50).
- **Programmanordnung:** EG je Haus mit Eingangsraum, Studios, Gemeinschaftsraum, Waschküche und Nebenräumen; 5 OG mit je 2 Wohnungen; 1. UG Keller/Sportgeräte (direkt von aussen erschlossen); 2. UG Einstellhalle und Anbindung an Haus A (Projekttext «Architektur»).
- **Wohnungstyp:** Dreiseitig orientierte Wohnungen, Erschliessung im Zentrum der Wohnung, Individualräume und Wohnraum an der Fassade aufgereiht, dazwischengeschobenes Bad mit Tageslicht, über Eck organisierter Wohnbereich mit windgeschützter Eckloggia, Cheminée (Projekttext «Wohnungen»). Freier Grundriss mit umlaufenden Raumverbindungen (Jurybericht S. 50).
- **Material/Tragwerk:** Massivholz-Elementbau aus verdübelten Vollholzbrettern (leimfrei), Rippendecken mit Vollholzbalken je Wohneinheit, keine Stahlbeton-Treppenhauskerne; UG als wasserdichte Wanne in Stahlbeton (Projekttext «Konstruktion» — dort mit Vermerk «Weitere Text nicht bearbeitet (Stand Hittnau)», d.h. Konstruktionstext moeglicherweise nicht abgabefinal).
- **Kennzahlen (GF/GV):** Nicht belegt extrahierbar — das Datenblatt Kosten (`08_Abgabe/02 Formulare/A06_Datenblatt Kosten_Mengen.pdf`) liefert im Textextrakt keinen Inhalt; siehe Offene Fragen.

## 4. Jury-Urteil (Projektwürdigung «Juli», Jurybericht S. 50)

**Kritik (dominierend):**
- Zur identischen Dachkante trotz Hanglage: «Ein Entwurfsentscheid, der ortsbaulich unverständlich ist.» (Jurybericht S. 50)
- Zur Setzung: «Die zwei Volumen stehen zudem zu nahe zusammen und erzeugen hohe räumliche Dichten. Hier verliert der Gedanke, von alleinstehenden Gebäuden im Park, stark an Kraft.» (Jurybericht S. 50)
- Zur Zweiteilung: «Mit Blick auf die Wirtschaftlichkeit sind zwei Gebäude ungünstig» (Jurybericht S. 50); «die Aufteilung auf zwei Baukörper an sich [führt] zu höheren Kosten» (Jurybericht S. 50).
- Zum Grundriss: «Der freie Grundriss mit umlaufenden Raumverbindungen ist ein spannender Ansatz, bringt aber dem zukünftigen Nutzer keinen Mehrwert – im Gegenteil, es entstehen zu viele Verkehrsflächen. Übrig bleiben schlecht möblierbare Räume.» (Jurybericht S. 50)
- Zu Kosten/Statik: «Es sind einzelne Baugrundsicherungen/ Unterfangungen und im Haus A erhebliche statische Eingriffe erforderlich. Die Erstellungskosten pro Wohnung sind im Verhältnis relativ hoch, sodass mit einer weniger guten Wirtschaftlichkeit zu rechnen ist.» (Jurybericht S. 50)

**Lob (knapp):**
- «Das Projekt überzeugt auf den ersten Blick mit der Körnigkeit der Volumen» (Jurybericht S. 50); die Baukörper «in sich sind kompakt ausgebildet» (Jurybericht S. 50); der freie Grundriss wird als «spannender Ansatz» anerkannt (Jurybericht S. 50).

**Fazit der Jury:** Das Projekt «vermag aber in der städtebaulichen Setzung, der inneren Organisation und der Wirtschaftlichkeit nicht zu überzeugen» (Jurybericht S. 50) — Ausscheiden im 1. Beurteilungsrundgang (Jurybericht S. 14).

### Delta zum 1. Rang («Sonnenblume», Jurybericht S. 18–19)

- **Ein Körper statt zwei:** Der Sieger setzte EINEN «rundum segmentierten und ausgreifenden Baukörper» auf die Parzelle, von der Strasse abgerückt, auf einem Sockel, der das fallende Terrain aufnimmt (S. 18) — während «Juli» mit zwei identischen Körpern hohe Dichte und Mehrkosten erzeugte (S. 50).
- **Verbindendes Herzstück:** Sonnenblume verband alle drei Wohnhäuser im Gartengeschoss durch eine «attraktive, verglaste Gartenhalle» mit Fitness, Coiffeur, Massage, Podologie — als Treffpunkt bis hin zum Spazieren im Winter (S. 18); die Gartenhalle «bietet einen Mehrwert für alle Bewohner, Besucherinnen und Mitarbeiter» (S. 19). «Juli» bot kein vergleichbares verbindendes Element; die Anbindung lag im 2. UG (Projekttext «Architektur»).
- **Wohnungsqualität mit Nutzerfokus:** Beim Sieger «fächerartig angeordnete» Wohnungen zur Sonne und Berglandschaft, wählbar zuschaltbare Zimmer, «verschiedene Zirkulationsschlaufen», Panoramafenster mit niedriger Brüstung (S. 18); bei «Juli» dagegen zu viele Verkehrsflächen und schlecht möblierbare Räume (S. 50).
- **Studios als Pluspunkt:** Sonnenblume legte die Studios auf jedes Wohngeschoss (Gäste-/Angehörigenzimmer nahe der Wohnung) (S. 18); «Juli» sammelte die Studios im EG (Projekttext «Architektur»).
- **Wirtschaftlichkeit:** Sonnenblume «mittelmässig kompakt», wenige Baugrundsicherungen, geringe Eingriffe in Haus A, Erstellungskosten im oberen Durchschnitt = mittlere Wirtschaftlichkeit (S. 19); «Juli» erhebliche statische Eingriffe in Haus A und relativ hohe Kosten pro Wohnung (S. 50).
- **Einfühlung als Jury-Argument:** «Überhaupt haben sich die Projektverfasserinnen in das Leben der Menschen im Zentrum Guggerbach hineinversetzt» (S. 19); die Empfehlung erfolgte wegen «sehr hohe[r] ortsbaulicher Qualität, typologische[r] Herleitung und Einfügung auf dem Areal» (S. 15).

## 5. Lessons / DNA

1. **Beim Alterszentrums-Anbau am Hang prämierte die Jury den EINEN terrainfolgenden, gegliederten Solitär — nicht das Zweikörper-Ensemble.** Zwei Baukörper = doppelte Fassade, doppelte Erschliessung, hohe Dichte im Zwischenraum und Mehrkosten; die Jury rechnete das direkt in «weniger gute Wirtschaftlichkeit» um (Jurybericht S. 50 vs. S. 18–19). Wirtschaftlichkeit war gleichgewichtetes Kriterium (Programm S. 8).
2. **Bei Erweiterungen eines Pflege-/Wohnensembles gewinnt das verbindende soziale Herzstück.** Der Sieger machte die zwingende Anbindung (Passerelle/Knotenpunkt) zur belebten, verglasten Gartenhalle mit Mehrwert für ALLE Nutzer statt zu einem UG-Korridor (Jurybericht S. 18–19). Muss-Verbindungen im Programm nie nur funktional abhaken, sondern zum architektonischen Hauptargument machen.
3. **Identische, gedrehte Baukörper sind bei der Jury doppelt angreifbar:** Sie erzeugen «unterschiedliche Wertung der Wohnungen» (Belichtung) ohne räumlichen Gegenwert (Jurybericht S. 50). Repetition spart Planung, aber die Jury liest sie als fehlende Differenzierung.
4. **«Freier Grundriss» braucht Nutzer-Nachweis:** Umlaufende Raumverbindungen wurden als Konzept gelobt, aber an Möblierbarkeit und Verkehrsflächenanteil gemessen und verworfen (Jurybericht S. 50). Beim Zielpublikum 65+ zählt Möblierbarkeit/Alltagstauglichkeit vor räumlicher Idee — der Sieger punktete genau dort (Zirkulationsschlaufen UND möblierbare Räume, S. 18).
5. **Terrain als Entwurfsgeber:** Gleiche Dachkante zweier Häuser am Hang wurde als «ortsbaulich unverständlich» abgestraft (S. 50); der Sieger nahm das fallende Terrain im Sockel auf (S. 18). Am Hang muss das Volumen das Terrain nachvollziehbar abbilden.
6. **Bestand schonen zählt doppelt:** Eingriffe in Haus A flossen beim Ausscheiden («erhebliche statische Eingriffe», S. 50) wie beim Sieg («geringe statische Eingriffe», S. 19) explizit in das Urteil ein — Eingriffstiefe in den Bestand früh minimieren und ausweisen.

## Offene Fragen

- Kennwort «Juli» auf den Abgabeplänen selbst nicht verifiziert (A0-PDFs `08_Abgabe/01_Plakate_A0/01–04_Plan.pdf` liessen sich mit pdftotext nicht in nützlicher Zeit extrahieren); Beleg bisher nur via Jurybericht S. 49. Sichtkontrolle eines Plans oder des Verfassercouverts steht aus.
- Diskrepanz Projekttitel: Der eigene Projekttext (`05_Texte/Guggerbach.docx`) nennt das Projekt durchgehend «Oberhaus & Underhuus»; der Jurybericht führt den Beitrag als «Juli». Vermutlich Arbeitstitel vs. finales Kennwort — mit Raphael klären. Der Text enthält zudem den Vermerk «Weitere Text nicht bearbeitet (Stand Hittnau)», d.h. Teile stammen aus einem anderen Wettbewerb (Hittnau) und sind evtl. nicht abgabeidentisch.
- Eigene Kennzahlen (GF/GV, Kosten) fehlen: `08_Abgabe/02 Formulare/A06_Datenblatt Kosten_Mengen.pdf` und `_Titelblatt.pdf` liefern keinen extrahierbaren Text (vermutlich Vektor-/Bildexport); `A06_Datenblatt Kosten.xlsx` noch nicht ausgewertet.
- Rolle von Raphael Jans im Team: laut Verfassernachweis Mitarbeiter (Entwurfsverfasser Andrea Marco Zarn, Atelier Arpagaus Sommer Zarn) — Anteil/Verantwortung von Raphael im Entwurf mit ihm klären (relevant für DNA-Gewichtung dieser Teilnahme).
- PQ-Bewerbung (Kriterien, Referenzen, Nachwuchs-Status-Begründung) noch nicht destilliert (`12_PQ/06_Abgabe Bewerbung/`, `12_PQ/07_Entscheid/`).
