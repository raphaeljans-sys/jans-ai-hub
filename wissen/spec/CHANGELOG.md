...
- **F-ENV1 gelöst, exemplarisch am eigenen Artikel:** die seit Run 20 (29.07.) offene
  Bring-Schuld «Zählstand-Nachführung» ist eingelöst. `wiki/environment-jans-hub.md` trägt
  jetzt einen datierten Nachmessungs-Abschnitt mit fünf Kennzahlen (Skills 51, KBs 20,
  Rule-Dateien 29 davon 24 @-importiert, CLAUDE.md 19'014 B) **plus dem exakten, reproduzierbaren
  Befehl je Kennzahl** statt einer eingefrorenen Zahl. Alle fünf Werte sind seit der letzten
  Korrektur (29.07.) gewachsen — belegt die F-ENV1-These direkt am eigenen Text. Ältere
  Zählstand-Absätze bleiben als datierter historischer Beleg stehen, gelten aber nicht mehr
  als Ist-Stand.
- **Reue-Checks ohne neuen Fund:** F1 (Überdehnungs-Gegentest) weiterhin unbelegt; F3
  (Marketing/Text als letzte offene Verifier-Domäne) weiterhin kein realer Fall trotz gezielter
  Suche; SYN-02-Codeseite laut `wissen/koordination/SYNERGIE-REGISTER.md` (Stand 13.08.2026)
  unverändert offen, Mandat beim Synergie-Orchestrator.
- Geändert: `wiki/environment-jans-hub.md`, `wiki/QUESTIONS.md` (F-ENV1 gelöst, F1 + F3
  Reue-Check-Stand ergänzt). Bericht: `outputs/2026-08-24_training-run39.md`.
...
- **[struktur]** Die Programmdatei trug keinen Hinweis darauf, dass `spec-training` seit dem
  **26.07.2026** (Entscheid Raphael, Registry-Nachzug 27.07.) auf `enabled=false` steht und nur
  noch **auf Zuruf bei realer Spec-Anwendung** laeuft. Ihr Abschnitt «Takt & Umfang (verbindlich)»
  las sich damit wie ein laufender Kalendertakt.
- **Warum das zaehlt:** die Mac-Mini-Nachtschicht waehlt in **Prioritaet 4** ueber
  ...
  Programmdatei, nicht das Task-Register. `spec` war mit CHANGELOG-Stand 27.07. 22:21 die
  **zweitaelteste** von neun Kandidatinnen.
- **Massnahme:** Statuskopf gesetzt (Ereignis-Trigger, kein gueltiges Ziel fuer Prioritaet 4;
  «Takt & Umfang» beschreibt den Umfang eines ausgeloesten Laufs, nicht einen Takt). Kein
  inhaltlicher Eingriff, keine Aenderung an Takt oder Register.
...
- **P1 (mail@-Blindfleck `ag-gruendung-monitor`) hält weiter:** SKILL.md-mtime unverändert
  21.07.2026 07:49; `fristen.md` (Radar-Stand 25.07.) bestätigt UBS 29 Tage ohne Antwort.
- **Neuer Befund:** der in Rule `auto-verbesserungen.md` (260725) gemeldete Fix — `spec-
  training` per `update_scheduled_task` auf 2x täglich (04:10/16:10) gesetzt, aus der
  VOLLGAS-Runner-`EXCLUDE_RE` ausgenommen — wirkt am Nachmittag nicht wie beschrieben:
  Läufe 28–34 feuerten zwischen 13:35 und 20:58 im Schnitt stündlich, nicht 2x/Tag. Nur das
  Symptom (Zeitstempel-Diff) verifiziert, Ursache (Runner-Dispatch vs. Cron-Registrierung)
  liegt ausserhalb des KB-Mandats — an Raphael zu klären.
- Environment-Zählstand (49/22/16) unverändert seit Lauf 26 — kein voller Sweep fällig.
- **Bewusst nicht getan:** kein erfundenes F1-Überdehnungsbeispiel; keine Rule-/Skill-/Wiki-
  Änderung ohne neuen methodischen Beleg; kein Eingriff in Runner-Konfiguration/Task-
  Registry; kein Mailversand.
- Offen: F1-Überdehnungs-Gegentest unbelegt; F3 Marketing/Text letzte offene Domänenzeile;
  UBS-Strecke 29 Tage überfällig; stündliches Feuern von `spec-training` trotz gemeldeter
  2x/Tag-Umstellung Raphael vorzulegen.
