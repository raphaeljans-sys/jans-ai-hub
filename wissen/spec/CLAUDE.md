...
liefert die Methodik fuer den Skill `spec` und die Rule `spec-methode` und verbessert sich
kompoundierend ueber den Trainings-Loop (`training/PROGRAMM.md`).
...
- Die **JANS-Auspraegung**: wann anwenden (Gate-Test), wie durchfuehren, Abgrenzung zu
...
- Prompts im Original (englisch) UND mit JANS-Fassung (deutsch) fuehren.
- Jede nicht-triviale Aussage traegt eine Quelle aus `sources` (Transkript/Original/Doku).
- Status ehrlich: `speculative` solange nur via Screenshot-Transkript belegt; auf
  `established` heben, sobald durch Originalquelle (Karpathy/Cherny/Anthropic) bestaetigt.
