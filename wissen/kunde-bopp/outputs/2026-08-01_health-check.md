# Wissens-Health-Check: kunde-bopp — 2026-08-01
...
| F Schreibregel-Verstoesse | 10 |
| G Promotion-/Ruecklauf-Kandidaten | 0 |

## Top-3 (Raphaels Aufmerksamkeit)

1. **5 Backlinks zeigen auf Artikel, die es nicht gibt** (Audit B) — und zwar sowohl aus `INDEX.md` als auch aus `profil-christoph-bopp.md`: `[[auftrags-muster-bopp]]` und `[[zusammenarbeit-lessons-bopp]]` (je zweifach) sowie `[[project_2620_bauherrschaft]]`. Die ersten beiden sind offenkundig geplante, nie geschriebene Artikel; der dritte zeigt auf einen **Memory-Dateinamen**, nicht auf einen Wiki-Artikel.

2. **Nur 2 Sachartikel bei 6 Outputs** — die Prototyp-KB «pro Kunde» ist seit Anlage kaum gewachsen. 0 Outputs seit dem letzten Check (2026-07-25).

3. **10 ae/oe-Verstoesse in 2 Artikeln** (Audit F). Der Befund ist KEIN Einzelfall dieser KB, sondern die hub-weite ae/oe-Drift: der Wrapper `scripts/claude-run.sh` — laut Rule 260729 der verbindliche Einstieg fuer automatische Laeufe — setzt **kein** `LANG`/`LC_ALL`, waehrend `scripts/dispatch-run.sh` seit dem Juli-Fix `de_CH.UTF-8` exportiert (Z. 81/82). Alles, was ueber `claude-run.sh` laeuft (`arbeits-weiche.sh`, `multi-claude.sh`, `vollgas-runner.sh`, `wissens-trigger.sh`), schreibt weiter ae/oe statt ä/ö/ü.
...
- `INDEX.md` — `[[auftrags-muster-bopp]]` existiert nicht
- `INDEX.md` — `[[zusammenarbeit-lessons-bopp]]` existiert nicht
- `profil-christoph-bopp.md` — `[[auftrags-muster-bopp]]` existiert nicht
- `profil-christoph-bopp.md` — `[[zusammenarbeit-lessons-bopp]]` existiert nicht
- `profil-christoph-bopp.md` — `[[project_2620_bauherrschaft]]` ist ein Memory-Dateiname, kein Wiki-Artikel
...
- 10 ae/oe-Treffer in 2 Artikeln
- Keine Deko-Symbole, kein ß

### G. Promotion-Kandidaten

- Beide Artikel auf `emerging`; 0 Outputs seit dem letzten Check
