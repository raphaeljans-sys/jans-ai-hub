...
|---|---|---|---|
| T-Regelgeschoss | **HNF/GF-Regelgeschoss-Faktor 0.70 vs. 0.75** — Wissens-Chef Cross-KB-Run 15 (25.07.2026) hat per `needs-decision`-Box in `wiki/realwert-sachwert.md` (adversarial bestaetigt) einen Widerspruch geflaggt: der Hub-weite E1-Entscheid (12.07.2026) setzt 0.70, der eigene fuehrende Flaechen-Artikel `flaechendefinitionen-sia:84` fuehrt belegte 0.75 (Flaechenkonzeption Wohnen, R. Jans 20.10.2024 — deckungsgleich mit T8/Run-4 unten). **Fachentscheid Raphael**, nicht autonom durch den Loop aenderbar; bis Entscheid gilt 0.70 weiter. Nach Entscheid: kleiner Folge-Task in `residualwertmethode`/`realwert-sachwert`. | `wissen/koordination/outputs/2026-07-25_wissens-chef-run15.md` | realwert-sachwert, residualwertmethode |
...
| T-Eidgenossen-Rendite | **10-j-Eidgenossen-Rendite nicht datumsscharf belegbar** (Run 51, 27.07.2026). Der KB-Anker ist ~0.32–0.40 % (Juni 2026, Run 9). Die Web-Recherche fand im Juli 2026 nur **undatierte Live-Snapshots (~0.42–0.51 %)** ohne extrahierbaren Stichtag; das SNB-Datenportal (`data.snb.ch`, Reihe `rendoblim`/`rendoeid`) ist ein dynamisches Portal und per WebFetch nicht als Tabelle auslesbar. **Wert bewusst NICHT uebernommen** (Leitplanke: kein Wert ohne Datumsbeleg) — der KB-Wert bleibt stehen. Auto-schliessbar, sobald der CSV-Export von data.snb.ch angesprochen wird ODER eine datierte Sekundaerquelle (SNB-Quartalsheft, BFS) den Monatswert publiziert. Bewertungsrelevanz gering (risikofreies Basisniveau der Diskontsatz-Herleitung, Bandbreite unveraendert sehr tief), aber die moegliche Aufwaertsdrift von ~0.35 % auf ~0.45 % waere fuer [[ertragswert-dcf]] zu verfolgen. | data.snb.ch (CSV-Export) bzw. datierte Sekundaerquelle | investorenmarkt-makro, ertragswert-dcf |
