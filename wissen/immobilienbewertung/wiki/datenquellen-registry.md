...
| UBS Swiss Real Estate Bubble Index (SREBI) | `IMMO-02 UBSFS/srebi-q2-2024-de.pdf` (Ablage) + Web (ubs.com CIO) | nationaler Blasenrisiko-Index, regionale Risikokarte | **Q2-2026 = 0.72 «moderat»** (publ. 06.08.2026; abgelegtes PDF weiterhin Q2-2024) | Makro-Kontext, laufend via Web nachgefuehrt ([[investorenmarkt-makro]]). ⚠ **wird rueckwirkend revidiert** — Q1-2026 zwischen Mai- und August-Publikation von 0.69 auf 0.62 gesenkt; nie ohne Publikationsdatum zitieren, nie Werte aus zwei Quartalsberichten mischen (Marktpuls 13.08.2026). **Status `established` seit Marktpuls 01.09.2026** — der Q2-2026-Wert ist am UBS-Originalbericht (8 S., publ. 06.08.2026 06:00 UTC) verifiziert, samt den vier Perspektiv-Scores und der regionalen Karte. ⚙ **Zugangsweg** (die Sackgasse vom 13.08.2026 ist aufgeloest): das PDF ist ueber den **JCR-Link der UBS-Indexseite** abrufbar (`ubs.com/global/en/…/ubs-swiss-real-estate-bubble-index/_jcr_content/…/srebi-qN-JJJJ-de.pdf`); ein direkter `curl` auf denselben Pfad wird mit **HTTP 403** abgewiesen, der Weg fuehrt nur ueber WebFetch. Linkliste je Quartal auf der Indexseite selbst. |
...
| **SNB «Aktuelle Zinssaetze» (Feed)** | Web (`snb.ch/public/rss/de/interestRates`) | **taeglich**: Rendite 10-j. Eidgenossen (R10), SNB-Leitzins, SARON, Sondersatz EFF, Sichtguthaben-Saetze — maschinenlesbar (RSS/RDF), rund 5 Handelstage Historie je Abruf | **R10 = 0.469 % per 31.08.2026**; Leitzins **0.00 %**, SARON **−0.05 %** (Abruf 01.09.2026) | ⭐⭐ **amtlich, tagesaktuell, ohne Login** — loest das Quartalsheft als Bezugsquelle fuer das **risikofreie Basisniveau** ab (aufgenommen Marktpuls 01.09.2026; vorher trug die KB einen bis zu 3 Monate alten Heft-Wert). Fuer die erzaehlende Einordnung bleibt das Quartalsheft. |
| **Leerwohnungszaehlung (BFS + Kantone)** | Web: bfs.admin.ch (CH), zh.ch (ZH), `data.sz.ch` Explore-API v2.1, Datensatz `leerstehende-wohnungen-und-leerwohnungsziffer` (SZ) | amtlicher **Leerstands-Anker**, Stichtag **1. Juni**; Kantone publizieren im **August**, das BFS die CH-Zahl im **September** | **ZH 0.52 % per 01.06.2026** (4'189 Whg., publ. 18.08.2026; Neubau BJ 2024+ **3.02 %**); **CH und SZ per 01.06.2026 noch nicht publiziert** — letzter Stand CH **1.00 %** (48'455 Whg., publ. 09.09.2025), SZ **0.52 %** (440 Whg., 2025) | ⭐ amtlich — Belegquelle fuer den Parameter «struktureller Leerstand» in [[residualwertmethode]]. ⚠ **Kantonswert nie als CH-Proxy** verwenden (ZH 0.52 % gegen CH 1.00 %, Faktor zwei); ⚠ **Neubau-Ziffer nie mit der Bestandes-Ziffer verwechseln** (Nutzungsregel 9). |
...
7. **Bau-Teuerung immer mit der passenden Bauwerksart indexieren, nie mit dem Totalwert** (neu
   ...
   *(Regel **8** ist die Auflagen-Disziplin fuer Fachliteratur und steht im Abschnitt
   «Standardwerke und Fachliteratur» weiter oben.)*
9. **Leerstands-Annahmen nach Segment trennen, nie eine Ziffer fuer alles** (neu 01.09.2026,
   Marktpuls). Die amtliche Leerwohnungsziffer misst den **Bestand**; im Neubausegment liegt sie
   um ein Mehrfaches hoeher (Kanton ZH per 01.06.2026: Bestand **0.52 %**, Neubau Baujahr 2024+
   **3.02 %**). Eine Residualwert- oder DCF-Rechnung auf einem Neubauprojekt mit dem
   Bestandes-Basisfall von 1 % strukturellem Leerstand unterschaetzt die Erstvermietungsphase
   systematisch; die Vermarktungsphase gehoert mit dem Neubau-Wert oder als eigener Carry
   gerechnet ([[residualwertmethode]], [[projektstruktur-deliverables]]). Ebenso wenig ist ein
   Kantonswert ein Landeswert.
10. **Fuer den risikofreien Zins den taeglichen SNB-Feed nehmen, nicht das Quartalsheft**
   (neu 01.09.2026, Marktpuls). Dieselbe Groesse (R10) ist ueber `snb.ch/public/rss/de/interestRates`
   tagesaktuell und amtlich verfuegbar; das Quartalsheft liefert sie mit bis zu drei Monaten
   Verzug. Das Heft bleibt fuer die **Begruendung** der Zinsentwicklung die bessere Quelle, der
   **Zahlenwert** kommt aus dem Feed. Gleiche Familie wie Regel 5: die Publikation, aus der ein
   Wert stammt, gehoert zum Wert dazu.
