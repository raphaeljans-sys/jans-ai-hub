...
|---|---|---|---|---|---|
| SYN-01 | grobkosten-rechner · kostenschaetzung · realwert-rechner · wirtschaftlichkeit-rechner | 2 | Vier Instanzen nutzen Kosten-Kennwerte aus je eigener Quelle (KB `wissen/grobkosten`, Wuest-Referenzen, Bewertungs-KB, Skill-interne Werte). Gefahr divergenter Zahlen im selben Projekt. | `wissen/grobkosten` zur EINEN fuehrenden Kennwerte-DB erklaeren; die drei anderen beziehen Kennwerte von dort und schreiben neue Datenpunkte dorthin zurueck. | halb geschlossen (Lauf 03, 01.08.): `skills/kostenschaetzung/SKILL.md` Z. 84 und `agents/grobkosten-rechner.md` (4 Treffer) sind an `wissen/grobkosten` angeschlossen. **Offen bleiben die zwei Bewertungs-/Wirtschaftlichkeitsrechner:** `agents/realwert-rechner.md` und `agents/wirtschaftlichkeit-rechner.md` je 0 Treffer **Nachtrag 04.09.2026 (Lauf 28), Beteiligter bewegt, erste konkrete Sachfolge der offenen Haelfte:** Die offene Haelfte ist `agents/realwert-rechner.md` (weiterhin **0** Treffer auf `wissen/grobkosten`, ebenso `wirtschaftlichkeit-rechner` — in diesem Lauf nachgemessen). Am 03.09. hat Wissens-Chef Lauf 52 (`3e6f1cc76`) den SIA-416-Aussenflaechen-Vorbehalt in fuenf Bausteine gesetzt, darunter **den Wissensartikel, den dieser Agent bedient** (`wissen/immobilienbewertung/wiki/realwert-sachwert.md` Z. 512-519) — **den Agenten selbst nicht**. Er ist der direkteste Betroffene: Z. 33 rechnet den Neuwert ausdruecklich «Flaeche (aGF/BGF) bzw. Volumen (GV, m3) x Kostenkennwert», und Z. 41 mahnt sogar «Bezugsflaeche immer angeben (aGF vs. BGF vs. GV nicht verwechseln, SIA 416)» — er ist fuer genau diese Frage sensibilisiert und kennt den Vorbehalt trotzdem nicht. Damit ist die bisher abstrakte Doppelrechnungs-Diagnose erstmals mit einer Sachfolge belegt: nicht divergente Zahlen, sondern ein Vorbehalt, der die Bewertungslinie nicht erreicht, **weil sie nicht an der Kennwerte-Linie haengt**. **Nachtrag bewusst ohne Reparatur** — der Vorbehalt ist zur Haelfte selbst unbelegt (SYN-72), er gehoert erst nach dem Entscheid der KB `normen` in den Agenten. Kein neuer Befund zur Doppelrechnung, nur Nachfuehrung |
...
| SYN-43 | rules/anrede-kontakte.md (importiert) · rules/jans-dna-facetten.md (nicht importiert) · agents/email · agents/twin-beziehung · skills/korrektur | 1 | **Dieselbe Anrede-Frage steht in zwei Dateien, und die operative Anweisung der importierten Fassung ist am selben Korpus als verfehlt gemessen.** `rules/anrede-kontakte.md` (Commit `fe6224f0`, 22.08. 23:30, 10/0) traegt die Treffen-Klausel weiterhin mit Sofortwirkung: «die neue Stufe bleibt danach» und als praktische Folge «vor einer Mail an einen Fachplaner den Kalender des laufenden Tages pruefen; hat ein Treffen stattgefunden, ist die Anrede des letzten Threads ueberholt» (Z. 41-47). Der am selben Tag angehaengte Gegenbeleg korrigiert nur die Achse (kein Wechsel Sie → Du), nicht die Sofortwirkung. Sieben Stunden spaeter praezisiert `rules/jans-dna-facetten.md` (Commit `d4d79ced`, 23.08. 06:00, 26/26) dieselbe Frage eine Ebene tiefer: die Stufe sei eine **Trajektorie, kein Standbild**, der Kanalwechsel wirke **verzoegert, nicht sofort** (Furrer 24.07. → 28.07.; Stadelmann 22.07. trotz Telefonat unveraendert → erst 03.08.), und der belegte Opener gelte **innerhalb** des Vorgangs, nicht ueber Wochen; die Ausloeser Cc, Gewicht und Telefonat heben im selben Vorgang nicht automatisch (**260822, 3/3 verfehlt**). **Richtungsprobe:** `jans-dna-facetten` nennt `anrede-kontakte` (Z. 235), zurueck verweist nichts (`grep -c` = 0). Damit traegt die **immer geladene** Datei die groebere Fassung und die **nicht importierte** die belegte Praezisierung — die Asymmetrie zeigt in die ungeguenstige Richtung. Betroffen sind besonders die neun Bausteine ohne Facetten-Zeiger aus SYN-22. **Ehrliche Abgrenzung:** keine harte Kappung — der importierte Kern `jans-dna.md` weist jede Session an, die Facetten vor einem ausgehenden Text zu lesen; wer das tut, sieht die neuere Fassung. Verschoben hat sich die Verlaesslichkeit, nicht die Erreichbarkeit. | Verweis-Nachtrag in `rules/anrede-kontakte.md` gesetzt (9/0, 106→115 Zeilen, kein globales Ersetzen, Anker vorher auf 1 Treffer geprueft): zeigt auf die praezisere Fassung, benennt die gemessene 3/3-Verfehlung und laesst den Wortlaut der Klausel unangetastet. **Offen bleibt die Sachfrage** — die beiden Fassungen zusammenzufuehren heisst, den Regeltext zu aendern, und beide Dateien fuehren die Klausel ausdruecklich als «Entscheid Raphael ausstehend» (`wissen/twin/wiki/QUESTIONS.md` 260821 #1, 260822b #1). Wieder aufzurollen, sobald dieser Entscheid vorliegt. | teilweise umgesetzt (active-with-flagging, Lauf 16) **Lauf 24 (31.08.): Beteiligter bewegt, Muster unveraendert.** `rules/jans-dna-facetten.md` hat am 31.08. 06:02 (Commit `92545f372`, twin-fidelity-review) eine weitere praezise Anrede-Klausel aufgenommen — «Auch die Zahlungserinnerung ist namenlos, selbst im warmen Du (260831)». Die importierte Fassung `rules/anrede-kontakte.md` kennt sie nicht (0 Treffer, letzte Aenderung 27.08.). Der Verweis-Nachtrag aus Lauf 16 traegt weiterhin (Z. 57); neue Praezision landet weiterhin ausschliesslich in der nicht importierten Datei, die Sachfrage bleibt beim ausstehenden Entscheid. **Lauf 25 (01.09.): Beteiligter erneut bewegt, Muster unveraendert.** `rules/jans-dna-facetten.md` hat am 01.09. 06:29 (Commit `5141c74e3`, twin-fidelity-review, +6/−5) die Apparat-Regel 260901 aufgenommen — «Der Apparat gehoert der Antwort, nie der FRAGE … die erbetene Auskunft nennt Gegenstand und Alternative, keine Norm, auch gegenueber dem Fachplaner». Sie ist die Praezisierung derselben Frage, die importiert als `auto-verbesserungen` 260820 («Abklaerungen kurz halten») steht; dort ist sie nicht nachgetragen (`grep -c "Apparat geh"` ueber `rules/jans-dna.md`, `auto-verbesserungen.md`, `dokument-layout-standard.md` = 0). **Die fuenf Loeschungen sind geprueft und unbedenklich:** gestrichen wurden ein Beispielzusatz («acht Empfaenger, einer zustaendig») und ein Rueckverweis auf die Zeilenregel, die 40 Zeilen hoeher unveraendert steht («Ob ein- oder zweizeilig gestapelt wird, entscheidet das GEWICHT der Mail»). Dokumentierte Verdichtung, kein Substanzverlust — dritter Lauf in Folge, in dem eine Zeilenbilanz mit Loeschungen sich als harmlos erweist. | **Lauf 26 (02.09.):** `rules/jans-dna-facetten.md` hat sich zum **vierten Mal** in Folge bewegt (Commit `2a0489e92`, 02.09. 06:00, 14/15) — die Compounding-Klausel greift zum vierten Mal. Die Sachfrage der Anrede-Asymmetrie ist unveraendert (kein Commit auf `rules/anrede-kontakte.md` im Fenster, `git log --since` = 0); Status bleibt «teilweise umgesetzt». **Neu ist der Grund der Bewegung, nicht ihr Inhalt:** die Verdichtung dieses Laufs hat erstmals eine Regel geloescht statt Beleg-Prosa — siehe SYN-69. **Lauf 33 (10.09.): erster Gegenfluss.** Zum ersten Mal ist eine Praezisierung aus der twin-Seite in die **importierte** Rule gewandert statt umgekehrt: der Wissens-Chef Lauf 56 hat den Stadelmann-Befund (Sie → Du am 07.09.2026, Fidelity 260909 Befund 3) als ⚠⚠-Vermerk in `rules/anrede-kontakte.md` nachgetragen (`1081dfdd7`, 16/0, Wortlaut der Klausel unangetastet). Richtungsprobe: der Vermerk nennt `rules/jans-dna-facetten.md` (Trajektorie) und `wissen/twin/wiki/QUESTIONS.md` 260909b #6; das Kompilat fuehrt «Geschätzter Othmar» bereits im Du (Z. 242). Beide Seiten stimmen ueberein. Die Asymmetrie ist nicht aufgehoben (neue Praezision entsteht weiterhin nur in der twin-KB), sie hat aber erstmals einen Rueckkanal. Der Verweis-Nachtrag aus Lauf 16 zeigt weiterhin auf eine tragfaehige Fassung: die Riegel-Kuerzung vom 10.09. an genau dieser Stelle ist in SYN-80 gegengeprueft und ohne Bedingungsverlust.
...
| SYN-64 | scripts/nas-auto-mount.sh · scripts/ensure-nas-mounted.sh · scripts/nas-keepalive.sh · scripts/heartbeat.sh · Rule betrieb-chronik 260824 | 1+3 | **Drei Waechter teilen dieselbe blinde Annahme, und der bekannte Ausfall passt genau hinein.** Alle pruefen den NAS-Zustand ueber die Frage «liegt ein Mount auf `/Volumes/daten`?» — `nas-auto-mount.sh` Z. 70 und Z. 173, `ensure-nas-mounted.sh` Z. 45-46 (`ready()`), `nas-keepalive.sh` Z. 46; `heartbeat.sh` Z. 51 fragt gleichbedeutend `ls "$NAS_BASE/"`. macOS haengt eine SMB-Freigabe aber unter `/Volumes/daten-1` ein, wenn der Name `daten` im Moment des Mountens noch belegt ist — genau den Zustand erzeugt `nas-auto-mount.sh` Z. 76 mit `umount -f` selbst. Danach schliesst sich die Falle: `mount volume` Z. 151 meldet OK, weil die Freigabe unter anderem Namen laengst haengt, die Verifikation Z. 173 findet nichts und schreibt ERROR — im Kreis, ohne Ausgang. Die Mountpunkt-Vorbereitung Z. 142-145 raeumt nur ein verwaistes Verzeichnis auf, keine Fremdmontage. **Am 31.08. gemessen:** letzter `[OK]` 13:23:25, danach 83 `[ERROR]`-Zeilen bis 17:19:34 in `.git/nas-auto-mount.log`, der Mount lag um 17:11 nachweislich auf `/Volumes/daten-1`, `ensure-nas-mounted.sh` protokollierte um 13:21:10 den sauberen Abbruch (rc 1). Rund vier Stunden, in denen jeder headless-Lauf folgenlos abbrach und dieser Aufsichtslauf selbst ohne Skills, Agenten und importierte Rules startete (SessionStart-Waechter). **Der Ausfallmodus ist seit dem 24.08. beschrieben** (`rules/betrieb-chronik.md` Z. 817, «dort lag der Mount zeitweise auf /Volumes/daten-1»), aber kein Script kennt ihn: Grep `daten-[0-9]` ueber `scripts/`, `rules/` und `skills/heartbeat/` trifft ausschliesslich diese eine Chronikzeile. | Vor dem Mountversuch die Freigabe unter Fremd-Mountpunkt suchen und sauber trennen (`mount | grep "/${SMB_SHARE} on /Volumes/${SMB_SHARE}-"` → `umount`), erst danach `mount volume`; dieselbe Erkennung als Diagnose in `ensure-nas-mounted.sh` und `nas-keepalive.sh`, damit der Zustand benannt statt nur gezaehlt wird. Ohne diese Pruefung heilt keiner der drei Waechter den Zustand, den der erste selbst erzeugt. **Nicht in diesem Lauf gebaut** — Eingriff in den Mount-Weg der Station, das gehoert nicht nebenbei in einen Aufsichtslauf (Freigabe-Schwelle). Manuell geheilt wurde nur die laufende Station, protokolliert im Laufbericht und in der Chronik. | offen — gemeldet 31.08.2026 (Lauf 24), Entscheid Raphael **Lauf 25 (01.09.): unveraendert offen.** Keiner der vier Waechter wurde seit der Meldung angefasst (`git log --since` ueber `nas-auto-mount.sh`, `ensure-nas-mounted.sh`, `nas-keepalive.sh`, `heartbeat.sh`: leer), und `grep -rln 'daten-[0-9]'` ueber `scripts/` trifft weiterhin **keine** Datei — die Fremdmontage-Erkennung fehlt unveraendert. Auf der laufenden Station lag der Mount in diesem Fenster korrekt auf `/Volumes/daten` (`mount | grep -i daten`), `/Volumes/daten-1` existiert hier nicht; der akute Zustand ist geheilt, die Konstruktionsluecke nicht. |
| SYN-65 | skills/heartbeat/SKILL.md · scripts/heartbeat.sh · scripts/git-divergenz.sh · Scheduled Task heartbeat-daily · scripts/nas-selfcommit.sh | 1 | **Die Divergenz-Messung existiert, der taegliche Waechter fuehrt sie nicht aus.** `scripts/git-divergenz.sh` wurde am 24.08. genau fuer diesen Fall gebaut — Kopfkommentar Z. 8-12: «NAS und GitHub liefen sechs Stunden auseinander (26/51 Commits), ohne dass es jemand sah … Dieser Check ist das Fenster darauf». Aufgerufen wird er an genau einer Stelle: `skills/heartbeat/SKILL.md` Z. 313-317. Der Scheduled Task `heartbeat-daily` fuehrt aber nicht den Skill aus, sondern das Script («bash /Volumes/daten/jans-ai-hub/scripts/heartbeat.sh ausfuehren und Status berichten»), und `heartbeat.sh` prueft in Abschnitt 2 (Z. 68-83) nur `git status --short` des SSD-Klons, also sauber gegen schmutzig, nie ahead/behind. Grep `git-divergenz` ueber `scripts/`, `skills/`, `rules/`, `commands/` und alle 33 Scheduled Tasks: **ein einziger Treffer**, die SKILL.md-Zeile. Damit liegt die Gegenmassnahme im Text und nicht im ausfuehrenden Pfad (Muster SYN-56). **Folge, heute gemessen:** das NAS-Repo steht seit dem 29.08. 22:04 (merge-base `01204714a`) quer zu GitHub, `main` ist **ahead 471 / behind 267**, der native Selfcommit meldet seit 29.08. 21:51 durchgehend «Konflikte brauchen ein Urteil», der Zaehler `sync-tasks/log/selfcommit-fehlversuche` steht auf **555**. Dieser Zaehler liegt in genau dem Ordner, den `heartbeat.sh` Abschnitt 5 (Z. 136-151) ohnehin abzaehlt — eine Datei daneben, ungelesen. Gefunden wurde die Divergenz beide Male von einem Synergie-Lauf, nie vom Waechter. | Zwei kleine Zeilen in `scripts/heartbeat.sh`: (a) in Abschnitt 5 den Zaehler `sync-tasks/log/selfcommit-fehlversuche` mitlesen und jeden Wert ueber 0 als Warnung ausgeben — der Ordner wird dort schon angefasst, die Messung kostet nichts; (b) `git-divergenz.sh` als eigenen Abschnitt aufrufen, sofern NAS und ssh erreichbar sind. Alternativ den Scheduled Task auf den Skill statt auf das Script umstellen, dann greift die bereits dokumentierte Pruefung ohne Codeaenderung. **Nicht in diesem Lauf gebaut** (Aenderung am taeglichen Waechter, Freigabe-Schwelle). | offen — gemeldet 31.08.2026 (Lauf 24), Entscheid Raphael **Nachgemessen im selben Lauf, verschaerfend:** `logbuch/fristen.md` fuehrt zu derselben Divergenz mindestens sieben datierte Nachtraege aus sieben verschiedenen Laeufen (30.08. 06:5x Logbuch-Radar 173/94, 30.08. 23:0x energie-training 364/245, 31.08. 02:50 wettbewerbs-dna-training 396/266 und weitere), jeder mit derselben Messung von Hand nativ per ssh. Das ist neben der Ketten-Luecke auch eine Doppelrechnung: dieselbe Zahl wird in jedem Nachtlauf neu erhoben, weil kein Waechter sie meldet. Die **Ursache** der Divergenz ist davon unabhaengig laengst gefunden und gespect (`docs/konzepte/260830-Laufzeitschicht-Umbau/SPEC.md`, Befund B6: `dispatch-run.sh` Z. 33 pinnt das Arbeitsverzeichnis auf den SSD-Klon) — SYN-65 zielt nicht auf die Ursache, sondern auf die fehlende Meldung. **Lauf 25 (01.09.), nachgemessen und deutlich verschaerft:** die Divergenz ist in 24 h um **121 Commits** gewachsen — `main` steht jetzt **592 voraus / 267 zurueck** (Lauf 24: 471/267), der Zaehler `sync-tasks/log/selfcommit-fehlversuche` auf **681** (Lauf 24: 555), also **126 weitere Fehlversuche**. Entscheidend ist die andere Richtung: der GitHub-Kopf steht unveraendert bei `66df04125` vom **31.08. 06:12** — im gesamten Lauf-Fenster ist **kein einziger** Commit nach GitHub geflossen (`git log --since='2026-08-31 17:10' origin/main | wc -l` = **0**), waehrend das NAS 122 Commits erzeugte. Das Backup ist damit seit ueber 35 Stunden ohne Zufluss. Der native Selfcommit meldet weiterhin im Minutentakt «abgleich fehlgeschlagen (rebase und merge, Konflikte brauchen ein Urteil)» (Log 01.09. 17:00:07). Gefunden hat es erneut dieser Lauf, nicht der Waechter — dritter Lauf in Folge. Die **Ursache**-Seite ist jetzt eigenstaendig als SYN-66 erfasst (B6-Bestandsaufnahme unvollstaendig). |
| SYN-66 | scripts/dispatch-run.sh · scripts/vollgas-runner.sh · scripts/arbeits-weiche.sh · scripts/nachtschicht-run.sh · docs/konzepte/260830-Laufzeitschicht-Umbau/SPEC.md Befund B6 · Rule sync-kanonische-quelle (importiert) | 1 · 6 | **Die Bestandsaufnahme zu B6 nennt einen Startpfad, es sind vier — wer B6 nach der Spec behebt, laesst drei offen.** SPEC Z. 27 fuehrt B6 woertlich als «Kein kanonischer Arbeitsbaum fuer automatische Laeufe» und belegt ihn an genau einer Stelle: «`dispatch-run.sh` Z. 33 pinnt `REPO="${DISPATCH_REPO:-$HOME/Developer/jans-ai-hub}"`». Z. 61 leitet daraus die Anforderung ab, «in genau EINEM Arbeitsbaum» zu laufen (Entscheid Raphael). **Drei weitere Starter tun dasselbe und kommen in der Spec nicht vor** (`grep -c "arbeits-weiche"` ueber SPEC.md = **0**): `vollgas-runner.sh` Z. 73 `REPO="${VOLLGAS_REPO:-$HOME/Developer/jans-ai-hub}"` + Z. 162 `cd "$REPO"` — konstruktionsgleich, bis zum env-Override; `arbeits-weiche.sh` Z. 186 und Z. 191 `cd ~/Developer/jans-ai-hub && … claude-run.sh`, also zweimal im selben Script; `nachtschicht-run.sh` Z. 155 dasselbe Muster. Alle vier starten **Claude-Laeufe**, nicht Wartungsarbeit — der Unterschied zu den sechs legitimen Treffern desselben Greps (`git-divergenz.sh`, `nas-commit-now.sh`, `install-*.sh`, `neue-station.sh`, `setup-ssh-macbook.sh`), die im Klon arbeiten **muessen**. Damit verletzt der Hub die importierte Rule `sync-kanonische-quelle` («geteilte Inhalte NUR auf dem NAS-Pfad editieren — nie im SSD-top-level») auf vier Wegen statt auf einem. **Adversariale Gegenpruefung:** (a) Steht es woanders? Nein — grep ueber `logbuch/fristen.md`, dieses Register und die SPEC nach den drei Scriptnamen im CWD-/Arbeitsbaum-Zusammenhang: **0 Treffer**. Die Divergenz selbst ist vielfach notiert, ihre Bestandsaufnahme nicht. (b) Latent? Nein — alle drei sind in `templates/launchd/` bzw. von Supervisor-Scripts referenziert, und `logbuch/vollgas/RADAR.md` traegt den Zeitstempel **01.09. 17:15**, also aus diesem Lauf-Fenster. (c) Ist es dieselbe Sache wie SYN-65? Nein — SYN-65 zielt auf die fehlende **Meldung** der Divergenz, dieser Eintrag auf die unvollstaendige Erfassung ihrer **Ursache**. | **Nur gemeldet, kein Eingriff.** Die vier Startpfade auf den NAS-Arbeitsbaum umzustellen ist genau die Entscheidung, die Z. 61 der SPEC Raphael vorbehaelt; ein Aufsichtslauf aendert weder ausfuehrende Scripts (Praezedenz SYN-49/56/61/62) noch ein Konzeptdokument, das einen offenen Entscheid traegt. Empfohlen wird, die drei fehlenden Startpfade in B6 nachzutragen, bevor die Massnahme gebaut wird — sonst misst die Erfolgskontrolle an einem Viertel des Bestands. | offen — gemeldet 01.09.2026 (Lauf 25), Entscheid Raphael / Betriebsschicht |
| SYN-67 | rules/betrieb-chronik.md 260901 (NICHT importiert) · Rule sync-kanonische-quelle (importiert) · Rule auto-verbesserungen 260730b Ziff. 2 (importiert) · alle Sessions auf NAS-Pfaden | 1 · 3 | **Ein gemessener Weg, unbemerkt am kanonischen Repo vorbeizuschreiben, steht ausschliesslich in der nicht importierten Chronik.** `rules/betrieb-chronik.md` haelt seit dem 01.09. (Commit `caf46faaa`, +14/−0) fest: ein fehlgeschlagenes `cd` auf den NAS-Pfad bricht die Shell **nicht** ab, alle nachfolgenden **relativen** Pfade greifen still im SSD-Klon (CWD bleibt `~/Developer/jans-ai-hub`); Lesevorgaenge liefern dann inhaltlich richtige Daten, «**nichts** im Output deutet darauf hin, dass die kanonische Quelle gar nicht angefasst wurde», und ein **Schreib**vorgang «waere beim naechsten `git pull` spurlos verschwunden». Die Chronik ordnet den Fall selbst ein: «Gleiche Familie wie 260730b Ziff. 2 — dort las man das Falsche, hier schreibt man am Richtigen vorbei.» **Die importierte Schicht kennt nur die Lese-Haelfte.** `auto-verbesserungen` 260730b Ziff. 2 warnt vor dem veralteten Lesestand ueber SMB; die Schreib-Haelfte fehlt. Gemessen ueber die sechs Dateien des Grundkontexts (`sync-kanonische-quelle`, `auto-verbesserungen`, `interaktive-eingriffe`, `wege-und-vollmachten`, `wissens-bibliothekar`, `CLAUDE.md`) nach `CWD|relative[rn]? Pfad|Arbeitsverzeichnis|cd .*exit 1`: **0 Treffer in allen sechs**. **Der Befund trifft die Session-Schicht, nicht die ausfuehrende** — und das ist die ehrliche Zuspitzung: in `scripts/` ist das Muster praktisch abwesend (strenger Grep auf ungesichertes `cd` in NAS-Pfade: **0 Zeilen**). Gefaehrdet ist, was eine Session tippt, und genau dafuer ist der Grundkontext zustaendig. Dass `sync-kanonische-quelle` das NAS-Editieren **anordnet**, schuetzt nicht: wer die Regel befolgt und `cd /Volumes/daten/…` schreibt, haelt sich fuer sicher — der Ausfallmodus liegt hinter der Regel, nicht vor ihr. **Selbstbeleg aus diesem Lauf:** um 17:2x riss der Mount transient ab, ein Zugriff auf `rules/auto-verbesserungen.md` scheiterte mit «No such file or directory», `mount | grep -i daten` zeigte den Mount unmittelbar danach wieder intakt (Idle-Stall-Signatur nach Chronik 260831b, 59 Abrisse am 31.08.). Sichtbar wurde der Abriss **nur, weil der Zugriff absolut adressiert war**; relativ waere er still in den Klon gelaufen. | Querverweis in `rules/sync-kanonische-quelle.md` gesetzt (active-with-flagging): ⚠-Absatz im Abschnitt «NIE», der den Ausfallmodus benennt, auf Chronik 260901 zeigt und die Konsequenz nennt — auf NAS-Inhalte **absolut** zugreifen, ein `cd` dorthin mit `|| exit 1` absichern, nie mit stillem Fallback. Kein Regelinhalt geaendert, kein Script angefasst. Die Sachfrage, ob die Warnung in den importierten Regeltext selbst gehoert, bleibt offen — den Wortlaut einer Rule aendert dieser Lauf nicht. | teilweise umgesetzt (active-with-flagging, Lauf 25); Aufnahme in den Regeltext offen, Entscheid Raphael |
| SYN-68 | scripts/schutzmechanik-selbsttest.sh (Check 6) · scripts/claude-session-waechter.sh (neu 02.09.) · scripts/kontingent-waechter.sh · scripts/speicher-waechter.sh · Rule auto-verbesserungen 260902 (importiert) | 1 | **Der Drift-Waechter erntet Scriptnamen aus den launchd-Plists — verkettete Scripts haben keine Plist und fallen damit aus genau der Pruefung, die fuer ihr Risiko gebaut wurde.** Check 6 in `scripts/schutzmechanik-selbsttest.sh` (Z. 148-170) durchsucht `~/Library/LaunchAgents/*.plist` nach `jans-ai-hub/scripts/<name>.sh` und vergleicht nur die so gefundenen Scripts md5-weise NAS gegen SSD. Selbst gemessen am 02.09.: die Ernte liefert **10 Namen** (claude-autoupdate, git-auto-sync, nas-auto-mount, nas-keepalive, speicher-waechter, station-status, sync-task-run, transcript-rotation, widerruf-queue, wissens-trigger). **Nicht darunter sind die zwei verketteten Waechter** — `kontingent-waechter.sh` (seit laengerem) und der am 02.09. neu gebaute `claude-session-waechter.sh` (Commit `b079d6d2b`, 171 Z.). Beide werden am Ende von `speicher-waechter.sh` (Z. 211-221) mit **SSD-Vorrang** aufgerufen (`SW="$HOME/Developer/..."`, NAS nur als Fallback) — also mit exakt dem Startmuster, dessen Drift Check 6 abfangen soll («Faellt der Commit-/Pull-Zyklus aus, laeuft die Station auf altem Stand, waehrend die Aufsicht 'behoben' meldet», Z. 150-152). **Ehrliche Abgrenzung: heute liegt keine Drift vor** — beide Scripts md5-identisch NAS/SSD nachgemessen. Es ist ein blinder Fleck, kein Defekt. Er wiegt aber schwerer als bei den uebrigen Gliedern, weil `claude-session-waechter.sh` das **einzige zerstoerende** Glied der Kette ist (beendet Session-Prozesse, startet die App im Nachtfenster neu) und weil die Sync-Divergenz aus SYN-65/66 aktuell offen ist, der Pull-Zyklus also nicht als verlaesslich gelten kann. | Check 6 nicht nur aus den Plists ernten, sondern die dort gefundenen Scripts zusaetzlich nach verketteten Aufrufen mit SSD-Vorrang durchsuchen (ein `grep -oE` auf dieselbe Pfad-Signatur im Scriptinhalt genuegt und findet beide Waechter). **Dieser Lauf hat den Selbsttest nicht angefasst** — ein Schutzmechanik-Script zu aendern faellt unter die Klassen «Systemdienste/Persistenz» und gehoert nicht in einen unbeaufsichtigten Aufsichtslauf (Rule `interaktive-eingriffe`). | offen |
| SYN-69 | skills/twin/tools/build_dna.py (MAX_AUTO_BYTES) · rules/jans-dna-facetten.md (nicht importiert, Pflichtlektuere vor jedem ausgehenden Text) · wissen/twin/wiki/arbeitsweise.md · agents/email · agents/dokument · skills/korrektur | 1 · 3 | **Der Wachstums-Riegel zahlt jetzt mit Regeln, und die Deckungszusage fuer die erste geloeschte Regel haelt auf der Ebene nicht, die die Konsumenten lesen.** Am 02.09. wurde aus dem kompilierten Don't-Block der Marker «eine Folge einzeln zu beantwortender Fragen als offene Bullet-Liste tarnen» entfernt; `wissen/twin/wiki/arbeitsweise.md` Z. 4057-4060 nennt beide Gruende offen: er werde «von der 260824-Regel im Do-Block vollstaendig abgedeckt», und «die Streichung diente zugleich dem Riegel (Reserve von 14 B auf 29 B)». **Erste Haelfte geprueft, sie haelt nicht:** die 260824-Regel im kompilierten Block (`rules/jans-dna-facetten.md` Z. 378-381) sagt, die Drift sei Vereinheitlichung und Raphael entscheide je Block neu — sie traegt das Kriterium nicht. Die naechstliegende Deckung ist die 260804-Regel (Z. 348-354), und die prueft eine **andere** Sache: nummeriert wird, «worauf spaeter zurueckverwiesen wird», nicht, was einzeln zu beantworten ist. Das explizite Kriterium steht nur noch im Wiki-Artikel (`arbeitsweise.md` Z. 60-66) — den lesen die Konsumenten nicht, sie lesen die kompilierte Datei. **Zweite Haelfte gemessen:** der Auto-Block steht am 02.09. bei **33'922 B von 34'000** (eigene Nachrechnung, `build_dna.py`-Extraktion lesend nachgebaut, ohne zu schreiben), Reserve **78 B** = 0,23 %; die Warnschwelle 30'000 ist laengst passiert. Damit ist zugleich der offengebliebene Punkt (b) aus Lauf 21 belegt: der Loop misst nur die sechs Do/Don't-Sektionen, die Datei selbst ist 35'197 B gross — die Differenz war korrekt vermutet, jetzt ist sie gemessen. **Der Kopfkommentar von `build_dna.py` Z. 57-60 hat genau diesen Zustand am 18.08. als Grenze benannt**: bei 29 B Reserve wurde 30'000 auf 34'000 angehoben, weil «die naechste Runde Regeln geloescht haette statt Doppelungen — der Punkt, an dem der Riegel schadet statt schuetzt». Zwei Wochen spaeter ist dieselbe Reserve erreicht, und diesmal wurde die Regel geloescht, statt den Entscheid erneut vorzulegen. | Zwei Dinge trennen: (a) das geloeschte Kriterium ist **fachlich unstrittig** und gehoert in den kompilierten Do-Block zurueck, in der Formulierung der 260804-Regel («einzeln zu beantworten» als zweites Kriterium neben «Rueckverweis») — kostet zweistellig Bytes und braucht deshalb (b) den ohnehin faelligen Riegel-Entscheid Raphaels (`wissen/twin/wiki/QUESTIONS.md` #2/#4/#5, dort seit dem 16.08. mehrfach vorgelegt): verdichten, anheben, oder Verdichten kuenftig ohne Rueckfrage. **Dieser Lauf aendert weder `build_dna.py` noch die kompilierte Datei** (Kopfzeile «Nicht von Hand editieren»); Flag in der twin-KB gesetzt. | offen, Entscheid Raphael. **Nachtrag 09.09.2026 (Lauf 32):** derselbe Riegel-Entscheid liegt zusaetzlich als **SYN-78 (b)**, als eigener Eintrag in `logbuch/fristen.md` Z. 49 und im `twin-fidelity-review` vor (dort zum fuenften Lauf in Folge empfohlen; sechs Reports nennen 36'000 B). Gebuendelt in **SYN-80** — die Vorlage ist eine, nicht vier |
| SYN-70 | connectors/WEGE.md (Abschnitt 3 «Belegte Sackgassen») · Rule wege-und-vollmachten Ziff. 3 (importiert) · Rule auto-verbesserungen 260807 Spiegelfall (importiert, neu 03.09.) | 1 · 3 | **Die neue Lehre nennt das Wege-Register als «gleiche Familie» — und das Register erfaehrt nichts davon.** Commit `48adc3c0f` (03.09. 13:02, +15/−0) ergaenzt Rule `auto-verbesserungen` 260807 um den Spiegelfall: «eine festgeschriebene Stoerung wird periodisch gegengemessen, nicht als Dauerzustand fortgeschrieben», belegt an der PATH-Probe des Vollgas-Radars (29.08. als «gewedgtes Binary» diagnostiziert, elf Laeufe lang nicht mehr versucht, am 03.09. in 13 Sekunden mit rc=0 beantwortet, bei **unveraendertem** Binary und Symlink). Der Eintrag nennt ausdruecklich `wege-und-vollmachten` als gleiche Familie. **Richtungsprobe:** `connectors/WEGE.md` hat **0 Treffer** auf `erneut pruef|wiedervorlage|gegenmess|periodisch`; `rules/wege-und-vollmachten.md` nennt 260807 genau einmal (Z. 19) und ausschliesslich wegen Punkt 5 («ein leeres Ergebnis ist zuerst eine Aussage ueber das Werkzeug»), nicht wegen des Spiegelfalls. **Der Befund liegt tiefer als eine fehlende Rueckkante:** Abschnitt 3 des Registers ueberschreibt seine Tabelle kategorisch mit «Wer hier steht, muss nicht nochmals probiert werden» (Z. 429) und mischt darunter zwei Klassen. **Strukturell** (zu Recht endgueltig): SPO-Shell Windows-only, `git` ueber SMB haengt, Logdatei nicht UTF-8, zsh-Arithmetik in doppelten Anfuehrungszeichen. **Zustandsabhaengig** (vergaenglich, trotzdem unter derselben Ueberschrift): `m365 spo tenant setting list` «existiert nicht **in v11.5.0**» (versionsgebunden), `mdfind` ueber `~/Library/Mail` «im Zustand ‹unknown indexing state›» (ein Spotlight-Index repariert sich), der Apple-Mail-`whose`-Filter «auf **frisch** synchronisierten Nachrichten» (Sync-Stand), Graph `403` mangels App-Berechtigung (nachtragbar) und — der Prototyp — Z. 653 «Sackgassen dieser Station (**nicht erneut laufen**): … `ssh` zum Mac Mini scheitert (**Mini seit drei Tagen offline**)», waehrend der Mini seit dem 24.08. wieder im Tailnet ist (Vorfall 260824c behoben). Wirkung: das Register, das nach Rule `wege-und-vollmachten` Ziff. 1 gelesen wird, **bevor** eine Aufgabe als unmoeglich gilt, erklaert vergangene Zustaende zu dauerhaften Grenzen — genau der Fehler, den der neue Eintrag benennt, an dem Ort, an dem er am teuersten ist. **Adversariale Gegenpruefung:** (a) Steht es schon woanders? Nein, 0 Treffer, s.o. (b) Die Rule steht doch im Grundkontext, jede Session kennt sie? Empirisch widerlegt — bei SYN-50 hatte der `hub-chef` dasselbe Register und griff drei Tage nicht danach; und der Spiegelfall selbst **ist** der Beleg: der Radar hatte den Auftragstext, der die Umstellung verlangte, und schrieb elf Laeufe fort. Wer die Tabelle liest, liest die kategorische Zeile, nicht die Rule. (c) Widerspricht die Gegenmessung Grundsatz 3 («dieselbe Sackgasse zweimal zu laufen ist der teuerste vermeidbare Fehler»)? Das ist der ernsthafteste Einwand, und er traegt nicht: der Spiegelfall verlangt eine **billige** Gegenmessung (Sekunden, der Testbefehl der Zeile), nicht den zweiten vollen Anlauf. Die Aufloesung ist die Klassentrennung, nicht das Aufheben von Grundsatz 3. | Klassentrennung im Register nachtragen, Grundsatz 3 unangetastet lassen; zustandsabhaengige Eintraege nennen kuenftig, **woran man ihr Ende erkennt**. **Nicht** getan: die veralteten Einzelzeilen (Mini offline, `v11.5.0`, Mail-Index) selbst gegenzumessen und umzuschreiben — das ist Sachpflege am Register und braucht eine Messung je Zeile, nicht einen Aufsichtslauf. | umgesetzt als Querverweis 03.09.2026 (Lauf 27, active-with-flagging): ⚠-Absatz in `connectors/WEGE.md` Abschnitt 3 (+25/0; 1010 → 1035), der beide Klassen mit den Belegzeilen benennt, die Regel 260807 zitiert und Grundsatz 3 ausdruecklich bestaetigt. Nachmessen der veralteten Einzelzeilen offen |
| SYN-71 | rules/anrede-kontakte.md (importiert, Klausel «Gewicht des Anliegens») · rules/jans-dna-facetten.md (nicht importiert) · wissen/twin (Fidelity-Review 260903) | 1 | **Derselbe Commit verschiebt die Grenze einer importierten Klausel und legt den Beleg dafuer in dieselbe Datei — nur der Klauseltext bleibt stehen.** Commit `cc4367efe` (03.09. 06:11) traegt beide Aenderungen dieses Fensters an importierten Rules: `anrede-kontakte.md` (1/1) und `jans-dna-facetten.md` (13/13). Die Klausel «Gewicht des Anliegens schlaegt den Routine-Default» (Freigabe Raphael 29.07.2026, Z. 26-34) ist am **Du-Kontakt und am Vornamen** formuliert — «zieht «Geschaetzte/r \<Vorname\>» auch beim sonst lockersten **Du-Kontakt**» —, und alle drei Belege (Tschopp, Besnik, Hiltmann) sind Du-Kontakte. **Dass diese Formulierung als Ausschluss gelesen wurde, ist nicht Vermutung, sondern belegt:** `jans-dna-facetten.md` hielt bis zum 03.09. unter ausdruecklicher Berufung auf diese Rule fest, das Gewicht hebe die Stufe «bei **Du-Kontakten und Behoerden** (Rule `anrede-kontakte`), **nicht** bei der Privatkundin im warmen Sie». Genau diese Zeile ist im Diff ersetzt worden durch «Die Privatbauherrin traegt alle drei Sie-Stufen (260804 widerlegt 260903): «Guten Tag» / «Sehr geehrte» / «Geschaetzte Frau \<Nachname\>,» — es hebt das GEWICHT DES INHALTS, nicht der Cc». **Der Beleg dafuer steht seit demselben Commit in der Rule selbst**, in der neuen Hofer-Zeile (Z. 81): dieselbe Person, dieselbe Rolle, ueber drei Sie-Stufen zwischen 01.12.2025 und 08.01.2026, alle aus Raphaels eigener Hand. Die importierte Schicht traegt damit seit heute die Widerlegung in der Tabelle und die widerlegte Lesart-Grundlage im Klauseltext darueber. **Adversariale Gegenpruefung:** (a) Ist es ueberhaupt ein Widerspruch? Streng genommen nein — «auch beim sonst lockersten Du-Kontakt» ist ein A-fortiori-Beispiel, kein Ausschluss. Das entschaerft den Befund aber nicht, es erklaert ihn: die Formulierung **erlaubt** die Fehllesart, und sie ist nachweislich so gelesen worden, von einem Baustein, der sich dabei auf die Rule berief. (b) Reicht es, dass die Facetten-Datei korrigiert ist? Nein — sie ist **nicht importiert** und traegt die Kopfzeile «nicht von Hand editieren» (kompiliert aus den Wiki-Artikeln); die Rule ist die Schicht, die in jeder Session laedt. (c) Steht es schon im Register? Nein, 0 Treffer auf «Gewicht des Anliegens». | Vermerk an der Klausel, der die belegte Fehllesart benennt, die Widerlegung datiert und die praktische Folge nennt (Klausel auch im Sie-Register anwenden). **Den Klauselwortlaut selbst nicht angefasst** — die Klausel traegt eine Einzelfreigabe Raphaels vom 29.07.2026, und ob die Erweiterung in den Regeltext gehoert, ist sein Entscheid (Praezedenz SYN-67, SYN-43). | umgesetzt als Vermerk 03.09.2026 (Lauf 27, active-with-flagging): ⚠-Absatz unter der Klausel in `rules/anrede-kontakte.md` (+16/0; 117 → 133), Wortlaut der Klausel unveraendert; Aufnahme in den Regeltext offen, Entscheid Raphael  **Nachtrag Lauf 37 (16.09.2026), Beteiligter bewegt, kein neuer Befund hier:** dieselbe Klausel ist am 16.09. erneut von der Facetten-Seite her eingeschraenkt worden, diesmal nicht in ihrer Reichweite (WEN sie erfasst), sondern in ihrer Kernaussage (WAS hebt) — die Bitte um ein Zugestaendnis traegt laut Fidelity-Review den lockersten Opener. Als eigener Sachverhalt unter **SYN-88** gefuehrt; der dort gesetzte Vermerk steht unmittelbar unter dem Lauf-58-Vermerk und laesst den Klauseltext, um den es hier geht, unberuehrt. |
| SYN-72 | wissen/normen (`destillate/sia-416-2003.md` Z. 357) · wissen/grobkosten (`wiki/kennwerte.md`) · wissen/immobilienbewertung (`wiki/flaechendefinitionen-sia.md`, `wiki/realwert-sachwert.md`) · grobkosten-onepager · machbarkeit-studio · grobkosten-rechner · kostenschaetzung | 1 | **Der Aussenflaechen-Vorbehalt ist richtig, seine Abhilfe ist es zur Haelfte nicht — und sie steht in fuenf Bausteinen.** Wissens-Chef Lauf 52 (`3e6f1cc76`, 03.09. 23:30) hat den Kernbefund Run 78 hub-weit gesetzt: GV nach SIA 416 hat **kein Volumen-Pendant** fuer Balkone/Terrassen (ANF) und Laubengaenge (AVF), sie zaehlen zum AKV mit eigenstaendiger Ziff. 6. Richtig und gut belegt. Die **mitgelieferte Abhilfe** lautet an allen Stellen, den Vergleich «zusaetzlich ueber CHF/m2» zu fuehren; das Destillat formuliert die Asymmetrie als Eigenschaft der Kennwerte selbst («CHF/m³ GV ist gegenueber Aussenflaechen blind, CHF/m² nicht», Z. 357). **Gemessen an den Bezugsgroessen, die der Hub tatsaechlich fuehrt, traegt die zweite Haelfte nicht:** `skills/kostenschaetzung/SKILL.md` Z. 54 fuehrt CHF/m² **NF**, `wissen/grobkosten/wiki/kennwerte.md` Z. 65 und Z. 264 CHF/m² **HNF** bzw. CHF/m² **GF**, `skills/machbarkeit-studio/SKILL.md` Z. 125 CHF/m² **HNF**. NF, HNF und GF sind **alle** Innen-Kategorien der GF-Familie (Ziff. 1-2); AGF mit ANF/AVF ist nach **Ziff. 3** definiert als die genutzte Flaeche **ausserhalb** der allseitig umschlossenen GF und in keiner von ihnen enthalten (Destillat Z. 30, Z. 213). Ein CHF/m²-Vergleich auf diesen Bezugsgroessen ist gegenueber Balkonen also **genauso blind** wie der CHF/m³-Vergleich — die Abhilfe verlagert den blinden Fleck, statt ihn zu schliessen. **Schaerfster Fall:** `wissen/immobilienbewertung/wiki/realwert-sachwert.md` Z. 517 empfiehlt fuer Zimmerloggien und Dachterrassen ausdruecklich «zusaetzlich ueber CHF/m2 **GF** pruefen» — also ueber genau die Kategorie, **ausserhalb** derer dieselbe Norm die Loggia verortet. **Tragfaehig ist die andere Haelfte**, und `agents/grobkosten-rechner.md` Z. 30-32 formuliert sie als **einziger** der fuenf vollstaendig: «und die Aussenflaechen benennen» — separater Ausweis der ANF/AVF-Flaeche, nicht ein Wechsel der Kennwert-Bezugsgroesse. Typ 1, nicht Typ 6: die Kette Norm → KB → Skill traegt eine Empfehlung, die am Ende der Kette nicht mehr ausfuehrbar ist. | Die Wortlaute **nicht** umschreiben — der Sachentscheid gehoert der KB `normen`, die den Satz fuehrt (Praezedenz SYN-54: den Statusentscheid nicht vorwegnehmen). Stattdessen (a) Cross-KB-Bringschuld in `wissen/normen/wiki/QUESTIONS.md` registrieren, (b) ⚠-Vermerk additiv an der schaerfsten Stelle, damit niemand «CHF/m2 GF» als Gegenprobe befolgt, (c) sobald die Bezugsgroessen-Frage entschieden ist, die tragfaehige Fassung (separater ANF/AVF-Ausweis, Formulierung des `grobkosten-rechner`) in allen fuenf Stellen vereinheitlichen. | (a) und (b) umgesetzt 04.09.2026 (Lauf 28, active-with-flagging — Bringschuld und ⚠-Zeiger, kein Wortlaut-Eingriff). (c) offen, Sachentscheid bei `normen`. **Adversariale Gegenpruefung, zwei Daempfungen, die mitwandern:** (1) Die **Marktpraxis** rechnet Balkone anteilig in die vermietbare Flaeche ein — auf eine so definierte **Marktflaeche** trifft der Destillat-Satz zu; nur ist das keine SIA-416-Bezugsgroesse, und alle fuenf Bausteine nennen ausdruecklich SIA-416-Groessen. Der Fehler liegt in der **Benennung der Bezugsgroesse**, nicht im Grundgedanken. (2) Die Groessenordnungs-Daempfung, die Lauf 52 selbst gesetzt hat (bei mittleren Projekten meist innerhalb der ±25-%-Bandbreite, materiell bei kleinen und balkonlastigen Varianten), bleibt **unveraendert gueltig** — sie betrifft beide Kennwerte gleichermassen. **Bewusst NICHT gesetzt:** der Vorbehalt wurde nicht in `agents/realwert-rechner.md` nachgetragen (siehe SYN-01-Nachtrag) — einen zur Haelfte unbelegten Satz in einen **sechsten** Baustein zu kopieren, haette den Befund vergroessert statt behoben. **Nachgefuehrt 08.09.2026 (Lauf 31), Beteiligte bewegt, kein neuer Befund:** die Auslieferung des ⚠-Zeigers ist seit dem Registerstand oben von zwei auf **sechs** Stellen gewachsen — Wissens-Chef Lauf 53 setzte ihn in `wissen/grobkosten/wiki/kennwerte.md` Z. 55, Lauf 54 in `skills/grobkosten-onepager/SKILL.md` Z. 55, `skills/machbarkeit-studio/SKILL.md` Z. 74, `agents/grobkosten-rechner.md` Z. 35 und `wissen/immobilienbewertung/wiki/flaechendefinitionen-sia.md` Z. 179 (alle in diesem Lauf am Objekt nachgemessen, Commits `34fe0557a` und frueher). **Eine in der Beteiligtenspalte genannte Stelle bleibt ungedeckt: `skills/kostenschaetzung/SKILL.md`** — 0 Treffer auf `SYN-72` und 0 auf `Aussenflaech`. Sie hat den Vorbehalt aus Lauf 52 **nie bekommen** und traegt deshalb auch nichts zu widerrufen; zugleich fuehrt sie ihr ganzes Band in **CHF/m² NF** (Z. 46, Z. 54) und mahnt an Z. 83 sogar ausdruecklich «Bezugsflaeche beachten … das Band ist CHF/m² NF, nicht GF» — sie ist fuer genau diese Frage sensibilisiert und kennt die Aussenflaechen-Blindheit trotzdem nicht. Derselbe Fall wie `agents/realwert-rechner.md` im SYN-01-Nachtrag. **Bewusst weiterhin nicht gesetzt**, aus demselben Grund wie dort: einen zur Haelfte unbelegten Satz in einen weiteren Baustein zu tragen, vergroessert den Befund. Die Massnahme (c) — Vereinheitlichung nach dem Entscheid der KB `normen` — umfasst damit **sieben** Stellen, nicht fuenf |
| SYN-73 | wissen/normen (`destillate/sia-118-1991.md`) · kostenkontrolle · offertenpruefung · werkvertrag · unternehmerkontrolle | 1 | **Der Statuslabel-Nachzug erreichte vier von fuenf Zitierstellen.** Wissens-Chef Lauf 53 (`c53c79a6e`, 07.09. 08:26) hat die Herabstufung des Destillats `sia-118-1991.md` auf `status: speculative` (Run 62, 26.08.2026) in drei Skills nachgezogen: `offertenpruefung` Z. 72, `werkvertrag` Z. 76, `unternehmerkontrolle` Z. 100. **`skills/kostenkontrolle/SKILL.md` blieb aussen vor** — obwohl er dasselbe Destillat an **drei** Stellen fuehrt (Z. 36 Quellenblock, Z. 41 Herkunftssatz, Z. 90 Belegzeile) und laut Router `skills/normen/SKILL.md` Z. 51-53 «mit Art. 150/152/153-156/172/177/180/190 die **dichteste SIA-118-Strecke aller Fach-Skills**» fuehrt. Der Skill ist damit der **groesste** Abnehmer der herabgestuften Quelle und war der einzige ohne Statusvermerk. **Die Asymmetrie steht im selben Dokument:** fuer `sia-416-2003.md` traegt `kostenkontrolle` den speculative-Vorbehalt sehr wohl, zweimal (Z. 37 und Z. 122 mit ausfuehrlicher Begruendung) — die Sorgfalt existiert, sie wurde nur auf die andere Norm nicht angewandt. Nebenbefund: der Vermerk in `wissen/koordination/QUERBEZUEGE.md` Z. 2249-2250, `kostenkontrolle` habe «keine einzige Verbindung in die Normen-KB (0 Treffer)» und der Router fuehre ihn nicht, ist **beidseitig ueberholt** (Run 33 bzw. Synergie-Lauf 10/SYN-33) und dort nie nachgefuehrt worden. | Statusvermerk in `kostenkontrolle` Z. 36 nachziehen, wortgleich zu den drei Schwesterskills. Kuenftig gilt: wer ein Destillat-Statuslabel nachzieht, misst zuerst `grep -rl <destillat> skills/ agents/` und zieht **alle** Treffer nach — nicht nur die Dateien, die aus anderem Anlass ohnehin offen sind. | umgesetzt 07.09.2026 (Lauf 29, active-with-flagging): Vermerk in `skills/kostenkontrolle/SKILL.md` Z. 36 gesetzt, Wortlaut darueber unangetastet, 0 entfernte Zeilen. Der QUERBEZUEGE-Nebenbefund **bewusst nicht repariert** — die Datei gehoert der KB `koordination` als Wissens-Chef-Protokoll, ein Synergie-Lauf schreibt dort keine Historie um; als Bringschuld gemeldet |
| SYN-74 | wissen/normen (`destillate/sia-118-c1-2026.md`) · werkvertrag · unternehmerkontrolle · honorarberechnung-sia102 · ankaufspruefung · immobilienbewertung · kostenkontrolle | 1 | **Ein Destillat nennt fuenf Abnehmer, angekommen ist es bei einem.** `sia-118-c1-2026.md` (`status: established`, Refuter-verifiziert Run 47) fuehrt einen expliziten Abschnitt «JANS-Praxis-Transfer» mit **fuenf namentlich genannten Bausteinen** (Z. 54-58). Lauf 53 hat heute den 60-Tage-Befund in `skills/unternehmerkontrolle/SKILL.md` Z. 71-81 gehoben (Z. 55). Die uebrigen haben **0 Treffer** auf `C1:2026`/`60 Tage` in ihrer SKILL.md: `werkvertrag`, `honorarberechnung-sia102`, `ankaufspruefung`, `immobilienbewertung`, `kostenkontrolle`. **Schaerfster Fall `werkvertrag`** — das Destillat nennt ihn **an erster Stelle** (Z. 54: Korrigenda in die Vertragsbestandteil-Liste aufnehmen), sein eigenes Template `templates/werkvertrag-vorlage.md` Z. 123-126 traegt den Befund **seit Run 32 (14.08.2026)**, und Ablaufschritt 5 des Skills fuehrt exakt die einschlaegigen Fristen (Art. 172 Garantiefrist, Art. 180 Verjaehrung) — die 60-Tage-Ruegefrist fehlte dort als einzige. Verschaerfend: **Lauf 53 hatte genau diese Datei in derselben Minute in der Hand** (Statuslabel Z. 76, Commit `c53c79a6e`) und setzte die C1-Kante beim Nachbarn, nicht bei dem Skill, den die Quelle zuerst nennt. Zweitschaerfster Fall `honorarberechnung-sia102`: Art. 367 Abs. 1bis lit. b OR erstreckt die 60-Tage-Ordnung auf **Planerwerke** und betrifft damit die Haftung **von JANS selbst**; der Skill fuehrt zwar eine Linie in die Normen-KB, aber ausschliesslich zu SIA 102 (Z. 83, Z. 156) — auf der SIA-118-/OR-Linie hat er keinen Anker und wuerde die Korrigenda nicht finden. Rueckkante in allen Faellen fehlend: die Quelle kennt die Abnehmer, die Abnehmer die Quelle nicht — das Muster von SYN-10, SYN-14, SYN-16, SYN-20. | (a) `werkvertrag` direkt nachziehen: Hebung einer bereits established-verifizierten, im eigenen Template stehenden Aussage vom Dokument in den Ablaufschritt — kein neuer Sachentscheid. (b) `honorarberechnung-sia102`: Raphael vorlegen, nicht still setzen — es geht um die eigene Planerhaftung, das ist eine Sachfrage. (c) `ankaufspruefung`/`immobilienbewertung` (Art. 219a OR, Nachbesserungsanspruch + zwingende 5-Jahres-Verjaehrung bei Bauten unter 2 Jahren) und `kostenkontrolle` (ZGB 839 Abs. 3) beim naechsten Anlass des jeweiligen Skills mitnehmen. (d) Methodisch: wenn ein Destillat einen Abschnitt «JANS-Praxis-Transfer» fuehrt, ist dessen **Abnehmerliste die Checkliste der Auslieferung** — vollstaendig abhaken statt einzeln bedienen. | (a) umgesetzt 07.09.2026 (Lauf 29, active-with-flagging): 11 Zeilen additiv in `skills/werkvertrag/SKILL.md` Schritt 5, 0 entfernte Zeilen, Rueckkante auf das Destillat gesetzt. (b) **offen, Raphael vorzulegen.** (c) offen. **Adversariale Gegenpruefung:** Gegen den Befund spricht, dass Rule `normen-referenz` die Fach-Skills ohnehin verpflichtet, Fundstellen aus der KB zu holen — dann braeuchte kein Skill die Kante. Das traegt hier **nicht**: ein Skill konsultiert die KB entlang der Normen, die er kennt, und `honorarberechnung-sia102` kennt auf der Normen-Linie nur SIA 102. Wer nicht weiss, dass eine Korrigenda existiert, sucht sie nicht. Zudem hat Lauf 53 mit der Hebung bei `unternehmerkontrolle` die Notwendigkeit der Skill-Kante selbst anerkannt — der Befund misst nur ihre Unvollstaendigkeit. **Nicht gesetzt** wurde (b), weil die Reichweite der Planerhaftung ein Sachentscheid ist (Praezedenz SYN-54, SYN-72) |
| SYN-75 | skills/normen · skills/kostenkontrolle · wissen/normen · wissen/koordination (QUERBEZUEGE) | 1 | **Zeilennummer-Anker in wachsende Zielorte verfallen still — zwei von drei gepruefte waren tot.** Der Hub nutzt Zeilenanker sparsam: hub-weit nur **sechs** `Z. NNN`-Verweise in `skills/` und `agents/`, davon drei auf fremde Dateien. **Zwei davon zeigten ins Leere:** (1) `skills/kostenkontrolle/SKILL.md` Z. 41 berief sich auf `sia-118-1991.md` **Z. 148** als Abnehmer-Nennung — dort steht heute die Kapitelueberschrift «### Abnahme (Kap. 6.1)»; die Nennung ist nach **Z. 244** gewandert, weil Run 62 (26.08.) 84 Kernluecken in dasselbe Destillat einarbeitete. (2) `skills/normen/SKILL.md` Z. 59 verwies auf `QUERBEZUEGE.md` **Z. 348** — dort steht heute ein GT-RL-Befund aus Run 48/49; gemeint ist **Z. 2249-2250** in der auf 5000 Zeilen gewachsenen Datei. Der dritte Anker (`skills/hub-chef/SKILL.md` Z. 177 → `wissen/twin/wiki/arbeitsweise.md` Z. 3481-3483) **trifft** — er ist am 26.08.2026 gesetzt und damit jung. Das ist die Regel dahinter: der Anker altert nicht mit der eigenen Datei, sondern mit dem **Wachstum der fremden**. Genau die Dateien, auf die es sich zu verweisen lohnt (Destillate, Protokolle), sind die, die am staerksten wachsen. Kein grosses Feld, aber eine **stille** Fehlerart: ein toter Anker sieht wie ein Beleg aus und faellt erst auf, wenn jemand nachschlaegt. | Zeilennummern nur zusammen mit einem **grep-baren Textanker** setzen (Abschnittstitel oder Zitatfragment), damit die Fundstelle wiederauffindbar bleibt, wenn die Zahl verfaellt. Bestehende Anker beim Beruehren der Datei mitpruefen — bei sechs Stueck hub-weit ist das keine Kampagne, sondern eine Gewohnheit. | umgesetzt 07.09.2026 (Lauf 29, active-with-flagging): beide toten Anker korrigiert und mit Textanker versehen (`kostenkontrolle` Z. 42, `normen` Z. 59), je 0 entfernte Zeilen. Der dritte Anker geprueft und korrekt befunden, unangetastet. **Gegenpruefung zur Verallgemeinerung:** die Grundgesamtheit ist klein (3 pruefbare Faelle), 2 von 3 ist daher ein **Hinweis, keine Quote** — der Befund wird bewusst als Gewohnheitsregel formuliert, nicht als Kampagne oder Rule-Vorschlag |
| SYN-76 | logbuch/rollen/rollen-map.tsv (Z. 140) · rules/betrieb-chronik.md (260907, importiert-nah) · skills/ausschreibung/wissensbasis/15_bauleitung-training-register.md · templates/scheduled-tasks/macbook-pro/bauleitung-training/ | 1 | **Drei Register fuehren einen Loop, den die Task-Registry nicht kennt.** Der Vorgang vom 07.09.2026 (Commits `09d52ebe9`, `6a5739fd5`, `bf6bbb747`: Bauadministrations-Raster nach bbase.ch, 42 Prinzipien, Audit und Umsetzung in fuenf Bauleitungs-Skills, sechs neue Vorlagen, Register 15) ist in der Verdrahtung **vorbildlich** — alle fuenf Skills nennen Raster und Register, alle sechs Vorlagen sind in der SKILL.md verankert, die Rollen-Zeile ist gesetzt. **Nur die Task selbst fehlt.** Zwei unabhaengige Messungen: `ls ~/.claude/scheduled-tasks/` listet **33** Eintraege ohne `bauleitung-training`; `list_scheduled_tasks` liefert dieselben 33. Vorhanden ist ausschliesslich der Spiegel `templates/scheduled-tasks/macbook-pro/bauleitung-training/SKILL.md` — die Vorlage, nicht die laufende Task. Demgegenueber: `rollen-map.tsv` Z. 140 fuehrt Status **`aktiv`** und nennt als Ort genau den nicht existierenden Pfad `~/.claude/scheduled-tasks/bauleitung-training`; `betrieb-chronik.md` 260907 behauptet «Task-Datei `~/.claude/scheduled-tasks/bauleitung-training/SKILL.md`, Spiegel unter `templates/…`»; `15_bauleitung-training-register.md` nennt ihn als fuehrende Instanz («Gefuehrt vom Scheduled Task `bauleitung-training`, woechentlich Montag 04:20»). **Warum es nicht von selbst auffaellt:** der Loop ist still by default (Rule 260803) — ein stiller Loop, den es nicht gibt, ist von einem stillen Lauf ohne Befund nicht unterscheidbar. Zusaetzlich zaehlt die Rollen-Bilanz Laeufe **ueber den Namen im Log** (Rule `rollen-taxonomie`); ein Name ohne Laeufe erscheint dort als sweeper mit Ertrag null, also wie ein schwacher Loop, nicht wie ein fehlender. Erster planmaessiger Termin waere Montag, **08.09.2026 04:20**. Gleiche Familie wie Rule `auto-verbesserungen` 260807 und der Merksatz des `vollgas-chef-radar` («`enabled:`/`cron_target:` sind Dokumentation, nicht Live-Zustand») — hier eine Stufe schaerfer: nicht ein Feld widerspricht dem Betrieb, die Task fehlt ganz. | Task aus der bereitliegenden Vorlage anlegen (Inhalt fertig, Takt woechentlich Mo 04:20, Rolle sweeper) — oder, falls der Loop bewusst noch nicht laufen soll, die Rollen-Zeile auf `geplant` setzen, damit die drei Register nicht laenger etwas anderes behaupten. Methodisch: wer einen Loop anlegt, misst die **Registry** gegen, nicht die Datei — und traegt in den Chronik-Eintrag das Messergebnis, nicht die Absicht. | gemeldet, **nicht** repariert (07.09.2026, Lauf 30). Die Task wurde **nicht** angelegt: Persistenz/Autostart ist Klasse 5 der Rule `interaktive-eingriffe`, und dieser Lauf ist unbeaufsichtigt — dort gilt die Freigabe-Schwelle, nicht die Ankuendigungspflicht. Der Status `aktiv` in der `rollen-map.tsv` wurde **bewusst nicht** geaendert (Merksatz `vollgas-chef-radar`: nie ein Feld editieren, um einen Widerspruch zu heilen). Gesetzt wurde ein ⚠-Gegenmessungs-Absatz am Chronik-Eintrag 260907 (+19/0) und eine Zeile in `logbuch/fristen.md` (+2/0), damit der Punkt vor Montag frueh wiedervorgelegt wird (Rule 260805: das Register ist der Meldekanal, nicht der Laufbericht). **Adversariale Gegenpruefung:** (a) Steht die Task vielleicht auf dem Mac Mini? Nein — der Spiegel liegt unter `templates/scheduled-tasks/**macbook-pro**/`, und gemessen wurde auf dem MacBook Pro. (b) Wird sie noch angelegt, der Vorgang lief ja heute? Der letzte Commit der Kette ist von 16:15, die Messung von 17:1x, und die Chronik schreibt die Anlage bereits im Perfekt — die Behauptung steht also unabhaengig davon schon im Register. (c) Ist es ueberhaupt ein Synergie-Befund und nicht Betriebsaufsicht? Der Befund ist eine **Rueckkante**: drei Register zeigen auf einen Baustein, der nicht existiert — dasselbe Muster wie SYN-10/14/16/20, nur mit leerem Ziel **Nachtrag 11.09.2026 (Lauf 34): in der Hauptsache widerlegt, gemessen am falschen Ort.** Die Task liegt seit **07.09.2026 16:04:51** in der Registry des **Mac Mini** (`ssh mini`, lesend: `~/.claude/scheduled-tasks/bauleitung-training/SKILL.md`); Lauf 30 hat 70 Minuten spaeter nur die Registry des MacBook Pro gemessen, obwohl die Lage «eine Registry pro Station, `list_scheduled_tasks` zeigt nur die lokale» bekannt ist. Die Methodenregel dieses Eintrags («misst die Registry gegen, nicht die Datei») gilt darum verschaerft: **beide** Registries. Richtig gefuehrt haben den Loop die Inventur der Vertiefung vom 11.09. (`docs/konzepte/260911-Vertiefung-Umbau-Verifikation-Routinen/260911-Vertiefung-Umbau-Verifikation-Routinen.md` Z. 84, Mini-Tabelle) und der V0+-Spiegel `templates/scheduled-tasks/mac-mini/bauleitung-training/` (`8408bbb4e`) — keine der beiden Stellen hat das Fristen-Register oder diesen Eintrag berichtigt, und die dort stehende Aktion Raphael «Task anlegen» haette auf dem MacBook einen **Doppellauf** erzeugt (zwei Instanzen schreiben in dieselben fuenf Skills). Berichtigt als datierte Nachtraege in `logbuch/fristen.md` (Befund 07.09.) und `rules/betrieb-chronik.md` 260907; `rollen-map.tsv` Z. 140 stimmt (der Pfad gilt auf dem Mini) und bleibt unveraendert. Titel «Scheduled Task MacBook Pro» in Chronik 260907 und der Spiegel unter `templates/scheduled-tasks/macbook-pro/bauleitung-training/` sind Fehlablagen desselben Irrtums; der Spiegel faellt beim naechsten MacBook-Spiegellauf weg (`rsync --delete`; letzter MacBook-Spiegel 07.09., `09d52ebe9`). **Rest-Befund, klein:** der erste planmaessige Lauf (Mo 08.09. 04:20) hat keine Spur hinterlassen (Laufjournal in `15_bauleitung-training-register.md` nur Lauf 0, `training/` nur `2026-09-07_bauleitung-run0.md`); ob der Takt in der Mini-Registry gesetzt ist, ist von hier nicht messbar. Gegenmessung Mo 14.09.2026 nach 04:20. Status: **Hauptbefund widerlegt, Rest offen.** **Nachtrag Lauf 35 (14.09.2026):** Gegenmessung gefahren, per `ssh mini` lesend im App-Log: 14.09. 04:26:08 «Spawning new session … lastRunAt: undefined», 04:26:09 `Cannot start session … OAuthError (session_stale_relogin)`, 04:35:52 «Cleared stale pending dispatch». Der Loop ist damit **nie gelaufen** (2 Slots, 0 Laeufe), diesmal aber nicht wegen der Task, sondern wegen des stationsweiten App-Latchs (SYN-85). Fuer den 08.09. zeigt `main1.log` keinen Dispatch dieser Task (Ursache offen). Naechste Gegenmessung Mo 21.09.2026 nach 04:20. Status: **Rest offen.** |
| SYN-77 | skills/ausschreibung/wissensbasis/15_bauleitung-training-register.md · templates/scheduled-tasks/macbook-pro/bauleitung-training/SKILL.md · offertenpruefung · protokoll · pendenzenliste · terminplanung | 1 | **Das Register fuehrt acht Skills als Pruefgegenstand, die Auswahlregel des Loops kennt fuenf — und die Rueckkanten folgen der Auswahlregel, nicht dem Register.** Die Tabelle «Stand je Skill» in `15_bauleitung-training-register.md` hat **sieben** Zeilen: die fuenf mit Pruefdatum 07.09.2026 (`honorarberechnung-sia102`, `ausschreibung + offertenpruefung`, `werkvertrag`, `unternehmerkontrolle`, `kostenkontrolle`) und zwei mit «ausstehend» (`protokoll + pendenzenliste` fuer Prinzip 04.40, `terminplanung` fuer 04.41). Die `description` der Task nennt als Auswahlmenge dagegen nur «je Lauf einen Skill (honorar, ausschreibung, werkvertrag, unternehmerkontrolle, kostenkontrolle)»; Schritt 2 waehlt «den Skill mit dem **aeltesten Pruefdatum**» — auf einem leeren Feld ist das nicht definiert. Die beiden Quer-Zeilen werden also strukturell nie erreicht, waehrend die Abbruchklausel («jedes Prinzip des **gewaehlten** Skills gruen» plus drei ertragslose Laeufe, Rule 260830) allein ueber die Fuenf misst: **eine Saettigungsmeldung kann ergehen, waehrend zwei der 42 Rasterprinzipien nie geprueft wurden.** 04.40 (Notiz → Pendenz → Protokoll aus derselben Quelle, mit Zuordnung Phase/Bauteil/VE/BKP) und 04.41 (Terminplan mit den fuenf Partnerzeilen Bauherr/Benutzer/Behoerden/Planer/Baustelle) sind reale Bauadministrationsfunktionen, fuer die JANS die Skills `protokoll`, `pendenzenliste` und `terminplanung` hat. **Die Kante folgt der Auswahlmenge, gemessen:** alle fuenf Skills der Auswahlmenge nennen das Raster in ihrer SKILL.md; **null** der vier uebrigen Pruefgegenstaende taten es (`offertenpruefung`, `protokoll`, `pendenzenliste`, `terminplanung` — je 0 Treffer auf `bauadministration-prinzipien` und `15_bauleitung-training-register`). Das Muster ist dasselbe wie in SYN-74 vom selben Tag: die Auslieferung erreicht die Stellen, die im Arbeitsauftrag standen, nicht die, die die Quelle nennt. | (a) `offertenpruefung` direkt nachziehen — er steht namentlich in einer Zeile mit Pruefdatum, die Kante ist reiner Querverweis. (b) Fuer `protokoll`, `pendenzenliste`, `terminplanung` **keinen** Zeiger setzen: solange nicht entschieden ist, ob sie eigene Laeufe bekommen oder beim jeweiligen Anlass mitlaufen, waere der Verweis eine Zustaendigkeitsbehauptung, kein Querverweis. (c) Vermerk im Register 15, der die Diskrepanz benennt, ohne die Tabelle oder die Auswahlmenge zu aendern. (d) Methodisch, allgemein: wenn ein Loop ein Register fuehrt, sind **Auswahlmenge und Registerzeilen dieselbe Liste** — faellt eine Zeile aus der Auswahlmenge, gehoert an sie, wer sie sonst abdeckt. | (a) und (c) umgesetzt 07.09.2026 (Lauf 30, active-with-flagging): Contract-Zeile «Pruefraster» in `skills/offertenpruefung/SKILL.md` (+1/0; 175 → 176), ⚠-Absatz vor dem Laufjournal in `15_bauleitung-training-register.md` (+17/0; 24 → 41), Tabelle und Auswahlmenge unangetastet. (b) offen, Zuschnitt-Entscheid Raphael. **Adversariale Gegenpruefung:** (a) Der Ziel-Absatz des Prompts nennt die Quer-Skills sehr wohl («quer: protokoll, pendenzenliste, terminplanung») — ein sorgfaeltiger Lauf koennte sie mitnehmen. Das entschaerft den Befund nicht: die **Auswahlregel** in Schritt 2 und die Auswahlmenge in der `description` sind die operativen Saetze, der Zielabsatz ist Rahmen; und die gemessene Rueckkanten-Verteilung (5 von 5 gegen 0 von 4) zeigt, welcher der beiden Saetze tatsaechlich gewirkt hat. (b) Ist die Luecke gross? Zwei von 42 Prinzipien, also klein — der Befund liegt nicht im Umfang, sondern darin, dass die Saettigungsklausel eine Vollstaendigkeit erklaeren wuerde, die nicht gemessen wurde. (c) Steht es schon woanders? Nein, 0 Treffer auf `04.40`/`04.41` ausserhalb des Rasters und des Registers |
| SYN-78 | rules/jans-dna-facetten.md · wissen/twin/wiki/stimme.md · skills/twin/tools/build_dna.py · twin-fidelity-review | 1 | **Der Byte-Riegel der Facetten-Rule ist erreicht — und die erste Verdichtung hat Belege aus der Pflichtlektuere in den Teil verschoben, den nach Konstruktion niemand laedt.** `build_dna.py` kompiliert je Facette **nur** den `## Do / Don't`-Block des Wiki-Artikels in `rules/jans-dna-facetten.md` (Z. 67-74 des Scripts); genau dieser Block ist die «PFLICHT-LEKTUERE vor jedem ausgehenden Texterzeugnis». Gemessen an HEAD: **33'993 B bei MAX 34'000 — 7 B Reserve.** Der `twin-fidelity-review` vom 08.09.2026 (`641664511`, 06:05) musste kuerzen, um neue Marker setzen zu koennen, und hat dafuer im Do/Don't-Block von `stimme.md` die **neun namentlich belegten Service-Satz-Gattungen** gestrichen (belegt gesetzt: Rechnungs-/Schluss-Liefermail, Auftrags-Dank, beratende Du-Mail, erbetene Auskunft, Erst-Offertanfrage, Antrag ans Amt, Abgebot, Transmittal, Statusmail; belegt weggelassen: praezisierte Zweitanfrage, Amts-Ersuchen, dieselbe Statusmail-Gattung), ersetzt durch den Satz «Der Gattungs-Messstand steht im Artikel». **Kein Datenverlust, ein Zugriffsverlust:** die Gattungen sind in `stimme.md` weiterhin vorhanden (gemessen: «Abgebot» 8, «Erst-Offertanfrage» 1, «Zweitanfrage» 1, «Amts-Ersuchen» 1 Treffer), im Do/Don't-Block dagegen **0** — also ausserhalb dessen, was ein Texterzeugnis laedt. **Der Ersatz-Zeiger loeste in keinem der beiden Kontexte auf:** in `stimme.md` war «im Artikel» ein Selbstverweis, in der kompilierten Rule ein Verweis ohne Adresse (die Rule buendelt sechs Facetten-Artikel). Das ist die Falle der Kompilierung: der Satz wurde fuer die Sicht der Rule formuliert und in die Quelle geschrieben, wo er sinnlos ist. Typ 3 (Compounding-Luecke). **Der Zustand war am 18.08.2026 vorhergesagt** — die Anhebung 30'000 → 34'000 wurde damals genau damit begruendet, dass «die naechste Runde Regeln geloescht haette statt Doppelungen» (`build_dna.py` Z. 57-62). Nach 21 Tagen ist der Fall eingetreten. | (a) Zeiger konkret machen (Querverweis, kein Sachentscheid). (b) **Raphael vorlegen, nicht still entscheiden:** der Riegel laesst nur noch Verdraengung zu. Drei Wege — Grenze anheben (hebt die Ladekosten jedes Texterzeugnisses, Entscheid wie am 18.08.), Verdichtungsregel schaerfen (Beleg-Prosa raus, Regel rein), oder den Do/Don't-Block in zwei Ladestufen splitten (Kern immer, Gattungstabellen auf Zuruf). | (a) **umgesetzt 08.09.2026 (Lauf 31, active-with-flagging):** `wissen/twin/wiki/stimme.md` Z. 57-58 nennt jetzt «die Gattungs-Abschnitte weiter unten in `wiki/stimme.md`»; Quelle editiert, nicht das Kompilat (`rules/jans-dna-facetten.md` traegt den Warnhinweis «Nicht von Hand editieren»), danach `build_dna.py` neu gefahren. Zeilenbilanz beide Dateien unveraendert (stimme.md 5552, Rule 391), eine Zeile ersetzt, keine entfernt. (b) **offen, Raphael vorzulegen.** **Adversariale Gegenpruefung:** Gegen den Befund spricht, dass der Review inhaltlich richtig gehandelt hat — die geloeschte «Bestbeschreibung 7 von 8» war am 08.09. tatsaechlich widerlegt, und eine widerlegte Hypothese gehoert weg. Das traegt fuer die Hypothese, **nicht fuer die Gattungsliste**: die ist Beleg, nicht Vermutung, und wurde im selben Schnitt mitgenommen. Zweitens koennte man einwenden, die Twin-Agenten laesen `wissen/twin/wiki/` ohnehin direkt (so `rules/jans-dna.md`) — das gilt fuer sie, nicht fuer jede Session, die nur die Pflichtlektuere laedt. **Eigenmessung nach der Reparatur: 33'929 B, 71 B Reserve** — das Script warnt jetzt selbst («Verdichten einplanen»); dieser Lauf hat 50 B davon verbraucht und weist das aus, statt es zu verschweigen. **Nachtrag 09.09.2026 (Lauf 32): (a) ist zurueckgefallen.** `303c7672e` (09.09. 06:00) hat den reparierten Zeiger nach **13 Stunden** erneut auf «Gattungs-Messstand im Artikel» gekuerzt, und vier weitere belegte Marker sind nach demselben Muster aus dem Do/Don't-Block verdraengt worden; Reserve jetzt **15 B**. Der aeltere Eintrag **SYN-69** (02.09., «offen, Entscheid Raphael») traegt dieselbe Frage — diese Kante fehlte hier und ist in **SYN-80** nachgezogen |
| SYN-79 | wissen/WISSEN-CLAUDE.md · skills/wissenscheck (Audit B und F) · 85 `outputs/`-Reports in mindestens zehn KBs | 2 | **Eine neue Konvention erklaert einen gewachsenen Bestand zu Verstoessen, ohne etwas ueber ihn zu sagen.** Am 08.09.2026 (`f2bf87873`) wurde beidseitig sauber verdrahtet — `wissen/WISSEN-CLAUDE.md` Z. 60-66 haelt fest, dass `links:` nur fuer `wiki/`-Artikel gilt und `outputs/`-Reports das Feld **nicht** tragen, und `skills/wissenscheck/SKILL.md` Pruefung B nennt die Meta-Datei zurueck. Die Rueckkante ist vorbildlich, der Befund liegt daneben: die Klarstellung sagt zugleich, ein `outputs/`-Report mit eckigen Klammern sei «ein Schreibregel-Verstoss fuer Audit F». **Gemessen ueber den ganzen Wissens-Layer: 85 `outputs/`-Reports fuehren ein `links:`-Feld**, verteilt auf mindestens zehn KBs (`wettbewerbs-dna`, `bauprodukte`, `claude-code`, `firmengruendung-ch`, `immobilienbewertung`, `auflagebereinigung`, `baurecht`, `grobkosten`, `twin`, `projekt-lessons`) und in drei Bauformen (`[[wikilink]]`, einfache `[klammern]` in `bauprodukte`, relative Pfade `[[../wiki/stimme]]` in `twin`). Der Anlass waren zwei tote Zeiger in **einer** KB (PL46-2/PL46-3). Ohne Aussage zum Altbestand produziert der naechste `wissenscheck` in jeder dieser KBs einen Schwall Audit-F-Befunde aus Altlast — genau das Rauschen, das die Klarstellung vermeiden sollte. Typ 1 (Ketten-Luecke): die Regel ist gesetzt, ihre Wirkung auf den Bestand nicht. | Eine Bestandsaussage in `wissen/WISSEN-CLAUDE.md` ergaenzen — entweder «Altbestand nicht nachpflegen, nur bei Neuanlage anwenden» oder «beim naechsten Anlass der jeweiligen KB bereinigen». Welche der beiden gilt, entscheidet der Wissens-Chef bzw. Raphael, nicht dieser Lauf. | **offen, bewusst nicht gesetzt** (Praezedenz SYN-54 und SYN-72: den Sachentscheid der fuehrenden Instanz nicht vorwegnehmen — «wie mit Altbestand umgehen» ist eine Entscheidung, kein Querverweis). **Adversariale Gegenpruefung:** Ein grosser Teil der 85 Treffer ist ein **leeres** `links:`-Feld (z.B. `claude-code`, `auflagebereinigung`) und damit harmlos — der Befund ist entsprechend als Prioritaet 2 gefuehrt, nicht 1. Er faellt trotzdem nicht weg: der Wortlaut der Klarstellung unterscheidet leer und gefuellt nicht, und die gefuellten Faelle (`baurecht` mit sechs Zielen, `immobilienbewertung` mit vier, `twin` mit relativen Pfaden) sind real und tragen echte Querverweise, die nach der neuen Konvention in den Body gehoeren. **Nachtrag 09.09.2026 (Lauf 32): der Bestand waechst weiter — 85 → 88.** Von den sieben seit dem 08.09. 17:10 neu angelegten `outputs/`-Reports tragen **zwei** ein `links:`-Feld (`wissen/claude-code/outputs/2026-09-09_verdichtung-verliert-zuerst-die-bedingung.md` und `wissen/twin/outputs/2026-09-09_fidelity.md`), beide **nach** der Klarstellung vom 08.09. entstanden. Die Konvention wirkt damit nicht einmal auf Neuanlagen, nicht nur nicht auf den Altbestand; `wissen/WISSEN-CLAUDE.md` hat weiterhin 0 Treffer auf «Altbestand» |
| SYN-80 | skills/twin/tools/build_dna.py (MAX_AUTO_BYTES) · rules/jans-dna-facetten.md · wissen/twin/wiki/stimme.md + beziehungsregister.md + haltung.md · wissen/claude-code/outputs/2026-09-09 · logbuch/fristen.md Z. 49 · SYN-69 · SYN-78 | 1 · 2 | **Dieselbe Entscheidungsvorlage liegt an vier Stellen, keine nennt die anderen — und weil sie zersplittert vorliegt, erreicht sie nie das Gewicht, das sie inhaltlich hat.** Der Byte-Riegel der Facetten-Pflichtlektuere ist seit dem 02.09.2026 als **SYN-69** («offen, Entscheid Raphael»), seit dem 08.09. zusaetzlich als **SYN-78 (b)** und als eigener Eintrag in `logbuch/fristen.md` Z. 49 erfasst, und der `twin-fidelity-review` empfiehlt die Anhebung im Report vom 09.09. ausdruecklich zum **fuenften Lauf in Folge** (gemessen: sechs Fidelity-Reports nennen 36'000 — 08.08., 13.08., 03.09., 04.09., 08.09., 09.09.). **Richtungsprobe, alle vier Kanten fehlen:** SYN-78 nennt keinen anderen SYN-Eintrag (gemessen: einziger Treffer auf `SYN-[0-9]*` in Z. 89 ist SYN-78 selbst) — mein eigener Vorgaenger hat also gestern einen zweiten Riegel-Eintrag neben SYN-69 angelegt, ohne die Kante zu ziehen; der Fristen-Eintrag Z. 49 nennt SYN-54 und SYN-72 als Praezedenz, aber weder SYN-69 noch SYN-78; der Ruecklauf-Report `wissen/claude-code/outputs/2026-09-09_verdichtung-verliert-zuerst-die-bedingung.md` hat **0 Treffer** auf «SYN-78», «SYNERGIE» und «koordination» zugleich, obwohl er denselben Mechanismus praeziser beschreibt als beide Registereintraege («der Riegel selektiert systematisch gegen Bedingungen und fuer unbedingte Regeln, weil eine Bedingung mehr Zeichen braucht als der Imperativ, den sie einschraenkt»; Marker-Messung Artikel **38** gegen kompilierter Block **1**). **Die Folge ist gemessen, nicht befuerchtet:** die SYN-78-Reparatur vom 08.09. 17:21 (`baadd6921`, Zeiger auf die volle Adresse «Gattungs-Abschnitte weiter unten in `wiki/stimme.md`») wurde am 09.09. um 06:00 durch `303c7672e` wieder auf «Gattungs-Messstand im Artikel» zurueckgekuerzt — **Standzeit 13 Stunden**, und der zurueckgekuerzte Wortlaut ist exakt der, den SYN-78 als adresslos beanstandet hatte (im Kompilat, das sechs Facetten buendelt, loest «im Artikel» nicht auf; in der Quelle inzwischen schon, weil dort seit 08.09. eine eigene H2-Sektion «Service-Satz — Gattungs-Messstand» steht). Im selben Zug sind **vier weitere belegte Marker** aus der Pflichtlektuere verdraengt worden, alle mit demselben Muster wie bei SYN-78 — im Artikel vorhanden, im Do/Don't-Block auf null (gemessen je Datei: «Bestbeschreibung» 1/0 und «rahmt die dazwischen» 1/0 in `stimme.md`, «statische Ausl» 1/0 und «dreimal zu hoch» 1/0 in `beziehungsregister.md`); «kontrollierter Vergleich» in `haltung.md` ist als Methodenangabe ganz gestrichen (0 Treffer). **Reserve jetzt 15 B von 34'000** (eigene Nachrechnung mit der Extraktionslogik von `build_dna.py` Z. 67-74, lesend nachgebaut: stimme 7069, denken 2930, haltung 5991, beziehungsregister 5995, fachsignatur 4978, arbeitsweise 6874) — nach 7 B am 08.09. vor der Reparatur und 71 B danach; der Review nennt es selbst «die niedrigste je gemessene Reserve» und fuehrt **vierzehn belegte, verhaltenswirksame Marker ausserhalb des Gehirns**. Typ 2 (Doppelrechnung) und Typ 1 (Ketten-Luecke) zugleich: vier Kanaele rechnen denselben Befund einzeln, jeder wirkt fuer sich wie ein Einzelfall mittlerer Prioritaet. | (a) Die vier Fundstellen gegenseitig verzeigern, damit die Vorlage als **eine** Frage sichtbar wird — Querverweis, kein Sachentscheid. (b) Der Riegel-Entscheid selbst bleibt bei Raphael (SYN-69 und SYN-78 (b) unveraendert); dieser Eintrag ersetzt sie nicht, er buendelt sie. (c) Fuer den Wissens-Chef zu pruefen: eine Reparatur an einer byte-gedeckelten, taeglich neu kompilierten Datei ist nicht haltbar, solange der Druck besteht — Reparaturen dort erst nach dem Entscheid setzen oder gegen Rueckkuerzung sichern, sonst verbraucht jeder Lauf Arbeit, die der naechste einzieht. | (a) **umgesetzt 09.09.2026 (Lauf 32, active-with-flagging):** Verweiszeilen in SYN-69, SYN-78, `logbuch/fristen.md` Z. 49 und im Ruecklauf-Report gesetzt; kein Wortlaut eines bestehenden Befundes geaendert. (b) und (c) **offen.** **Adversariale Gegenpruefung:** (1) Ist das nur Buchhaltung ueber bestehende Eintraege? Nein — die 13-Stunden-Standzeit der Reparatur und die vier neu verdraengten Marker sind an diesem Lauf gemessen und in keinem der bestehenden Eintraege enthalten; SYN-69 und SYN-78 kennen sie nicht. (2) Existiert der Befund schon im Ruecklauf-Report? Der Mechanismus ja, und besser formuliert — **die Zersplitterung nicht**: der Report weiss nichts vom Register, und gerade deshalb hat er die Reparatur ueberschrieben, statt sie zu kennen. (3) Spricht dagegen, dass der Nacht-Lauf falsch gehandelt haette? Nein, im Gegenteil: er hat neue belegte Regeln gesetzt (Fragezeichen nur bei Bitte und Vorwurfsfrage, konditionale Nachmeldung statt Imperativ, Zahlungserinnerung institutionell gegen privat) und die Verdichtung offen ausgewiesen. Der Befund richtet sich nicht gegen den Lauf, sondern gegen den Zustand, in dem die einzige Waehrung fuer eine neue Regel die Belegdichte einer alten ist — und in dem eine gesetzte Reparatur als Fettpolster gilt **Nachtrag 10.09.2026 (Lauf 33): die Buendelung erreicht die twin-Seite, und der Riegel hat einen belegten Schadensfall.** (1) Der `twin-fidelity-review` vom 10.09. (`efbaafac5`, 06:15) nennt erstmals einen Registereintrag (SYN-69, `wissen/twin/outputs/2026-09-10_fidelity.md` Z. 280-281) und uebernimmt (c) als eigene Empfehlung 3 («Vor dem naechsten Nachziehen die Byte-Frage entscheiden lassen», Z. 294-295): (a) wirkt. (2) Befund 1 desselben Reports belegt den ersten Schaden, **im Blindtest am historischen Gold, nicht in einer versandten Mail**: der Marker «Nullform-Weiterleitung» war am 29.08. ausdruecklich wegen 48 B Reserve nicht kompiliert worden; am 10.09. schrieb der Zwilling 20 statt 2 Woerter, in der am 29.08. vorhergesagten Gestalt. Nachgezogen mit 708 B Verdichtung, Reserve danach 16 B, Empfehlung 36'000 B zum sechsten Lauf in Folge. (3) **Zwei der sieben Verdichtungen haben Satzbau zerbrochen**, weil der Lauf den Anker (genau ein Treffer) prueft, nicht das Ergebnis: `wissen/twin/wiki/stimme.md` Z. 104 begann mit einem verwaisten « . Der Abbruch …» (Datumsverweis gestrichen, Punkt stehen gelassen), `wissen/twin/wiki/arbeitsweise.md` Z. 188 verlor den Satzpunkt («aus vier Richtungen Der Zwilling …»). Beide in der Quelle repariert (active-with-flagging), `build_dna.py` neu gefahren, Zeilenbilanz unveraendert (5782 / 4655 / 390), **Reserve 16 → 15 B** (+1 B fuer den Punkt, die andere Reparatur byte-neutral). Uebriges Kompilat auf verwaiste Satzzeichen und leere Klammern gescannt: sauber. (4) **Gegenprobe Bedingungsverlust, widerlegt:** die 89-B-Kuerzung des «Ausloeser-Satzes 260822» strich «notwendig, nicht hinreichend», «3/3 verfehlt» und «IM SELBEN VORGANG», also genau die Stelle, auf die die importierte `rules/anrede-kontakte.md` als «praezisere Facetten-Fassung» verweist (SYN-43). Die Bedingung ueberlebt aber im unmittelbar folgenden Trajektorie-Punkt («Der belegte Opener gilt **innerhalb** des Vorgangs, nicht über Wochen», `rules/jans-dna-facetten.md` Z. 239-240), und die Zahl 3/3 steht in der importierten Rule selbst. Kein Befund. **Vorschlag (d), Entwurf fuer die twin-KB:** ein Satzbau-Check nach jeder Verdichtung (Zeilenanfang auf Satzzeichen, fehlender Punkt an der Schnittstelle), billig und er haette beide Artefakte gefunden. (b) weiterhin offen, Entscheid Raphael **Nachtrag 11.09.2026 (Lauf 34): (d) wirkt, der Schaden waechst.** Der Review vom 11.09. (`9f3371519`, 16/19) faehrt den Satzbau-Check ausdruecklich «(SYN-80) ohne Befund» (`wissen/twin/outputs/2026-09-11_fidelity.md` Z. 276) und nennt SYN-69 (Z. 284): Vorschlag (d) ist uebernommen. Die sieben Freischnitte stehen offen im Report (Z. 266-272) und sind gegengeprueft: sechs sind Beleg-Prosa oder Erlaeuterung, die Fragezeichen-Regel ueberlebt in `rules/jans-dna-facetten.md` Z. 60. **Einer ist kein Beleg, sondern der SYN-78-Zeiger selbst:** «vier Linien gefallen (260807–260908), Gattungs-Messstand im Artikel» ist ganz gestrichen; `wiki/stimme.md` traegt den Messstand weiter, das Kompilat zeigt nicht mehr hin. Das ist die in (c) vorhergesagte Folge (Reparatur 08.09., Rueckkuerzung 09.09., Streichung 11.09.) und nach (c) bewusst nicht repariert. **Zweite Schadensmessung, staerker als die erste:** an **ungesehenen** Golds gehen zwei der vier Anredefehler und zwei der drei Grussfehler auf Wiki-Befunde zurueck, die aus Byte-Gruenden nie kompiliert wurden (Z. 283-285); Empfehlung 36'000 B zum **siebten** Lauf in Folge, Reserve 16 B. (b) weiterhin offen, Entscheid Raphael; `logbuch/fristen.md` (Riegel-Befund 08.09.) nachgetragen. |
| SYN-81 | ~/.claude/scheduled-tasks/wissenscheck-monatlich/SKILL.md (+ Vorlage templates/scheduled-tasks/macbook-pro/) · immobewertung-training (dito) · Rule sync-kanonische-quelle (importiert) · Rule auto-verbesserungen 260726 + 260830 Ziff. 3 (importiert) · SYN-61 | 2 | **Zwei Task-Prompts verlangen noch den git-Weg, den die Rule seit dem 26.07. verbietet, und einer beruft sich dabei auf genau diese Rule.** `wissenscheck-monatlich` Schritt 4: «committe und pushe die Aenderungen im NAS-Repo (git add wissen/ && git commit && git push github main), gemaess Rule sync-kanoni[sche-quelle]». `immobewertung-training` Schritt 7: «cd /Volumes/daten/jans-ai-hub && git add -A && git commit && git push github main) gemaess Rule sync-kanonische-quelle.md» — `-A` nimmt zudem alles mit, was andere Laeufe gerade halb geschrieben haben. Rule `sync-kanonische-quelle` verbietet «IRGENDEIN `git`-Schreibbefehl (commit/push/pull/rebase) ueber den SMB-Mount auf das NAS-`.git`», weil er uninterruptibel haengt und `.git/index.lock` fuer alle blockiert. **Messung, Richtungsprobe:** `grep` ueber alle 34 lokalen Task-Prompts, alle 43 Vorlagen unter `templates/scheduled-tasks/` und (per `ssh mini`, lesend) die 9 Prompts des Mac Mini: je 4 Treffer lokal und in den Vorlagen (Prompt und Vorlage identisch), 0 auf dem Mini. Zwei der vier sind erledigte One-Times (`wettbewerbs-dna-reaktivierung`, `wettbewerbs-layer-nachbrenner`, beide `enabled: false`); **live relevant sind zwei:** `wissenscheck-monatlich` (enabled, naechster Lauf **01.10.2026 19:30**) und `immobewertung-training` (stillgelegt, laut Beschreibung auf Ereignis-Zuruf reaktivierbar). **Der Schutz hat bisher gehalten, und das ist der Befund:** der Lauf vom 01.09. hat nicht dem Prompt, sondern der importierten Rule gehorcht und ueber den nativen Committer gebucht (`3028d0a1d`, 19:43, «Wissenscheck monatlich 2026-09-01»). Die Sicherheit haengt damit allein am Vorrang der Rule vor dem Auftragstext — ein Prompt, der den verbotenen Befehl woertlich vorgibt, ist die staerkere Anweisung, sobald ein Lauf die Rules nicht laedt oder einen Konflikt zugunsten des konkreten Auftrags aufloest. Der Nachfolger-Task `immobewertung-marktpuls-261201` Z. 46 zeigt die richtige Fassung («NIE `git` schreibend ueber den SMB-Mount … der native 15-Minuten-Committer»); die Korrektur wurde also schon einmal gemacht, nur nicht in die Vorgaenger zurueckgetragen. Gleiche Familie wie SYN-61 (dort ein Script, hier ein Prompt) und wie `auto-verbesserungen` 260830 Ziff. 3: ein wiederholt zugestellter Auftragstext traegt einen ueberholten Stand, hier einen ueberholten **Weg**. Nebenbefund im selben Prompt: «Aktuelle KBs (Stand 02.06.2026): baurecht, projekt-lessons» — harmlos, weil Schritt 2 die KBs selbst auflistet. Typ 1 (Ketten-Luecke). | Schritt 4 bzw. 7 in Prompt **und** Vorlage auf `bash /Volumes/daten/jans-ai-hub/scripts/nas-commit-now.sh "<Message>"` umstellen, Wortlaut nach `immobewertung-marktpuls-261201` Z. 46; die Stand-Zeile vom 02.06. streichen. Die zwei erledigten One-Times koennen bleiben. | **offen, bewusst nicht gesetzt** (Praezedenz SYN-49/SYN-56: ausfuehrende Bausteine, hier Task-Prompts, aendert dieser Lauf nicht; zudem verlangt eine Registry-Aenderung einen schreibenden Werkzeugaufruf, den dieser Task nicht vorsieht). Frist vor dem 01.10.2026 in `logbuch/fristen.md` eingetragen (Rule `auto-verbesserungen` 260805). **Adversariale Gegenpruefung:** (1) Hat der Fehler je geschadet? Nein, gemessen am 01.09. — darum Prioritaet 2, nicht 1. (2) Ist er schon bekannt? `grep` auf die beiden Task-Namen im Register 0 Treffer, in `rules/betrieb-chronik.md` nur Takt- und Stilllegungsvermerke, keiner zum Commit-Schritt. (3) Koennte der Prompt bewusst so stehen? Nein: er beruft sich auf die Rule, die das Gegenteil sagt; der Text stammt erkennbar aus der Zeit vor dem 26.07. **Nachtrag 11.09.2026 (Lauf 34): die Messung war zu eng, der Befund ist breiter.** Die Richtungsprobe oben suchte woertliche Befehle; Prosa («NAS-Repo committen + pushen») und die VOLLGAS-Lokalcommit-Fassung fielen durch — «0 auf dem Mini» war eine Aussage ueber das Suchmuster. Neu gemessen mit `git (add|commit|push|pull)|committen|pushen` und Gegenmuster `nas-commit-now|NIEMALS git|Kein git`: **Mac Mini 6 von 12 Live-Prompts ohne sicheren Weg** (per `ssh mini`, lesend) — `grobkosten-training` (Takt 09/21, kein `enabled: false`; `git add wissen/grobkosten` + `git commit`, «der git-auto-sync-Job erledigt pull --rebase und push», Architektur vor 260726), `normen-training-mini` (woertlich `cd /Volumes/daten/jans-ai-hub && git pull`, `enabled: false`), `synobsis-batch-nacht` («im Repo /Volumes/daten/jans-ai-hub … committen und pushen», deaktiviert), Prosa in `baurecht-buch-training`, `planungsgrundlagen-training` und `energie-training` (`enabled: false`, feuert laut Registry-Befund 06.08. trotzdem). **MacBook Pro** zusaetzlich Prosa ohne Gegenmuster in `wissens-chef`, `spec-training`, `twin-fidelity-review`; die One-Time `immobewertung-marktpuls-260901` (Lokalcommit) ist vorbei. **Drei Formen, drei Risiken:** woertlicher Befehl (hoch: die zwei Faelle oben plus `normen-training-mini`), VOLLGAS-Lokalcommit ins Arbeitsverzeichnis (mittel: schreibend ueber SMB, sobald der Lauf im NAS-Pfad steht), Prosa mit Rule-Verweis (niedrig: `git-auto-push` leitet das NAS-Repo selbst auf `nas-commit-now` um; Beleg Wissens-Chef Lauf 57, `1917f4cef` mit Autor «JANS NAS Selfcommit»). **Warum es jetzt zaehlt:** V0+ verlegt die Lern-Loops als zweite Welle auf den Mini (`rules/betrieb-chronik.md` 260911b), und die Mini-Prompts sind die aeltesten. Die Vertiefung kennt `grobkosten-training` («Git-Anweisung im Prompt», Z. 88), ihre Ersetzungsliste E1 (Z. 148) gehoert aber zu V2, nicht zu V0+. Vorschlag erweitert: **vor der zweiten Welle** die Mini-Prompts der Formen 1 und 2 auf `nas-commit-now.sh` stellen; `logbuch/fristen.md` (Befund 10.09.) nachgetragen. |
| SYN-82 | skills/wissenscheck (Audits A, B, E; Contract «Vorgelagert: —») · Task wissenscheck-monatlich · wissen/tools/README.md (sieben Pruefwerkzeuge) · wissen/tools/datenstand-waechter.py · wissen/energie/training/PROGRAMM.md | 2 | **Der monatliche Wissens-Health-Check kennt keines der sieben Pruefwerkzeuge, die seit dem 28.07. fuer genau seine Audits gewachsen sind, und sein Veraltungs-Audit misst mit dem Mass, das der Alterswaechter als ungeeignet belegt hat.** Anlass im Delta: `datenstand-waechter.py` wurde am 09.09. erweitert (`8b0a2b9d6`, 15/0, energie Run 189). **Richtungsprobe:** `skills/wissenscheck/SKILL.md` hat 0 Treffer auf `wissen/tools`, `wiki-konsistenz`, `link-zielabgleich`, `waechter`, `fehloffen`, `bezugsgroessen`; der Contract fuehrt «Vorgelagert: —» (Z. 16); der Task-Prompt ebenso 0. Rueckrichtung: `wissen/tools/README.md` nennt `wissenscheck` nicht, obwohl er selbst festhaelt, die Werkzeuge «eignen sich also fuer einen Wartungslauf». **Methodischer Widerspruch:** Audit E (`SKILL.md` Z. 118-120, nach dem Querverweis-Einschub dieses Laufs; vorher 112-114) zaehlt Veraltung nur, wenn `last_updated` aelter als 90 Tage ist **und** eine neuere raw-Quelle existiert: «Reines Alter ist kein Problem». Der Docstring von `datenstand-waechter.py` (Z. 10-11 und 18-19) haelt aus dem Energie-Audit vom 23.08. das Gegenteil fest: «`last_updated` misst den letzten Zugriff, nicht das Alter der Zahlen», und «Geld und Markt dagegen veralten, ohne dass es jemand mitteilt» (35 von 37 Themenartikeln ohne `datenstand`, Preisstaende 2019 als heutige CHF-Zahl). Einen solchen Fall findet Audit E per Konstruktion nie, weil keine neuere raw-Quelle einlaeuft. Genutzt werden die Werkzeuge nur KB-weise (`energie/training/PROGRAMM.md` Z. 140) und einmal hub-weit vom Wissens-Chef (`wissen/koordination/CHANGELOG.md` Z. 1015, 420 Kandidaten). Typ 3 (Compounding-Luecke) und Typ 6 (Redundanz: zwei Mechanismen fuer dieselbe Frage, keiner kennt den anderen). | (a) Beidseitige Querverweise, damit der naechste Monatslauf (01.10.) die Werkzeuge und den Widerspruch sieht. (b) Sachentscheid, nicht dieses Laufs: Werkzeuge als maschinelle Vorstufe in die Audits einbinden (A/B mit `wiki-konsistenz.sh` und `link-zielabgleich.sh`, E um `datenstand-waechter.py` ergaenzen) oder bewusst getrennt halten; Audit E fuer Geld- und Marktzahlen neu fassen. Zustaendig Wissens-Chef bzw. Raphael. | (a) **umgesetzt 10.09.2026 (Lauf 33, active-with-flagging):** Querverweis-Absatz in `skills/wissenscheck/SKILL.md` unter dem Contract (173 → 179 Zeilen, Audit-Definition ausdruecklich unveraendert), Rueckkante als neuer Abschnitt am Ende von `wissen/tools/README.md` (262 → 271, reiner Anhang). Contract-Feld «Vorgelagert» bewusst **nicht** gesetzt: es beschriebe eine Kette, die heute nicht laeuft. (b) **offen.** **Adversariale Gegenpruefung:** (1) Ist die Trennung Absicht, weil `wissenscheck` ein urteilendes LLM-Audit ist und die Werkzeuge Kandidaten-Melder? Moeglich, aber nirgends festgehalten, und der Methodenwiderspruch bei E bleibt davon unberuehrt. (2) Ist Audit E damit falsch? Nicht fuer Rechts- und Norm-Destillate (der Waechter nimmt sie selbst aus, sie werden `superseded`), wohl aber fuer Geld und Markt. (3) Doppelt der Befund einen Registereintrag? `grep` auf die sieben Werkzeugnamen im Register 0 Treffer. |
| SYN-83 | connectors/WEGE.md (Nachtrag 10.09.2026 «`m365-graph.mjs` haengt ohne Zertifikat») · connectors/m365-graph.mjs (Z. 47-52, 120, 146, 168, 311-313) · connectors/hub-setup-daten.json (Station macbook-revendo) · Rule auto-verbesserungen 260730b · Rule wege-und-vollmachten Ziff. 5 | 3 | **Das Wege-Register schreibt einem Werkzeug eine Haenge-Ursache zu, die der Code ausschliesst — die verordnete Vorsichtsmassnahme schuetzt darum nicht vor dem echten Haenger.** `connectors/WEGE.md` Z. 1098-1105 (Nachtrag 10.09.): `m365-graph.mjs --get …` laufe ohne `~/.cli-m365-cert-combined.pem` «nicht in einen Fehler, sondern haengt» (ueber vier Minuten), darum «vor jedem Graph-Aufruf auf dieser Station zuerst `ls ~/.cli-m365-cert-combined.pem`». Der Code prueft das Zertifikat **vor** dem ersten Netzaufruf und wirft (`zertifikatLesen()` Z. 47-52: `existsSync` → `throw new Error("Zertifikat fehlt …")`; `main().catch` → `process.exit(1)`, Z. 311-313), und das seit der Fassung vom 09.08. (`bb1663278`). **Gegenprobe 11.09.2026 17:3x auf dem MacBook Pro** mit `HOME` auf ein leeres Verzeichnis: `--get /sites/root` endet in **0 s** mit «FEHLER: Zertifikat fehlt …». Der Haenger auf Revendo hatte also eine andere Ursache. Naheliegender Kandidat: alle drei `fetch`-Aufrufe (Z. 120, 146, 168) laufen **ohne Zeitdeckel** (`grep AbortSignal` ueber `connectors/*.mjs`: 0 Treffer) — Plausibilitaet, nicht Beleg. Folge: auf einer Station **mit** Zertifikat haengt ein Netz-Stall weiterhin unbegrenzt, und die `ls`-Probe meldet vorher «in Ordnung». Seit 11.09. hat Revendo das Zertifikat (`hub-setup-daten.json`, `9fc5e7c63`); der Anlass ist dort erledigt, der Defekt nicht. Typ 1 (Ketten-Luecke Register → Werkzeug); gleiche Familie wie 260730b («Plausibilitaet und Beleg getrennt ausweisen»). | (a) Unterpunkt im WEGE-Nachtrag: Ursache ungeklaert, Gegenprobe genannt. (b) Werkzeug, nicht dieses Laufs: Zeitdeckel an die drei `fetch`-Aufrufe (`AbortSignal.timeout(…)`), damit ein Stall als Fehler endet; danach die `ls`-Vorschrift durch den Werkzeugfehler ersetzen. | (a) **umgesetzt 11.09.2026 (Lauf 34, active-with-flagging):** Unterpunkt unter dem ersten Punkt des WEGE-Nachtrags, rein additiv, Ursprungssatz unangetastet. (b) **offen** (ausfuehrender Baustein, Praezedenz SYN-49/56). **Adversariale Gegenpruefung:** (1) Kann eine vorhandene, aber defekte PEM den Haenger erklaeren? Nein: sie passiert `existsSync`, scheitert aber in Z. 60 («Kein CERTIFICATE-Block») ebenfalls vor dem Netzaufruf. (2) Lief auf Revendo eine aeltere Fassung ohne Pruefung? Nicht ausgeschlossen, aber die Pruefung steht schon in der Fassung vom 09.08. (3) Existiert der Befund schon? `grep` auf «m365-graph» im Register und in `rules/betrieb-chronik.md` 260910: nur der OneDrive-Befund derselben Messung, keine Aussage zur Ursache. |
| SYN-84 | Rule auto-verbesserungen 260913 (importiert) · agents/dokument.md (Z. 17, Z. 24-28) · Rule dokument-layout-standard (importiert, Abschnitt «Geltung») · Projektordner 2619 KISPI `09_Dokumente/CLAUDE/04_Sitzung-Auftragslage/` + `05_Honorarverhandlung/` · Harness-Skill `anthropic-skills:pptx` | 1 · 3 | **Die neue Praesentations-Regel erreicht ihren einzigen Erzeuger nicht, und die Regel, auf die sie sich beruft, fuehrt PPTX weiter als eigenes Format.** Eintrag 260913 (Selfcommit `6b6e45e64`, 13.09. 23:00, 9/0) verlangt Folien und Sitzungsvorlagen als `.indd` + `.idml` + PDF, «nie als PPTX», und verweist fuer Dokumente auf `dokument-layout-standard`. Rueckkanten: (a) `agents/dokument.md` Z. 17 «Praesentationen (PPTX/Folien)» ist der im Hub einzige benannte Folien-Erzeuger und nennt die Regel nicht; (b) `rules/dokument-layout-standard.md` Z. 56 nimmt «PPTX (eigene Konvention …)» ausdruecklich aus, ohne Hinweis, dass es kein Lieferformat mehr ist; (c) der Harness-Skill `anthropic-skills:pptx` loest laut Beschreibung bei jeder «presentation» aus, wenn kein eigener Folien-Skill vorhanden ist, und im Hub gibt es keinen. **Nebenbefund beim Lesen der Rueckkante (alt, nicht im Delta):** `agents/dokument.md` «Design-Vorgaben» (Z. 24-28, Blame 25.07.) schreibt Arial, Dunkelblau `#2B5797`, 1" Rand und einen eigenen Footer vor, gegen die als abhaengig genannte Rule (Cambria 11 pt, schwarz, 20 mm, keine Akzentfarben) und die User-CLAUDE.md. `grep` auf «Arial|2B5797» im Register: 0, also nicht bekannt. **Compounding:** die Regel wird gelebt: drei InDesign-Erzeugnisse vom 14.09. im KISPI-Ordner (`260914-KISPI-Abgleich-Umbau-Auftragslage.indd`, `260914-KISPI-Roter-Faden-A4.indd`, `260921-KISPI-Strategie-Honorar-A4.indd`, je mit `.idml`), aber **kein Generator-Script** liegt im Hub (`mdfind` auf `*.jsx` seit 12.09.: 0; `skills/`, `templates/`, `scripts/` ohne InDesign-Werkzeug). InDesign 2026 ist auf beiden Stationen installiert (MacBook `/Applications/Adobe InDesign 2026`, Mini 2024/2025/2026 per `ssh mini`); das Werkzeug fehlt also nicht, nur die Wiederverwendung. | (a) Vermerk in `agents/dokument.md` unter Z. 17 und (b) Querverweis am Ende von `dokument-layout-standard` «Geltung». (c) Vermerk ueber «Design-Vorgaben» in `agents/dokument.md`: ueberholt, es gilt die Rule; Streichung der vier Zeilen ist Entscheid Raphael. (d) Das InDesign-Scripting der KISPI-Vorlagen als Werkzeug in den Hub heben (`templates/jans-praesentation/` oder `skills/<…>/tools/`, Master.indd als Grundlage), damit die vierte Vorlage nicht wieder von Grund auf entsteht; Rolle nach `rollen-taxonomie` zuerst bestimmen (builder, kein neuer Prototyper). | (a) und (b) **umgesetzt 14.09.2026 (Lauf 35, active-with-flagging)**, rein additiv, Wortlaut unangetastet. (c) **umgesetzt als Vermerk**, Streichung offen (Raphael). (d) **offen**, Entwurf. **Adversariale Gegenpruefung:** (1) Gilt der Agent ueberhaupt fuer Folien, oder ist er nie beauftragt worden? Sein Contract nennt «Zielformat (Word/PDF/Excel)» und im Text «PPTX/Folien»; ein Aufruf mit Folienauftrag landet dort. (2) Deckt der Grundkontext das nicht ohnehin ab? Der Subagent laedt die importierte Rule 260913, aber der eigene Agententext steht naeher am Auftrag und sagt das Gegenteil; derselbe Mechanismus war SYN-31. (3) Existiert der Befund schon? `grep` «pptx|InDesign|260913» im Register: nur diese Zeile. |
| SYN-85 | ~/Library/Logs/Claude/main.log beider Stationen · logbuch/fristen.md (P1-Block Z. 3-14, Erledigt-Kriterium «sobald im MacBook-Log …») · Mini-Nachtschicht (CLI, `claude-run.sh`) · Mini-App-Tasks `vollgas-chef-radar`, `vollgas-fruehwarnung`, `heartbeat-daily`, `energie-training`, `claude-abo-auslastung`, `bauleitung-training` · logbuch/vollgas/RADAR.md Z. 70 · Rule auto-verbesserungen 260824 · SYN-76 | 1 | **Die Aufsicht hat den App-Ausfall der anderen Station viermal gemessen und denselben Ausfall auf der eigenen nie.** MacBook-App im `session_stale_relogin`-Latch 11.09. 20:08 bis 14.09. 13:59:07 (P1, ab 12.09. im Register). **Mac-Mini-App im selben Latch 12.09. 22:37:10 bis 14.09. 13:57:54**, in keinem Register: `main1.log` 5, `main.log` 6 Zeilen `Cannot start session … OAuthError (session_stale_relogin)`, darunter der Mini-`vollgas-chef-radar` selbst (13.09. 00:57, 12:57; 14.09. 00:58, 12:58), `heartbeat-daily` 14.09. 08:01 und `bauleitung-training` 14.09. 04:26. Die Latch-Freigabe 14.09. 01:41:51 war «zombie re-stamp» ohne Wirkung. **Warum es niemand sah:** die Mini-Nachtschicht laeuft ueber die CLI und blieb gesund; sie hat das Register viermal frisch gemessen (12.09. 13:0x, 13.09. 05:3x und 13:3x, 14.09. 13:3x), jedes Mal per `ssh macbook`, weil das Erledigt-Kriterium der P1-Zeile nur «das MacBook-Log» nennt. Die Instanzen, die den eigenen Ausfall haetten sehen koennen (Mini-Radar, Mini-heartbeat), waren genau die ausgefallenen App-Tasks. **Kein Pruefbaustein liest das lokale App-Log:** `grep` auf «Logs/Claude/main.log» in `scripts/`, `skills/heartbeat/`, `templates/scheduled-tasks/`: 0; einzige Fundstelle ist der Bericht `logbuch/vollgas/RADAR.md` Z. 70, eine Handlungsanweisung, kein Check. Gleiche Fehlerklasse wie SYN-76 (eine Registry gemessen, die andere nicht) und wie 260824 (eine gesunde Schicht verdeckt den Ausfall der anderen); hier traf es die Aufsicht selbst. | (a) Erledigt-Kriterien von Stationsausfaellen immer fuer **beide** Stationen formulieren. (b) Eine billige CLI-Probe, die unabhaengig von der App laeuft und beide Logs zaehlt: `grep -c "Cannot start session" ~/Library/Logs/Claude/main.log` lokal und per `ssh <andere>` seit dem letzten Lauf, Meldung ab einem Fehlstart; Ort: `heartbeat`-Script als CLI-Check oder `stationen-watchdog`. (c) `bauleitung-training` und `energie-training` haben durch den Mini-Latch Slots verloren; `energie`-Nachtschichten liefen per CLI weiter, der Verlust dort ist klein. | (a) teilweise: Mini-Befund und Nachtrag im Fristen-Register gesetzt (14.09., Lauf 35), die Kriterien der geschlossenen P1-Zeile nicht umgeschrieben. (b) **offen**, Entwurf (ausfuehrender Baustein, Praezedenz SYN-49/56; Werkzeug im Hauptmodell-Urteil zu bauen, nicht in diesem Lauf). **Adversariale Gegenpruefung:** (1) Ist der Mini-Ausfall vielleicht dieselbe Ursache und darum im MacBook-P1 «mitgemeint»? Nein: kein Eintrag nennt den Mini, der Beginn liegt 26 h spaeter, und die Nachtraege schreiben «Kein Handeln moeglich» nur fuer das MacBook. (2) War der Ausfall folgenlos, weil die Nachtschicht per CLI lief? Nicht ganz: vier Mini-Radar-Slots und ein Mini-heartbeat fehlen, `bauleitung-training` hat seinen einzigen Slot verloren. (3) Ist er inzwischen behoben? Ja, seit 13:57:54 kein Fehlschlag; der naechste Mini-App-Start (`energie-training` 22:37) ist die Gegenprobe. **Nachtrag Lauf 36 (15.09.2026): Gegenprobe (3) bestanden.** Mini-`main.log` per `ssh mini` lesend: `energie-training` 14.09. 22:37:53 «Spawning», 22:37:54 «Confirmed task run»; letzter `Cannot start session` auf dem Mini 14.09. 12:58:07, auf dem MacBook 14.09. 12:57:19, seither auf **beiden** Stationen 0. (b) unveraendert offen. |
| SYN-86 | skills/tenant-hygiene/SKILL.md (Nachtrag 14.09., `ea61c2313`) · tenant-hygiene/reports/260914-hygiene.md · logbuch/fristen.md (Zeile «NEU 13.08. — Der SharePoint-Speicherpool …», Status «Rest offen») · connectors/WEGE.md (Weg 3 PnP, Z. 386-412) · Rule auto-verbesserungen 260805 · Rule wege-und-vollmachten | 1 · 3 | **Die Korrektur einer Handlungsempfehlung erreicht das Register nicht, und der neue Behebungsweg liegt auf der falschen Plattform.** Der Hygiene-Lauf vom 14.09. 20:10 hat richtig gemessen, dass der seit dem 13.07. empfohlene Hebel «Tenant-Versionslimit auf Automatisch» **seit spaetestens 27.07. erfuellt** ist und nur auf neue Bibliotheken wirkt; die sieben Bibliotheken von JANS.PROJEKTE stehen weiter manuell auf 500. Er hat die Korrektur in Report und SKILL geschrieben (`ea61c2313`, 12/0), **nicht** ins Fristen-Register: dort steht an der Speicherpool-Zeile weiterhin «**Rest offen: Tenant-Versionslimit auf «Automatisch» stellen** (Admin Center)», also die widerlegte Aktion. Genau diese Zeile traegt schon die Lehre «ein Laufbericht ist kein Meldekanal» (sie entstand am 13.08., weil der Hebel einen Monat nur in Reports stand); das Muster wiederholt sich an derselben Stelle. Zweitens nennt der neue SKILL-Absatz als Behebung «interaktiv, **SPO Management Shell**, kein CLI-Set-Befehl vorhanden». `connectors/WEGE.md` fuehrt die SPO-Shell als **Windows-only** («Damit erreichbar, was sonst die Windows-only SPO-Management-Shell braeuchte») und nennt fuer diesen Fall Weg 3, PnP mit geliehenem Token. Gemessen 15.09. per `pwsh` `Get-Command` (lesend): das installierte `PnP.PowerShell 3.1.0` hat `Set-PnPSiteVersionPolicy` mit `-EnableAutoExpirationVersionTrim` und `-ApplyToExistingDocumentLibraries`. Die Wirkung ist nicht gemessen. Stand 14.09.: 917.67 GB / 87.07 %, 88-%-Marke 9.85 GB entfernt. Nebenbefund Contract: die SKILL fuehrte `Verwandt`, aber nicht die Pflichtfelder `Vorgelagert`/`Nachgelagert`. | (a) Nachtrag an der Fristen-Zeile mit dem richtigen Hebel. (b) Vermerk in der SKILL: PnP-Gegenstueck statt SPO-Shell, Signatur gemessen, Wirkung offen. (c) Contract-Pflichtfelder, `Nachgelagert: logbuch` mit dem Hinweis auf 260805. (d) Methodisch fuer alle Aufsichts-Loops: wer eine eigene fruehere Empfehlung korrigiert, sucht die Stelle, an der sie im Register steht, und traegt die Korrektur dort nach; Kandidat fuer den Schlussschritt im `tenant-hygiene`-Ablauf (Rolle maintainer, kein neuer Baustein). (e) Ob `Set-PnPSiteVersionPolicy` mit dem App-Token genuegt, einmal interaktiv mit Raphael pruefen und in WEGE Weg 3 eintragen. | (a) **umgesetzt 15.09.2026 (Lauf 36, active-with-flagging)**, in-place-Nachtrag an der bestehenden Zeile, Zeilenzahl 5521 → 5521. (b) und (c) **umgesetzt**, additiv, SKILL 85 → 99, Wortlaut des 14.09.-Absatzes unangetastet. (d) **offen**, Entwurf. (e) **erledigt 15.09.2026 17:28** (interaktiv, Auftrag Raphael): app-only scheitert, delegiert als rj@ gesetzt, Job `7fc93ccd…` «New»; WEGE-Nachtrag 15.09. mit Weg und Sackgasse, Chronik 260915. Trim bleibt Raphael (endgueltige Loeschung), Befehl im Fristen-Register. Urspruenglich: WEGE.md in diesem Lauf bewusst **nicht** editiert, weil eine interaktive Session die Datei um 16:4x zuletzt geschrieben hat. **Adversariale Gegenpruefung:** (1) Hat Raphael den richtigen Hebel nicht ohnehin per Mail? Wahrscheinlich ja, der Lauf vom 14.09. hatte den Sendegrund Montag; das Register ist aber der Ort, den `hub-chef` und `logbuch-radar` lesen, und dort stand das Gegenteil. (2) Ist die Zeile nicht erledigt und darum harmlos? Sie ist erledigt **bis auf einen «Rest offen»**, und genau dieser Rest ist falsch. (3) Existiert der Befund schon? `grep` «ApplyToExisting|Set-PnPSiteVersionPolicy» im Register und im Fristen-Register vor diesem Lauf: 0. **Nachtrag 18.09.2026 (Lauf 39):** Kern **geschlossen** — der Hygiene-Lauf vom 17.09. 20:1x hat die Versions-Frage app-only beantwortet (`spo site versionpolicy get` + `spo tenant settings list`: Site erbt, Tenant `EnableAutoExpirationVersionTrim: true`) und die Korrektur **selbst** als Nachtrag an die Speicherpool-Zeile in `logbuch/fristen.md` gesetzt (Z. 5647) sowie in WEGE (Nachtrag 17.09.). Damit hat der Schlussschritt aus (b), `skills/tenant-hygiene/SKILL.md` Z. 15-16, im ersten Anlassfall gegriffen. (d) bleibt als Verallgemeinerung fuer die uebrigen Aufsichts-Loops **offen, Entwurf**. |
| SYN-87 | connectors/WEGE.md (Nachtrag 15.09. «Kanal-/Leitungskataster Stadt Zürich», Selfcommit `3cdbfcc65`; Nachtrag 08.09.) · wissen/planungsgrundlagen/wiki/kartenportale-werkleitungskataster.md · skills/machbarkeit (Z. 101) · skills/behoerden-vorabklaerung (Z. 39) · skills/planungsgrundlagen/connectors/geoshop-zh.mjs · wissen/baurecht (Ruecklauf Durchleitungsrecht, `a30be5b94`) | 3 | **Ein neuer, belegter Weg landet im Wege-Register, aber nicht in dem Wiki-Artikel, auf den das Register selbst verweist.** Der WEGE-Nachtrag vom 15.09. (Anlass Steinbruechelstrasse 26/28, Lage der Hauptabwasserleitung, Einwand A. Tonet 10.09.) nennt zwei verifizierte Bezugswege fuer die Stadt Zuerich: GeoShop Stadt Zuerich (AGOV-Login) und das ERZ-Planarchiv Liegenschaftsentwaesserung. Der Nachtrag vom 08.09. verweist fuer «weitere Wege» auf `kartenportale-werkleitungskataster.md`; dieser Artikel (`last_updated` 2026-08-01) kennt nur EKZ, Swisscom, Sunrise-UPC und den SZ-Fall EW Lachen, die Stadt Zuerich fehlt, obwohl `machbarkeit` und `behoerden-vorabklaerung` Werkleitungsfragen dorthin schicken. Die Baurecht-Seite desselben Vorgangs ist sauber zurueckgelaufen (`a30be5b94`, Durchleitungsrecht Art. 742 vs. 693 ZGB), die Geodaten-Seite nicht. Zudem widersprechen sich die beiden WEGE-Nachtraege im Kostenpunkt (08.09. ERZ «kostenpflichtig», 15.09. «beide kostenlos»), ohne Vermerk am aelteren. Gemessen im selben Lauf: der kantonale `geoshop-zh.mjs` fuehrt keinen Kanal-/Leitungskataster (`--list --filter leitung`: nur 410 AV-Rohrleitungen und 1676 Quellstraenge; «kanal», «abwasser»: 0). | (a) Offene Frage mit beiden Wegen, dem Kostenwiderspruch und der GeoShop-Messung in `wissen/planungsgrundlagen/wiki/QUESTIONS.md` (Cross-KB-Bringschuld, grep-faehig). (b) Einarbeitung als §8 «Stadt Zuerich» in den Artikel nach Gegenlesen der beiden Quellseiten, durch einen Lauf mit Web-Zugang; der monatliche `planungsgrundlagen-wartung` produziert laut Statuskopf kein neues Wissen, also eher der naechste Projektlauf, der Werkleitungen braucht. (c) Am WEGE-Nachtrag 08.09. einen Verweis «Kostenfrage siehe 15.09.» setzen, sobald die Datei nicht mehr in Bearbeitung ist. | (a) **umgesetzt 15.09.2026 (Lauf 36, active-with-flagging)**, QUESTIONS 2264 → 2287, CHANGELOG der KB nachgefuehrt; Punkt (c) der Frage im selben Lauf gemessen und abgehakt. (b) und (c) **offen**. **Adversariale Gegenpruefung:** (1) Arbeitet die interaktive Session von heute den Artikel vielleicht noch nach? Moeglich; die Frage ist dann schnell abzuhaken, ein doppelter Artikel entsteht dadurch nicht. (2) Ist das nicht blosse KB-Inhaltsarbeit ohne Synergie-Bezug? Nein, es ist eine Rueckkante zwischen dem Wege-Register (Connector-Schicht) und der KB, auf die es verweist. (3) Existiert der Befund schon? `grep` «Leitungskataster|GeoShop Stadt|ERZ» im Register: 0. |
| SYN-88 | rules/anrede-kontakte.md (importiert, Klausel «Gewicht des Anliegens schlaegt den Routine-Default», Z. 26-34 + Vermerk Lauf 58 Z. 52-59) · rules/jans-dna-facetten.md (Auto-Block, Abschnitt Beziehungsregister, Selfcommit `e78436e8e` 16.09. 06:19, 18/28) · wissen/twin/wiki/QUESTIONS.md 260916b #1 · wissen/twin/outputs/2026-09-16_fidelity.md · skills/twin/tools/build_dna.py · SYN-53 · SYN-71 | 1 | **Der Auto-Block traegt seit heute eine Einschraenkung als geltende Feinregel, welche die importierte Klausel in ihrer eigenen Kernaussage trifft.** Der Fidelity-Review vom 16.09. mass an der Abgebots-Bitte an Guido Giacometti (23.02.2026 16:06, CHF 1'000 Nachlass) «**Grüezi** Herr Giacometti» — bei demselben Kontakt am Vortag «Guten Tag», am Folgetag «Sehr geehrter Herr Giacometti». Die gewichtigste Mail des Vorgangs traegt also den **lockersten** Opener. Daraus kompiliert `build_dna.py` in `jans-dna-facetten.md`: «Gewichtig macht, was ich GEBE (Ergebnis, Recht, Antrag) — nicht Geld, nicht Konflikt und **nicht die Bitte um ein Zugestaendnis**». **Die importierte Rule sagt an zwei Stellen das Gegenteil:** die Klausel selbst («ein verbindliches Ergebnis oder eine **folgenreiche Bitte** zieht «Geschaetzte/r \<Vorname\>»») und der Vermerk aus Wissens-Chef Lauf 58 vom 14.09. («**ein Ergebnis oder eine Bitte hebt**, ein Konflikt oder Ultimatum nicht»). Eine Abgebots-Bitte ist eine folgenreiche Bitte; nach der importierten Fassung muesste sie heben, nach der kompilierten nicht. `grep` auf «Zugestaendnis|Abgebot|260916|Grüezi|GEBE» in `anrede-kontakte.md` vor diesem Lauf: **0**. **Die Bringschuld war erkannt und blieb liegen:** die Frage 260916b #1 nennt die Ziel-Rule beim Namen und begruendet richtig, warum der Twin-Lauf sie nicht selbst aendert («den Wortlaut einer Rule aendert Claude nicht selbst») — nur landete danach nichts an der Rule, waehrend die Facetten-Datei die unbestaetigte Lesart bereits als Regel fuehrt. Asymmetrie der Reichweite: die Rule laedt in **jede** Session, die Facetten-Datei nur, wer die Pflichtlektuere befolgt. | (a) Datierter Vermerk an der Klausel, Wortlaut unangetastet, mit der praktischen Folge bis zum Entscheid: bei einer Bitte um ein Zugestaendnis (Abgebot, Nachlass, Entgegenkommen, Fristerstreckung) den Routine-Opener halten; bei Ergebnis, Recht, Antrag weiter heben. (b) Methodisch fuer den `twin`-Harness: erkennt ein Fidelity-Lauf, dass ein Marker den Wortlaut einer **importierten** Rule beruehrt, gehoert ausser der Frage in QUESTIONS ein grep-faehiger Vermerk an die Rule selbst — dieselbe Cross-KB-Bringschuld, die `wissens-bibliothekar` seit 23.08. fuer KBs kennt, hier zwischen KB und Rule. Kandidat fuer den Schlussschritt des `twin-fidelity-review` (Rolle maintainer, kein neuer Baustein). (c) Frage 260916b #1 in die naechste Vorlage an Raphael aufnehmen; sie entscheidet ueber Klauseltext **und** Auto-Block zugleich. | (a) **umgesetzt 16.09.2026 (Lauf 37, active-with-flagging)**, rein additiv, `anrede-kontakte.md` 158 → 178 Zeilen, keine entfernte Zeile, Klauseltext und Kontaktliste (23 Tabellenzeilen) unveraendert. (b) und (c) **offen**, Entwurf. **Adversariale Gegenpruefung:** (1) Ist es ueberhaupt ein Widerspruch, oder praezisiert die Facetten-Fassung nur? Sie macht eine Ausnahme von «eine Bitte hebt», nennt also einen Fall, den die importierte Fassung umgekehrt entscheidet — eine Praezisierung waere es, wenn die Rule zur Bitte schwiege; sie tut es nicht, der Lauf-58-Vermerk sagt es ausdruecklich. (2) Ist der Beleg tragfaehig genug fuer einen Vermerk an einer importierten Rule? Der Vermerk behauptet nichts, er referiert die Messung und markiert die Frage als offen — genau das Muster, das diese Datei seit 23.08. viermal traegt. Ein einzelner Gold ist duenn; darum «offenbar» im Titel und der Entscheid ausdruecklich bei Raphael. (3) Existiert der Befund schon? `grep` «Zugestaendnis|Abgebot» im Register: 3 Treffer, alle in anderen Zusammenhaengen (Service-Satz, Kurzzahl); anrede-kontakte ist Beteiligter in SYN-43 (Treffen-Klausel), SYN-53 (englische Anrede) und SYN-71 (Reichweite Du/Sie) — **dieselbe Kanal-Familie, anderer Sachverhalt**: SYN-71 fragt, WEN die Klausel erfasst, SYN-88 fragt, WAS sie hebt.  **Nachtrag Wissens-Chef Lauf 61 (17.09.2026) — Teil der Kollision ist aufgeloest, und die Praemisse war schwaecher als angenommen:** die Rekompilierung vom 17.09. hat den Auto-Block an die Rule herangerueckt (`jans-dna-facetten.md` Z. 205: «Die Fach-/Empfehlungsanfrage ist KEIN Zugeständnis — sie HEBT»), die Facetten-Seite widerspricht der Klausel insoweit nicht mehr. Zweitens: der **Ankerfall der Klausel selbst** — Besnik, 21.05.2026, Betreff «Gewerke/Empfehlung», in `anrede-kontakte.md` Z. 30-32 zitiert — ist dieselbe Gattung am lockersten Du-Kontakt und **hebt**; der Stand ist damit **3:0**, nicht 2:0, und der dritte Beleg lag die ganze Zeit in der Rule. Die importierte Rule hat also nie falsch gesteuert, falsch steuerte allein der am 16.09. kompilierte Block. **Offen bleibt allein der Sachentscheid Raphaels** (hebt die Bitte generell, senkt allein das Zugestaendnis?); die Frage ist in `wissen/twin/wiki/QUESTIONS.md` 260916b #1 entsprechend praezisiert worden. Punkt (b) — grep-faehiger Vermerk an der Rule als Schlussschritt des `twin-fidelity-review` — bleibt unveraendert offen. **Nachtrag 18.09.2026 (Lauf 39), Beteiligter bewegt:** der Auto-Block wurde zweimal nachkompiliert, `anrede-kontakte.md` im Fenster nicht. (1) 260917 entschaerft die Spannung teilweise: «Die Fach-/Empfehlungsanfrage ist KEIN Zugestaendnis — sie HEBT auf ‹Geschätzter <Vorname>›, auch wo die Liste ‹Hoi› fuehrt (2:0)» — ausgenommen bleibt nur die **Zugestaendnis**-Bitte, die Lesart «ein Ergebnis oder eine Bitte hebt» der Klausel stimmt damit fuer die Sachbitte wieder. (2) 260918 (`6a6aa31ba`, `rules/jans-dna-facetten.md` Z. 206 und Z. 412-Umfeld) schneidet eine **neue Achse** ein, die die Klausel nicht kennt: «das Preisangebot INLINE im Faden bleibt ‹Hoi›», die Offerte als neue Mail mit Anhang hebt (Beleg Steinmann 19.05.2026 14:44 / 15:12, `wissen/twin/wiki/beziehungsregister.md` Z. 4698-4702; der Twin selbst: «fuenfter Lauf in Folge, in dem eine Heberegel ihre Nachbargattung verliert»). Nicht das Gewicht allein, auch der **Kanal** entscheidet. (3) Nebenbefund an der Kontaktliste: die Zeile **Roman Steinmann** (Z. 148) fuehrt «Salut Roman» als Routine und kennt weder die Sie-Phase bis Maerz 2026 («Sehr geehrter Herr Steinmann», Jan. 2026, beziehungsregister Z. 882-891) noch die belegten «Hoi Roman»/«Geschätzter Roman,» — dieselbe Luecke, die bei **Undine Hofer** am 03.09. der Fidelity-Review selbst in der Zeile geschlossen hat. Empfehlung (c) ergaenzt: die Steinmann-Zeile vom `twin`-Harness analog Hofer nachfuehren lassen (Zeile belegt, nur ergaenzen). **Dieser Lauf hat `anrede-kontakte.md` nicht angefasst** — (2) ist offene Frage an Raphael (`wissen/twin/wiki/QUESTIONS.md` 260918b), nicht Befund zum Setzen. |
| SYN-89 | rules/betrieb-chronik.md (Eintrag 260916, Selfcommit `0a0c15f50` 16.09. 10:25, 20/0) · logbuch/fristen.md (Z. 3, Z. 7) · scripts/heartbeat.sh · skills/heartbeat/SKILL.md (16 Checks) · skills/studien-generator/tools/md2docx.py · Rule interaktive-eingriffe · Rule auto-verbesserungen 260807 | 3 | **Zwei Werkzeugketten brachen am selben Tag aus demselben Grund; die eine wurde zum Dauercheck befoerdert, die andere blieb eine Chronik-Lehre.** Am 16.09. 08:4x riss die DOCX-Pipeline (Interpreter ohne `docx`, auf beiden Stationen verschieden gelagert) — behoben **und** in `scripts/heartbeat.sh` verankert: der Check erzeugt seither ein echtes Test-DOCX statt `import docx` zu messen (`grep` «md2docx|Dok-Pipeline» in `heartbeat.sh`: 6 Treffer). Am selben Tag 10:2x riss der Drucker-Kostenstellen-Dialog, weil das Update auf macOS 27 **Rosetta 2 entfernt** hatte und alle sieben Konica-Minolta-PDEs reine x86_64-Binaries sind. Der Lauf war vorbildlich (angekuendigt, umkehrbar, Chronik und Register gesetzt, Mac Mini per `ssh` gegengemessen: dort macOS 27 **mit** Rosetta, der Verlust betraf nur das MacBook Pro). Seine Schlusszeile formuliert eine stationsuebergreifende Lehre: «nach jedem grossen macOS-Update auf Apple Silicon zuerst Rosetta messen, bevor am Treiber gesucht wird; **gilt auch fuer den Mac Mini**». **Diese Lehre erreicht keinen Baustein.** `grep` «arch -x86_64|softwareupdate|oahd» in `scripts/heartbeat.sh`: **0**; keiner der 16 heartbeat-Checks misst die lokale Werkzeug-Architektur. Der Fristen-Eintrag traegt korrekt den **konkreten** Folgepunkt (nativer Treiber oder AirPrint vor macOS 28, weil Apple Rosetta dann auf Spiele beschraenkt), nicht die generische Messung. Die Chronik ist **nicht importiert** und wird laut CLAUDE.md nur gelesen, «wer an der Automatik arbeitet» — die Lehre richtet sich aber an jede Station nach jedem Major-Update. Gleiche Familie wie Rule 260807: der Zustand eines Werkzeugs gilt ungeprueft als unveraendert, bis jemand einen Anlass sieht. | (a) Billiger Architektur-Check im `heartbeat` (Abschnitt Produktions-Bereitschaft, neben dem neuen DOCX-Check): `arch -x86_64 /usr/bin/true` und die OS-Version gegen den letzten bekannten Stand; Meldung nur bei Wechsel oder Fehlschlag, also kein taegliches Rauschen. Er kostet Millisekunden und haette den Ausfall vor dem Drucker gemeldet. (b) Allgemeiner: nach einem Major-OS-Update einmal die Werkzeugketten messen, die auf Binaries Dritter stehen (Druckertreiber, CAD-Plugins, LibreDWG, c4dpy, venvs) — Kandidat fuer eine eigene kurze Liste im `heartbeat`-Skill statt fuer je einen Einzelbefund. (c) Rolle vor dem Bau bestimmen (`rollen-taxonomie`): maintainer am bestehenden `heartbeat`, kein neuer Baustein. | **offen**, Entwurf — in diesem Lauf bewusst nicht gebaut: ein Check in `scripts/heartbeat.sh` ist eine Script-Aenderung am laufenden Aufsichtsbaustein, keine «umsetzbare Kleinigkeit» im Sinn der Skill-Leitplanke. **Adversariale Gegenpruefung:** (1) Ist der Punkt nicht laengst im Register terminiert und damit sicher? Der Register-Eintrag sichert den **Drucker** vor macOS 28; er misst nicht, ob nach dem naechsten Update irgendeine andere x86_64-Kette faellt — und genau das sagt die Lehre. (2) Waere ein taeglicher Check nicht Rauschen fuer ein Ereignis, das zweimal im Jahr eintritt? Er schlaegt nur bei Wechsel an; die Kosten sind eine Prozessausfuehrung, der verhinderte Fall kostete einen halben Vormittag Suche am falschen Ort (Treiber statt Laufzeit). (3) Existiert der Befund schon? `grep` «Rosetta|x86_64|Apple Silicon|macOS-Update» im Register: **0**. (4) Ist die Diagnose des Laufs selbst vielleicht falsch und der Befund damit hinfaellig? Nein, sie ist dreifach belegt (fehlendes `oahd`, kein Receipt, dlopen-Gegenprobe «incompatible architecture» nativ / laedt unter Rosetta). |
| SYN-90 | Rule auto-verbesserungen 260702 (Neufassung 17.09.2026, importiert) · logbuch/AKTIONS-WHITELIST.md Z. 35 (A9) · Scheduled Task `bexio-buchen-monatlich` (Mac Mini, SKILL.md Z. 7 und Z. 17) · skills/hub-chef/SKILL.md (Z. 12, Z. 27) | 1 | **Die neugefasste Buchungsvollmacht zitiert als ihre eigene Grundlage eine Whitelist-Zeile, die im Zweifelsfall das Gegenteil anordnet.** Eintrag 260702 wurde am 17.09.2026 auf Entscheid Raphaels gekippt (Selfcommit-Kette `636514c70`…`210c6c873`): Claude bucht die Banktransaktionen «selbstaendig, ohne Betragsgrenze», und «was sich nicht belegen laesst, geht auf 1090 Transferkonto mit Textpraefix ABKLAERUNG», ausdruecklich «nie auf ein geratenes Aufwandkonto». Derselbe Eintrag schliesst mit «Mahnstufen weiter nach `logbuch/AKTIONS-WHITELIST.md` (A1, A9)». **A9 ist aber keine reine Mahn-Zeile:** sie heisst woertlich «bexio-Buchungen **und** Mahnstufe 2+» und regelt denselben Kontierungs-Zweifelsfall anders — «bei Zweifel an Betrag, Konto oder Duplikat: **Entwurf statt Buchung**». Zwei Anweisungen fuer eine Lage: buchen auf 1090 gegen nicht buchen. Zweite Achse derselben Zeile: A9 traegt ein **Widerrufsfenster von 60 Minuten**, waehrend die Neufassung den Bericht «im Tagesbriefing» vorsieht — bei einem **monatlichen** Batch-Lauf ist ein 60-Minuten-Fenster praktisch kein Widerruf. Kanten-Messung: die Rule nennt A9, A9 nennt die Rule **0-mal** (`grep` «260702» in `logbuch/AKTIONS-WHITELIST.md`: 0); die Whitelist wurde im Fenster **nicht angefasst** (`git log --since` auf die Datei: leer). Der `hub-chef` fuehrt die Whitelist als Governance-Dokument (SKILL.md Z. 12 «Governance in logbuch/AKTIONS-WHITELIST.md», Z. 27 als Input) und ist damit die Stelle, die A9 im Zweifel liest. | (a) In A9 einen datierten Vermerk setzen, dass fuer die **Kontierung** von Banktransaktionen seit 17.09.2026 Eintrag 260702 gilt (1090/ABKLAERUNG statt Entwurf) und dass A9 dort nur noch Trockenlauf, Protokoll und Stornierbarkeit traegt; Mahnstufe 2+ unveraendert. (b) Klaeren, ob das 60-Minuten-Widerrufsfenster fuer den Monatslauf durch die Berichtspflicht im Tagesbriefing ersetzt wird — sonst steht eine Frist im Dokument, die niemand einhalten kann. **Beides ist Raphaels Vollmachtstext; dieser Lauf schreibt ihn nicht.** | offen, vorgelegt. **Ehrliche Daempfung (gemessen, nicht vermutet):** der Vollzug ist gedeckt — der Task-Prompt auf dem Mac Mini regelt den Zweifelsfall selbst nach der Neufassung (Z. 17: «NICHT auf ein geratenes Aufwandkonto … auf Konto 1090 Transferkonto buchen mit Textpraefix ABKLAERUNG»), nennt die harten Grenzen vollstaendig (Z. 7) und beruft sich auf beide Grundlagen zugleich. Der Widerspruch trifft also nicht den laufenden Buchungslauf, sondern jede andere Stelle, die A9 als Vollmacht liest — namentlich den `hub-chef`. **Adversariale Gegenpruefung:** (1) Laesst sich A9 so lesen, dass «Entwurf statt Buchung» nur die Mahnstufe meint? Die Zelle nennt «Betrag, Konto oder Duplikat» — «Konto» ist woertlich der Kontierungsfall, nicht der Mahnfall. (2) Existiert der Befund schon? `grep` «bexio» im Register: nur SYN-05 (Stammdaten) und SYN-23 (Mahnwesen/Verzug), anderes Thema. (3) Ist die Whitelist vielleicht bewusst unberuehrt geblieben? Sie ist am 09.08.2026 auf v2 gehoben worden (`c634b7607`), A9 stammt aus diesem Stand; die Neufassung ist sechs Wochen juenger und nennt A9, ohne ihn anzupassen — das spricht fuer Liegenbleiben, nicht fuer Absicht. |
| SYN-91 | templates/user-level/CLAUDE.md Z. 57 (kanonisch) + ~/.claude/CLAUDE.md Z. 57 (verteilte Kopie, beide Stationen) · Rule auto-verbesserungen 260702 (Neufassung 17.09.2026) · scripts/user-claude-sync.sh | 1 | **Die Grundschicht, die in JEDER Session laedt, verbietet woertlich, was die Projekt-Rule seit heute anordnet.** Die User-Level-CLAUDE.md fuehrt unter «Grenzen» seit dem 29.07.2026 unveraendert (`29bcb1ac2`, letzte Aenderung der Datei): «Nie Zahlungen ausloesen, **nie Buchungen ohne ausdrueckliche Einzelfreigabe**.» Eintrag 260702 ersetzt genau diese Formel («ersetzt ‹buchen NUR nach Einzelfreigabe›») und laesst selbstaendiges Buchen ohne Betragsgrenze zu. Die verteilte Kopie traegt denselben Wortlaut (gemessen an beiden Pfaden, Z. 57 identisch). Damit stehen im selben Grundkontext eine absolut formulierte Geld-Grenze und ihre Aufhebung nebeneinander. | Entscheid Raphaels, ob die Zeile 57 den Buchungsteil verliert oder einen Verweis auf 260702 bekommt; danach `bash scripts/user-claude-sync.sh --alle`, damit beide Stationen denselben Stand tragen. **Dieser Lauf aendert die Sicherheitsgrundschicht nicht** — eine Vollmacht weitet man nicht durch einen Aufsichtslauf. | offen, vorgelegt. **Ehrliche Daempfung:** die User-CLAUDE.md loest den Rang selbst auf (Z. 51: «Im Hub-Repo gilt dessen CLAUDE.md und die importierten Rules; diese Datei ist nur die Grundschicht darunter»), formal gewinnt im Repo also die Projekt-Rule; und die Grundschicht ist die **strengere** Seite, ein Fehlgriff fuehrt zu Nicht-Handeln, nicht zu falschem Geld. Der Rest ist trotzdem real: die Rangklausel ist allgemein formuliert, die Grenze absolut («Nie … ohne ausdrueckliche Einzelfreigabe»), und ausserhalb des Hub-Repos laedt die Projekt-Rule gar nicht. **Adversariale Gegenpruefung:** (1) Schon registriert? `grep` «user-level|user-claude» im Register: 0. (2) Erreicht der Widerspruch den Buchungslauf? Nein — er laeuft im Repo, wo die Rangklausel greift, und sein Prompt nennt die harten Grenzen selbst; deshalb kein Fristen-Register-Eintrag nach Rule 260805. (3) Gehoert das ueberhaupt in dieses Register? Ja: es ist eine Kante zwischen zwei Kontext-Schichten, und Rule `auto-verbesserungen` 260719 weist die Schichtwahl ausdruecklich als Entwurfsfrage aus. |
| SYN-92 | connectors/WEGE.md (Nachtrag 18.09.2026 «Ausschnitt aus einem Revit-Geschoss-DWG», `748233cdf`; Absatz «Zu Zeile CAD» Z. 64-69; Faehigkeits-Tabelle Z. 58-59) · skills/pdf2dwg/SKILL.md (Abschnitt «Grenzen», Z. 95-98 vor diesem Lauf) · Rule wege-und-vollmachten · SYN-38 | 3 | **Eine neue CAD-Sackgasse landete nur im Wege-Register, obwohl dieses Register den Skill ausdruecklich zum kanonischen Ort der CAD-Sackgassen erklaert.** WEGE Z. 64-66: die Sackgassen dieses Wegs «stehen vollständig in `skills/pdf2dwg/SKILL.md` (Abschnitt ‹Grenzen›) und werden hier bewusst nicht kopiert». Der Nachtrag vom 18.09. (Fall 2619 KISPI, HdM-Deckenspiegel OG1, `AC1024`) belegt eine neue: LibreDWG `dwg2dxf` laeuft mit rc=0 und schreibt ein DXF **ohne ENTITIES-Sektion** — «das sieht nach Erfolg aus und ist keiner». Er steht nur am Dateiende von WEGE (Z. 1284 ff.). Der Skill empfahl in «Grenzen» (Z. 95-96) weiterhin unqualifiziert den «`dwg2dxf`-Roundtrip» als Pruefweg; wer vom Skill her kommt — und das ist der Weg, den WEGE selbst vorschreibt («Vor jedem CAD-Auftrag beide Seiten lesen») — erfaehrt nichts davon. Zweitens: die Faehigkeit «Fremd-DWG lesen/ausschneiden» hat in der Tabelle keine Zeile; die beiden CAD-Zeilen (Z. 58-59) decken PDF/DXF→DWG und ACIS→2D, nicht das Lesen eines Planer-DWG. Gleiche Familie wie SYN-38 (die Rueckkante pdf2dwg↔WEGE wurde damals im Skill gesetzt, weil WEGE zwei Angaben fuehrte, die dort fehlten). | (a) Querverweis im Skill «Grenzen» mit Sackgasse, Geltungsgrenze des Roundtrips (`AC1021` ja, `AC1024` nein) und Zeiger auf den WEGE-Nachtrag. (b) Zeiger unter «Zu Zeile CAD» in WEGE auf den Nachtrag, statt eine neue Tabellenzeile anzulegen (die Tabelle gehoert den Wegen, nicht den Nachtraegen; Raphael bzw. die naechste interaktive CAD-Session entscheidet, ob «Fremd-DWG lesen» eine eigene Zeile bekommt). (c) Methodisch fuer jeden, der einen WEGE-Nachtrag schreibt: pruefen, ob der Abschnitt einen anderen Ort zum kanonischen erklaert — dann dort spiegeln. | (a) und (b) **umgesetzt 18.09.2026 (Lauf 39, active-with-flagging)**, rein additiv: SKILL 98 → 106, WEGE 1313 → 1319, kein Wortlaut veraendert. (c) **offen**, Entwurf. **Adversariale Gegenpruefung:** (1) Ist der Skill ueberhaupt zustaendig, wenn er nur schreibt (PDF/DXF→DWG)? Ja fuer die Pruefung: «Grenzen» empfiehlt `dwg2dxf` als Verifikationsweg, und genau diese Empfehlung ist fuer Revit-DWG falsch-positiv. (2) Hat die Session vom 18.09. die Kante bewusst ausgelassen? Der Nachtrag nennt pdf2dwg nicht; die WEGE-Konvention Z. 64-66 widerspricht einer bewussten Auslassung. (3) Existiert der Befund schon? `grep` «AC1024|ENTITIES» in `skills/` vor diesem Lauf: 0. WEGE war zuletzt 14:26 geschrieben, der Eingriff 17:3x, keine parallele Schreibung erkennbar. |
| SYN-93 | skills/pdf2dwg/SKILL.md («Grenzen» Punkt a, Selfcommit `42473400a` 19.09. 15:30) · wissen/planungsgrundlagen/outputs/2026-09-19_dwg-export-vom-macbook-pro-via-ssh-mini.md · connectors/WEGE.md (Zeile «CAD», Z. 58) · rules/betrieb-chronik.md (ssh-PATH-Falle) | 3 | **Ein neuer Weg erreichte den Skill und die KB, aber nicht das Wege-Register.** Am 19.09. wurde belegt, dass DWG auch vom MacBook Pro aus sofort entsteht: DXF mit ezdxf aus `~/.venvs/volumen3d`, `scp` auf den Mac Mini, dort `/opt/homebrew/bin/dxf2dwg` mit vollem Pfad, `scp` zurueck. Skill und Output halten das fest; `connectors/WEGE.md` fuehrte in der CAD-Zeile nur `pdf2dwg` mit dem venv `~/.venvs/pdf2dwg`, das es laut Skill nur auf dem Mac Mini gibt, und kannte `dxf2dwg`/`volumen3d` in diesem Zusammenhang nicht (`grep` «dxf2dwg» in WEGE: 0 vor dem Eingriff). Genau der Fall der Rule `wege-und-vollmachten`: wer auf dem MacBook Pro liest «LibreDWG nur auf dem Mac Mini», haelt die Aufgabe fuer stationsgebunden. Gleiche Kantenfamilie wie SYN-92, nur in Gegenrichtung (dort Sackgasse WEGE → Skill, hier Weg Skill → WEGE). **Nebenbefund:** der Output ordnet die ssh-PATH-Falle der Rule `auto-verbesserungen` 260917b zu («dort fuer `brew` selbst notiert»); 260917b behandelt die `-o`-Flags und den `launchctl`-rc, die PATH-Falle steht in `rules/betrieb-chronik.md` (Abschnitt npm/uv/brew) und fuer `node` in WEGE. Nicht korrigiert, Output ist ein datierter Report. **Adversariale Gegenpruefung:** (1) Deckt der Verweis «Vor jedem CAD-Auftrag beide Seiten lesen» im Skill das nicht ab? Nur fuer den, der schon im Skill ist; WEGE ist der Ort, den die Rule vor «geht nicht» vorschreibt. (2) Doppelt der Nachtrag den Skill? Nein, er nennt den Weg in fuenf Zeilen und verweist fuer Schrittfolge und Abnahme auf Skill und Output. | Querverweis in `connectors/WEGE.md` nach dem SYN-92-Absatz gesetzt (active-with-flagging, 1319 → 1330 Zeilen, 0 entfernt). Optional spaeter: CAD-Tabellenzeile um den Zweitweg ergaenzen, wenn der Weg ein zweites Mal gelaufen ist. | umgesetzt (Lauf 40, 19.09.) |
...
Lauf 24: 31.08.2026 17:11-18:2x (Tagestakt, Delta-Basis 30.08.2026 17:10, Fenster 24,0 h,
179 Commits im NAS-Repo, 82 auf `origin/main`). **Uhr-Check bestanden:** Stationsuhr 17:20 CEST,
juengster nativer Synology-Zeitstempel 17:15:06 (Selfcommit-Log) und 17:15:01 (NAS-HEAD
`a9083705d`) — Abweichung unter fuenf Minuten, kein Nachstellen noetig.

**Der Lauf begann mit einem Betriebsbefund, nicht mit dem Delta.** Das NAS war nicht unter
`/Volumes/daten` erreichbar; die Freigabe hing seit rund vier Stunden auf `/Volumes/daten-1`, und
keiner der drei Waechter kann diesen Zustand sehen oder heilen (SYN-64). Diese Session startete
deshalb ohne Skills, Agenten und ohne die per `@` importierten Rules; SKILL-Text, Rules und jedes
zitierte Objekt wurden fuer diesen Lauf einzeln gelesen. Die laufende Station wurde von Hand
geheilt (`umount /Volumes/daten-1`, danach der hauseigene `ensure-nas-mounted.sh`), am Mount-Weg
selbst nichts geaendert. Der SMB-Mount fiel waehrend des Laufs zweimal erneut aus und wurde je vom
Guard zurueckgeholt — deshalb liefen alle Messungen und alle Schreibvorgaenge dieses Laufs **nativ
per ssh auf der Synology** statt ueber SMB.

**Delta beidseitig gemessen**, weil NAS und GitHub seit dem 29.08. auseinanderlaufen: im NAS-Repo
179 Commits im Fenster (HEAD `a9083705d`, 17:15, im Fenster — Gegenprobe bestanden), auf
`origin/main` 82 (Kopf `66df04125`, 31.08. 06:12). **Strukturelles Delta auf beiden Seiten leer:**
`--diff-filter=A` ueber `skills/ agents/ rules/ connectors/ services/ commands/` liefert
**keine neue Datei**, weder auf dem NAS noch auf `origin/main`. Geaendert wurden **zwei Rules**,
beide auf der NAS-Seite: `rules/sync-kanonische-quelle.md` (Commit `8d93a8431` — Eigenrauschen von
Lauf 23) und `rules/jans-dna-facetten.md` (Commit `92545f372`). Kein neuer Skill, kein neuer Agent,
kein neuer Connector, kein neuer Service, kein neues Command, keine neue KB. Damit **keine
Contract-Pflichtfelder, keine Rollen-Zeile, keine Allowlist-Freigabe und kein @-Import zu pruefen**;
der Frueh-Ausstieg nach Schritt 2 greift wegen der zwei Rule-Aenderungen nicht.

**Geprueft und ausdruecklich NICHT als Befund uebernommen.** (a) Die Aenderung an
`rules/jans-dna-facetten.md` entfernt in Z. 225 die Klausel «Gruss nach Mail-Typ; Stufe nur
Richtwert; Du gross» und fuegt in Z. 213 eine neue Klausel hinzu — `numstat` 2/2, also ein
Substanzverlust, den keine Zeilenzaehlung sichtbar macht (die Schreib-Kontrolle nach Rule
`auto-verbesserungen` 260811 misst Zeilen). Die Gegenprobe entlastet den Lauf trotzdem: die
Streichung ist begruendet und belegt dokumentiert (`wissen/twin/outputs/2026-08-31_fidelity.md`
Z. 177-178, `wissen/twin/CHANGELOG.md` Z. 26), und «Gruss nach Mail-Typ» steht weiterhin im immer
aktiven Kern `rules/jans-dna.md` Z. 52. Dokumentierte Entdopplung, kein stiller Verlust — kein
Befund, aber der Merksatz bleibt: eine 2/2-Zeile kann Substanz kosten. (b) Die Sync-Divergenz
selbst (471 voraus / 267 zurueck, Zaehler 555) ist wie in Lauf 23 **kein eigener Synergie-Befund**,
sondern in `logbuch/fristen.md` bereits siebenfach nachgetragen und mit Ursache B6 gespect; hier
zaehlt allein, dass kein Waechter sie meldet (SYN-65). (c) Die 336 Dateibewegungen in
`architektur-fachwissen` auf der GitHub-Seite sind reine KB-Inhaltsarbeit ohne neuen Baustein.

**Aufgerollt wegen Bewegung eines Beteiligten:** SYN-43 (`jans-dna-facetten.md` bewegt) — Muster
bestaetigt, Status nachgefuehrt. **Nicht aufgerollt (Compounding, kein Beteiligter bewegt):**
SYN-01 bis SYN-63 im Uebrigen; im strukturellen Delta liegt kein einziger ihrer Bausteine.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** NAS-Seite `energie` (28), `twin` (19),
`wettbewerbs-dna` (11), `koordination` (8, Eigenrauschen Lauf 23), `planungsgrundlagen` (2),
`architektur-fachwissen` (2), `spec` (1); GitHub-Seite `architektur-fachwissen` (336).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Trefferzahl 1 assertiert, kein globales Suchen-und-Ersetzen, alle Eingriffe zeilengenau am Anker,
Umfang vorher/nachher gemessen. Register **1161 → 1163 Zeilen, 63 → 65 Tabellenzeilen**, null
geloeschte Zeilen (Diff gegen Sicherungskopie im Scratchpad); die Statusnachtraege an SYN-43 und
SYN-65 sind rein additiv an das Zeilenende gesetzt (1163 → 1163). Laufbericht:
`outputs/2026-08-31_synergie-lauf-24.md`.

Lauf 25: 01.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 31.08.2026 17:10, Fenster 24,0 h,
122 Commits im NAS-Repo, **0 auf `origin/main`**). **Uhr-Check bestanden:** Stationsuhr 17:10 CEST,
juengster nativer Synology-Zeitstempel 17:00:06 (Selfcommit-Log) und 17:00:01 (NAS-HEAD
`ae644e967`) — Abweichung unter zehn Minuten, kein Nachstellen noetig. Das Delta wurde **nativ per
`ssh`** auf der Synology gezogen, nicht per `git` ueber SMB (Rule `sync-kanonische-quelle`,
Chronik 260729).

**Strukturelles Delta beidseitig leer:** `--diff-filter=A` ueber `skills/ agents/ rules/
connectors/ services/ commands/` liefert **keine neue Datei**. Geaendert wurden **zwei Rules**,
beide auf der NAS-Seite: `rules/betrieb-chronik.md` (5 Commits, davon vier reine Anhaenge und eine
2/2-Zahlkorrektur «dreimal»→«viermal», Commit `3e6c76732`) und `rules/jans-dna-facetten.md`
(Commit `5141c74e3`, 6/5). Kein neuer Skill, Agent, Connector, Service, Command, keine neue KB —
damit keine Contract-Pflichtfelder, keine Rollen-Zeile, keine Allowlist-Freigabe und kein
@-Import zu pruefen; der Frueh-Ausstieg nach Schritt 2 greift wegen der zwei Rule-Aenderungen nicht.

**Neu:** SYN-66 (die B6-Bestandsaufnahme in `docs/konzepte/260830-Laufzeitschicht-Umbau/SPEC.md`
belegt einen Startpfad, vier pinnen das Arbeitsverzeichnis auf den SSD-Klon — `vollgas-runner.sh`,
`arbeits-weiche.sh` und `nachtschicht-run.sh` kommen dort nicht vor, `grep -c` = 0; nur gemeldet)
und SYN-67 (der stille CWD-Fallback aus Chronik 260901 fehlt in allen sechs importierten
Grundkontext-Dateien, 0 Treffer; Querverweis in `rules/sync-kanonische-quelle.md` gesetzt).

**Geprueft und ausdruecklich NICHT als Befund uebernommen.** (a) Die 5 Loeschungen in
`jans-dna-facetten.md` sind Verdichtung: gestrichen wurden ein Beispielzusatz und ein Rueckverweis
auf die Zeilenregel, die 40 Zeilen hoeher unveraendert steht — **dritter Lauf in Folge, in dem eine
Zeilenbilanz mit Loeschungen sich als harmlos erweist**; der Merksatz aus Lauf 24 bleibt gueltig,
weil die Pruefung ihn jedes Mal erst erzeugt. (b) Die Sync-Divergenz selbst (592/267, Zaehler 681)
ist wie in Lauf 23 und 24 **kein eigener Synergie-Befund**, sondern im Fristen-Register mehrfach
nachgetragen; neu ist allein, dass ihre **Ursache**-Seite jetzt als SYN-66 erfasst ist. (c) Der
transiente Mount-Abriss gegen 17:2x ist die Idle-Stall-Signatur nach Chronik 260831b, nicht der
Fremdmontage-Zustand aus SYN-64 (`/Volumes/daten-1` existiert auf dieser Station nicht); er ist
zum Selbstbeleg fuer SYN-67 geworden.

**Aufgerollt wegen Bewegung eines Beteiligten:** SYN-43 (`jans-dna-facetten.md` bewegt, Apparat-Regel
260901), SYN-64 (Chronik-Nachtraege zum Mount) und SYN-65 (Faktenlage verschaerft) — alle drei
Status nachgefuehrt. **Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-63
im Uebrigen.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `twin` (31), `energie` (17),
...
Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Trefferzahl 1 assertiert, kein globales Suchen-und-Ersetzen, alle Eingriffe zeilengenau am Anker,
Umfang gegen Sicherungskopien im Scratchpad gemessen (nicht per `git` ueber SMB). Register
**1218 → 1220 Zeilen, 65 → 67 Tabellenzeilen**, null geloeschte Zeilen; die drei Statusnachtraege
sind additiv ans Zeilenende gesetzt (Zeichenlaenge 3140→4221, 2478→3040, 3061→4059, alle gewachsen).
`rules/sync-kanonische-quelle.md` **+12/−0** (50 → 62). Laufbericht:
`outputs/2026-09-01_synergie-lauf-25.md`.

Lauf 26: 02.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 01.09.2026 17:10, Fenster 24,0 h,
133 Commits). Uhrenprobe nach Schritt 0: Stationsuhr 17:10 CEST gegen den nativen
Synology-Log `sync-tasks/log/selfcommit-202609.log` (letzter Eintrag 17:00:07, push OK
`905f2e1cf`) — deckungsgleich, keine Schieflage. Gegenprobe zur Delta-Messung: aeltester
Commit im Fenster `45f413cf2` (01.09. 17:15) schliesst lueckenlos an die Delta-Basis an,
juengster ist der HEAD selbst (`905f2e1cf`); die Messung lief nativ im SSD-Klon, dessen HEAD
mit dem NAS-Kopf uebereinstimmt (nie `git` ueber SMB). Strukturelles Delta: **0 neue**
Skills/Agenten/Connectoren/Commands/Services/Rules — `--diff-filter=A` ueber `skills/ agents/
rules/ connectors/ services/ commands/` liefert keine neue Datei. Geaendert wurden **sieben**
Struktur-Dateien: `rules/betrieb-chronik.md` (3 Commits), `rules/auto-verbesserungen.md`,
`rules/jans-dna-facetten.md`, `rules/sync-kanonische-quelle.md` (Eigenrauschen Lauf 25),
`skills/wettbewerb/SKILL.md`, `skills/kostenkontrolle/SKILL.md`, `agents/flaechen-nachweis.md`.
Der Frueh-Ausstieg nach Schritt 2 greift damit nicht.

**Neu:** SYN-68 (Check 6 des Schutzmechanik-Selbsttests erntet Scriptnamen aus den Plists;
die zwei verketteten Waechter `kontingent-waechter.sh` und der neue `claude-session-waechter.sh`
haben keine Plist und fallen aus der Drift-Pruefung — gemessen, aktuell driftfrei, also blinder
Fleck statt Defekt) und SYN-69 (der Wachstums-Riegel hat am 02.09. erstmals eine Regel statt
Beleg-Prosa gekostet; die Deckungszusage haelt auf der kompilierten Ebene nicht, Auto-Block
33'922 von 34'000 B nachgemessen).

**Ein neuer Baustein, aber kein Skill/Agent/Connector:** `scripts/claude-session-waechter.sh`
(Commit `b079d6d2b`, 171 Z., verkettet in `speicher-waechter.sh`). Verdrahtung geprueft:
Kettenaufruf vorhanden (Z. 211-221 mit SSD-Vorrang), Rule-Verankerung vorhanden
(`auto-verbesserungen` 260902, importiert), Chronik-Eintrag vorhanden (`betrieb-chronik` 260902),
Log-Ziel vorhanden. **Die Behauptung «beide Stationen» wurde nicht geglaubt, sondern gemessen:**
`ssh mini` bestaetigt `ch.jans.speicher-waechter` geladen und die Scriptkopie im dortigen
SSD-Klon vorhanden. Kein eigener launchd-Job und keine Zeile in `logbuch/rollen/rollen-map.tsv` —
**das ist kein neuer Drift**, sondern das Muster der ganzen verketteten Waechter-Familie
(`speicher-waechter.sh` und `kontingent-waechter.sh` fehlen dort ebenfalls; der Registertyp
`script` fuehrt insgesamt nur fuenf Zeilen). Als Kontext in SYN-68 vermerkt, nicht als eigener
Befund gezaehlt.

**Geprueft und ausdruecklich NICHT als Befund uebernommen.** (a) Die drei geaenderten
Bausteine stammen alle aus **einem** Commit (`6c34e2db6`, normen Run 77) und tragen denselben
`speculative`-Vermerk zu `sia-416-2003.md` — Rueckkanten-Probe: **alle** Struktur-Konsumenten des
Destillats sind gestempelt (`agents/flaechen-nachweis.md`, `skills/kostenkontrolle/SKILL.md`,
`skills/wettbewerb/SKILL.md`), der vierte Konsument ist `wissen/baurecht/wiki/nutzungsziffern.md`.
Der einzige ungestempelte Fund ausserhalb der KB `normen`
(`wissen/architektur-fachwissen/wiki/kostenkennwerte-schulanlage-leutschenbach.md`) zitiert das
Destillat nur fuer das Metadatenfeld `ersetzt` (SIA 116:1952 → SIA 416:2003), nicht fuer Werte aus
dem Figurenanhang — unerheblich, kein Befund. (b) Die Zeilenbilanz 14/15 in
`jans-dna-facetten.md` ist zu 14/14 Verdichtung nach Beleglage 260902 (der Service-Satz-Carve-out
fuer Abgebot und Antrag ans Amt ist widerlegt und wurde umformuliert, mit Querverweis). **Vierter
Lauf in Folge, in dem eine Zeilenbilanz mit Loeschungen ueberwiegend harmlos ist** — der Merksatz
aus Lauf 24 bleibt gueltig, weil erst die Pruefung das feststellt: die **eine** verbleibende
Loeschung ist diesmal echt und wurde zu SYN-69. (c) Die Sync-Divergenz bleibt wie in Lauf 23-25
kein eigener Synergie-Befund (im Fristen-Register gefuehrt, Ursachenseite als SYN-66 erfasst);
neu ist allein, dass sie in SYN-68 als Grund zaehlt, warum der Pull-Zyklus nicht als verlaesslich
gelten darf.

**Aufgerollt wegen Bewegung eines Beteiligten:** SYN-43 (`jans-dna-facetten.md` zum vierten Mal
bewegt; Status nachgefuehrt, Sachfrage unveraendert am Entscheid Raphaels). **Nicht aufgerollt
(Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-67 im Uebrigen.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (34), `twin` (18),
`normen` (15), `koordination` (10, Eigenrauschen Lauf 25), `planungsgrundlagen` (6),
`wettbewerbs-dna` (5), `baurecht` (5), `architektur-fachwissen` (5), `projekt-lessons` (2),
`kunde-bopp` (2), `immobilienbewertung` (2), `grobkosten` (2), `firmengruendung-ch` (2),
`entwurfs-referenzen` (2), `claude-code` (2), `bauprodukte` (2), `auflagebereinigung` (2),
`architekten-synobsis` (2), `spec` (1).

**Kein direkter Eingriff (active-with-flagging bewusst nur als Flag).** Beide Befunde haetten
als Kleinigkeit gesetzt werden koennen — SYN-68 in `scripts/schutzmechanik-selbsttest.sh`,
SYN-69 in `rules/jans-dna-facetten.md`. Beides ist unterlassen: ein Schutzmechanik-Script faellt
unter die Klassen «Systemdienste/Persistenz» und gehoert nicht in einen unbeaufsichtigten
Aufsichtslauf (Rule `interaktive-eingriffe`), und die kompilierte Facetten-Datei traegt die
Kopfzeile «Nicht von Hand editieren» (Quelle sind die Wiki-Artikel). Gesetzt wurde stattdessen
ein grep-faehiges Flag in `wissen/twin/wiki/QUESTIONS.md` (Cross-KB-Bringschuld nach Rule
`wissens-bibliothekar`).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Zeilennummer UND Praefix assertiert, kein globales Suchen-und-Ersetzen, Sicherungskopie im
Scratchpad, Umfang vorher/nachher gemessen (nicht per `git` ueber SMB). Register **1269 → 1271
Zeilen, 67 → 69 Tabellenzeilen**, null geloeschte Zeilen (Diff-Gegenprobe gegen die
Sicherungskopie: 0 entfernte Zeilen); der SYN-43-Nachtrag ist additiv ans Zeilenende gesetzt
(Zeichenlaenge 4220 → 4735). `wissen/twin/wiki/QUESTIONS.md` rein anhaengend. Laufbericht:
`outputs/2026-09-02_synergie-lauf-26.md`.

Lauf 27: 03.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 02.09.2026 17:10, Fenster 23,8 h,
**112 Commits**). Uhr-Check bestanden: Stationsuhr 17:10 CEST gegen den nativen
Synology-Selfcommit-Log 17:00:05, Abweichung im Minutenbereich. Gegenprobe bestanden: aeltester
Commit im Fenster `8b3ed1897` (02.09. 17:15) schliesst lueckenlos an die Delta-Basis an (der
Commit unmittelbar davor ist `905f2e1cf`, 02.09. 17:00), juengster ist der HEAD selbst
(`a2767315e`). Strukturelles Delta: **0 neue** Skills/Agenten/Connectoren/Commands/Services/
Rules, **3 geaenderte Rules** — `rules/anrede-kontakte.md` (1/1) und `rules/jans-dna-facetten.md`
(13/13) aus `cc4367efe` (twin-fidelity-review 03.09.), `rules/auto-verbesserungen.md` (15/0) aus
`48adc3c0f` (vollgas-radar 03.09.). Der Frueh-Ausstieg griff daher nicht.

**Neu: SYN-70** (Wege-Register kennt kein Wiedervorlage-Kriterium fuer zustandsabhaengige
Sackgassen, obwohl die neue Lehre 260807 es ausdruecklich als «gleiche Familie» nennt) und
**SYN-71** (die importierte Gewichts-Klausel in `anrede-kontakte.md` traegt seit heute die
Widerlegung ihrer eigenen Lesart in der Kontakttabelle, waehrend der Klauseltext unveraendert
blieb). Beide Befunde stammen aus demselben Muster und aus den beiden geaenderten importierten
Rules — der Tagestakt hat hier genau das geleistet, wofuer er eingerichtet wurde: **beide Aenderungen
sind fuer sich genommen Verbesserungen, und beide erzeugen ihre Luecke erst im Verhaeltnis zu einem
Baustein, den ihr eigener Lauf nicht angefasst hat.**

**Zeilenbilanzen geprueft, beide harmlos** — **fuenfter Lauf in Folge**, in dem eine Bilanz mit
Loeschungen sich ueberwiegend als harmlos erweist; der Merksatz aus Lauf 24 bleibt gueltig, weil
erst die Pruefung das feststellt. (a) `anrede-kontakte.md` 1/1 ist eine Zeilenersetzung: die
Hofer-Zeile wuchs um den ⚠-Nachtrag «Das Du gilt erst ab Februar 2026», rein erweiternd; die
Rueckkante ist gesetzt (`wissen/twin/wiki/QUESTIONS.md` 260903b existiert). (b)
`jans-dna-facetten.md` 13/13 sind durchweg **Verdichtungen und Praezisierungen**, keine
Substanzverluste: die Service-Satz-Linie wurde um «Statusmail MIT Entscheid» erweitert und um die
Linie «offen, wo der EMPFAENGER entscheidet» ergaenzt, die Gruss-Default-Aussage von drei auf zwei
Zeilen verdichtet ohne Aussageverlust, und die Privatbauherrinnen-Zeile ersetzt — Letztere ist
kein Verlust, sondern die Widerlegung, die zu SYN-71 wurde.

**Aufgerollt wegen Bewegung eines Beteiligten:** keine. **Nicht aufgerollt (Compounding, kein
Beteiligter bewegt):** SYN-01 bis SYN-69.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `twin` (21), `energie` (18),
`normen` (8), `koordination` (6, Eigenrauschen Lauf 26), `wettbewerbs-dna` (3),
`architektur-fachwissen` (3), `spec` (1), `baurecht` (1).

**Selbstbeleg zu SYN-67 (Lauf 25), aus diesem Lauf.** Der Schreibvorgang wurde mit **relativen**
Pfaden formuliert, waehrend das Arbeitsverzeichnis auf dem NAS stand — genau die Konstellation,
deren Ausfallmodus SYN-67 beschreibt. Sie ging gut aus (der Mount hielt, das CWD war
`/Volumes/daten/jans-ai-hub`), und das ist der Punkt: **nachgemessen wurde absolut**, auf beiden
Seiten. NAS 1035/133 Zeilen, SSD-Klon unveraendert 1010/117 und `git status --porcelain` leer.
Ohne diese Gegenprobe waere ein Fehlschlag nicht sichtbar gewesen — der Ausfallmodus ist still.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Zeilennummer UND Textinhalt assertiert (Python-`assert`, kein globales Suchen-und-Ersetzen),
Sicherungskopien im Scratchpad, Umfang vorher/nachher gemessen (nicht per `git` ueber SMB).
Register **1350 → 1352 Zeilen, 69 → 71 Tabellenzeilen**; `connectors/WEGE.md` **1010 → 1035**,
`rules/anrede-kontakte.md` **117 → 133** — Diff-Gegenprobe gegen die Sicherungskopien: **0
entfernte Zeilen** in beiden. Laufbericht: `outputs/2026-09-03_synergie-lauf-27.md`.


Lauf 28: 04.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 03.09.2026 17:10, Fenster 23,9 h,
**119 Commits**). Uhr-Check bestanden: Stationsuhr 17:10 CEST gegen den nativen
Synology-Selfcommit-Log 17:00:06, Abweichung im Minutenbereich. Gegenprobe bestanden: aeltester
Commit im Fenster `a022a52d0` (03.09. 17:15) schliesst lueckenlos an die Delta-Basis an (der
Commit unmittelbar davor ist `a2767315e`, 03.09. 17:00 — der HEAD von Lauf 27), juengster ist der
HEAD selbst (`dde7fa010`). Gemessen wurde im SSD-Klon gegen `origin/main`, **nicht per `git` ueber
den SMB-Mount** (Rule `sync-kanonische-quelle`); der Klon stand auf demselben Commit wie der
letzte native Push. Strukturelles Delta: **0 neue** Skills/Agenten/Connectoren/Commands/Services/
Rules, **6 geaenderte** Contract-relevante Dateien — `skills/grobkosten-onepager/SKILL.md` (11/0),
`agents/grobkosten-rechner.md` (7/2), `skills/machbarkeit-studio/SKILL.md` (6/0) alle drei aus
`3e6f1cc76` (Wissens-Chef Lauf 52, 03.09. 23:30), `rules/jans-dna-facetten.md` (11/12) aus
`d93bd95f1`, `rules/betrieb-chronik.md` (30/0) aus `6385350d5`, `rules/anrede-kontakte.md` (16/0)
aus `530673c0c` — Letzteres ist das **Eigenrauschen von Lauf 27** und wurde nicht aufgerollt. Der
Frueh-Ausstieg griff daher nicht.

**Neu: SYN-72.** Der Lauf hat genau das geprueft, wofuer der Tagestakt eingerichtet wurde — die
**Verdrahtung** eines Vorgangs, der drei Bausteine am selben Tag angefasst hat. Die drei
Aenderungen sind fuer sich genommen richtig; der Befund entsteht erst im Verhaeltnis zu den
**Bezugsgroessen**, die am Ende der Kette tatsaechlich gerechnet werden. **Methodisch das Muster
der Laeufe 26/27, ein drittes Mal:** eine Verbesserung erzeugt ihre Luecke an einem Baustein, den
ihr eigener Lauf nicht angefasst hat.

**Aufgerollt wegen Bewegung eines Beteiligten: SYN-01** (halb geschlossen seit Lauf 03). Die
offene Haelfte — `agents/realwert-rechner.md` und `agents/wirtschaftlichkeit-rechner.md` haengen
nicht an der Kennwerte-Linie — ist in diesem Lauf nachgemessen (**weiterhin 0/0**) und traegt
erstmals eine **konkrete Sachfolge**: der Aussenflaechen-Vorbehalt erreichte den Wissensartikel
des Bewertungsagenten, aber nicht den Agenten. Nachtrag an SYN-01 gesetzt, bewusst ohne
Reparatur.

**Zeilenbilanz mit Loeschungen geprueft, harmlos — sechster Lauf in Folge.** (a)
`agents/grobkosten-rechner.md` 7/2: die zwei Loeschungen sind die Umnummerierung der Schritte
4.→5. und 5.→6. beim Einschub des neuen Schritts 4; kein Substanzverlust. (b)
`rules/jans-dna-facetten.md` 11/12 ist **zweigeteilt und beides ein Fortschritt**: der
Service-Satz-Block ersetzt eine als trennscharf behauptete Gattungsregel durch die ehrliche
Feststellung, dass **drei Linien gefallen sind** (Gattung, «wer erbittet», «wer entscheidet»),
und legt den Widerspruch offen, dass dieselbe Statusmail-Gattung ihn einmal traegt und einmal
nicht — eine widerlegte Regel weicht einer belegten Bestbeschreibung (7 von 8). Die zweite
Loeschung verdichtet die Honorar-Zeile im Denken-Block auf einen `[[haltung]]`-Verweis; **die
Rueckkante ist gesetzt und wurde am Objekt geprueft** — die Substanz («direkt und ohne Polster»,
«nie weichspuelen/kleinreden») steht in derselben Rule Z. 129 und Z. 143 sowie vierfach in
`wissen/twin/wiki/haltung.md`. Es war eine Doppelfuehrung, die jetzt einmal mit Zeiger gefuehrt
wird. Der Merksatz aus Lauf 24 bleibt gueltig, **weil erst die Pruefung das feststellt**.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-02 bis SYN-71 mit Ausnahme von
SYN-01.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `normen` (20), `twin` (19),
`energie` (13), `planungsgrundlagen` (9), `koordination` (6, Eigenrauschen Lauf 27), `baurecht`
(6), `bauprodukte` (5), `immobilienbewertung` (3), `grobkosten` (2), `kunde-bopp` (2),
`architektur-fachwissen` (2). Die drei KBs des Befunds (`normen`, `grobkosten`,
`immobilienbewertung`) sind ueber SYN-72 erfasst, nicht ueber ihre Inhaltsarbeit.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Zeilennummer UND Textinhalt assertiert (Python-`assert`, kein globales Suchen-und-Ersetzen),
Sicherungskopien im Scratchpad, alle Pfade **absolut** (SYN-67), Umfang vorher/nachher gemessen
(nicht per `git` ueber SMB).

Lauf 29: 07.09.2026 08:43-08:5x (Tagestakt, Delta-Basis 04.09.2026 17:10, Fenster **63,3 h**,
**271 Commits**). Das Fenster ist dreifach ueberlang, weil die Laeufe des 05. und 06.09. nicht
stattgefunden haben: **die Station war aus** (Boot 07.09. 07:53, belegt im vollgas-radar-Commit
`c53c79a6e`), der Mac Mini trug die Nachtarbeit allein. Uhr-Check bestanden: Stationsuhr 08:43
CEST gegen nativen Synology-Selfcommit-Log 08:30:05. Anschluss-Gegenprobe bestanden: aeltester
Commit im Fenster `1ba1533cc` (04.09. 17:15) schliesst lueckenlos an, der Commit davor ist
`dde7fa010` — der HEAD von Lauf 28. Gemessen im SSD-Klon gegen `origin/main`, **nicht** per `git`
ueber den SMB-Mount. Strukturelles Delta: **0 neue** Bausteine, **3 geaenderte** Contract-relevante
Dateien, alle aus **einem** Commit (`c53c79a6e`, Wissens-Chef Lauf 53): `offertenpruefung` (1/1),
`werkvertrag` (1/1), `unternehmerkontrolle` (12/1). Frueh-Ausstieg griff nicht.

**Neu: SYN-73, SYN-74, SYN-75 — dreimal dieselbe Frage, dreimal dieselbe Antwort.** Der Lauf hat
nur einen einzigen fremden Vorgang geprueft (Lauf 53) und dabei nicht dessen Richtigkeit gemessen,
sondern seine **Reichweite**. Alle drei Befunde sind Varianten eines Musters: eine Verbesserung
wird an den Stellen gesetzt, die gerade offen sind, statt an allen, die sie betrifft. Beim
Statuslabel fehlte der groesste Abnehmer (SYN-73), beim Sachbefund vier von fuenf namentlich
genannten (SYN-74), und die Belegketten selbst verfallen still, wo sie an Zeilennummern haengen
(SYN-75). **Methodisch das Muster der Laeufe 26/27/28, ein viertes Mal** — mit einer Verschaerfung:
diesmal lag der uebersehene Baustein nicht abseits, sondern **in derselben Datei und derselben
Minute** (`werkvertrag`, SYN-74).

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-72 mit Ausnahme der in
SYN-73/74 beteiligten Bausteine.
...
**Laufunterbrechung durch NAS-Ausfall.** Um 08:52, nach den Skill-Reparaturen und vor diesem
Registerschritt, war `diskstation918` weder ueber LAN (192.168.1.10) noch ueber Tailscale
erreichbar («offline, last seen 4m ago»), waehrend die Station im LAN blieb;
`ensure-nas-mounted.sh` gab korrekt rc 1. Register, Laufbericht und CHANGELOG wurden **nicht** vom
Lauf selbst nachgetragen, sondern erst am selben Tag um 17:1x durch **Lauf 30**, der den in
`~/jans-nachtraege/synergie-lauf-29/` hinterlegten, vollstaendig vorbereiteten Nachtrag vorfand
und einspielte; die drei Skill-Reparaturen wurden dabei am Objekt erneut verifiziert (alle drei
haben den Ausfall ueberlebt und waren committet). Laufbericht:
`outputs/2026-09-07_synergie-lauf-29.md`.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Anker vor dem Schreiben auf
Zeilennummer UND Textinhalt assertiert (Python-`assert`, kein globales Suchen-und-Ersetzen),
Sicherungskopien angelegt, Umfang vorher/nachher gemessen (nicht per `git` ueber SMB).

Lauf 30: 07.09.2026 17:10-17:4x (Tagestakt, Delta-Basis 04.09.2026 17:10, Fenster **71,8 h**,
**288 Commits**). Uhr-Check bestanden: Stationsuhr 17:10 CEST gegen nativen
Synology-Selfcommit-Log 17:00:05. Anschluss-Gegenprobe bestanden: aeltester Commit im Fenster
`1ba1533cc` (04.09. 17:15), der Commit davor ist `dde7fa010` — der HEAD von Lauf 28; HEAD dieses
Laufs `9a96588b0`. Gemessen im SSD-Klon, dessen HEAD mit dem Synology-Log identisch ist, **nicht**
per `git` ueber den SMB-Mount. Die Delta-Basis ist bewusst die von **Lauf 28** geblieben, obwohl
Lauf 29 dazwischen liegt: dieser hatte sein Fenster nie ins Register geschrieben (siehe unten),
und die Basis kommt laut Auftrag aus der Register-Fussnote. Strukturelles Delta: **0 neue** Skills,
Agenten, Rules oder Connectoren, **8 geaenderte** Contract-relevante Dateien (`werkvertrag`,
`unternehmerkontrolle`, `kostenkontrolle`, `honorarberechnung-sia102`, `ausschreibung`,
`offertenpruefung`, `normen`, `rules/betrieb-chronik.md`), dazu ein neuer Loop, sechs neue
Vorlagen und zwei neue Wissensbasis-Artikel. Frueh-Ausstieg griff nicht.

**Zuerst nachgetragen, nicht neu gefunden: Lauf 29 hatte drei IDs vergeben, ohne sie ins Register
zu schreiben.** SYN-73, SYN-74 und SYN-75 waren seit dem Morgen des 07.09. in `kostenkontrolle`
(zweimal), `werkvertrag` und `normen` zitiert — im Register standen sie nicht; die vier
Fundstellen zeigten ins Leere. Ursache belegt und nicht dem Lauf anzulasten: ein NAS-Ausfall um
08:52 (`diskstation918` weder ueber LAN noch ueber Tailscale, «last seen 4m ago») unterbrach ihn
genau zwischen den Skill-Reparaturen und dem Registerschritt. Er hatte den vollstaendigen Nachtrag
samt idempotentem Einspiel-Skript unter `~/jans-nachtraege/synergie-lauf-29/` hinterlegt; niemand
kam darauf zurueck. Dieser Lauf hat den Nachtrag geprueft, die drei Skill-Reparaturen am Objekt
verifiziert (alle drei haben den Ausfall ueberlebt und sind committet) und Register, Laufbericht
und CHANGELOG eingespielt. **Das Ausfuehren des hinterlegten Skripts hat der Auto-Mode-
Klassifikator blockiert** — der fertige Befehl waere `bash ~/jans-nachtraege/synergie-lauf-29/einspielen.sh`;
die Arbeit wurde stattdessen mit den regulaeren Datei-Werkzeugen und denselben Ankern von Hand
ausgefuehrt, Zeilenbilanz je Schritt gemessen (Register 1461 → 1464 → 1508, CHANGELOG 3372 → 3408,
je 0 entfernte Zeilen). Die Klassifikator-Schranke wurde nicht umgangen (Rule
`wege-und-vollmachten`).

**Neu: SYN-76 und SYN-77 — beide am Vorgang des Tages, beide an derselben Naht.** Das strukturelle
Delta bestand praktisch ganz aus **einem** Vorgang (Bauadministrations-Raster nach bbase.ch, Audit
und Umsetzung in fuenf Bauleitungs-Skills). Er ist in der Verdrahtung der beste, den dieser Skill
seit Laengerem gesehen hat: fuenf von fuenf Skills nennen Raster und Register, sechs von sechs
neuen Vorlagen sind in der SKILL.md verankert, Contract-Pflichtfelder vollstaendig, Rollen-Zeile
gesetzt, Urheberrecht der bbase-Extrakte ausdruecklich adressiert. Die beiden Befunde liegen
nicht im Inhalt, sondern an den beiden Raendern des Vorgangs: **oben** fehlt die Task, die drei
Register bereits als aktiv fuehren (SYN-76), **unten** deckt die Auswahlregel des Loops fuenf
Skills, waehrend das Register acht als Pruefgegenstand fuehrt — und die Rueckkanten folgen exakt
der Auswahlregel, 5 von 5 gegen 0 von 4 (SYN-77). Zusammen genommen ist das der Merksatz dieses
Laufs: **ein Vorgang wird an seinen Raendern unvollstaendig, nicht in seiner Mitte** — dort, wo er
an etwas anschliesst, das nicht mehr Teil des Arbeitsauftrags war.

**Zeilenbilanz mit Loeschungen geprueft, harmlos — siebter Lauf in Folge.** Der Vorgang loeschte
71 Zeilen in vier SKILL.md (`honorarberechnung-sia102` 26, `kostenkontrolle` 22,
`unternehmerkontrolle` 15, `werkvertrag` 6, bei 375 neuen). Am Objekt geprueft: (a) Die
groesste Einzelloeschung ist die alte SIA-102-Vollformel
`H = B × ((Z0 + Z1 × log10(B)) × q × n + a) × i × r × s` samt Variablenliste — sie ist **nicht
verloren, sondern ersetzt** durch die dreistufige Normfassung (p, T_m, H) mit Fundstellen aus dem
`established`-Destillat `sia-102-2014.md` Ziff. 7.2 bis 7.4 und einem Faktorprotokoll; eine
falsche Formel ist einer belegten gewichen. (b) «Format A» und die Schrittlisten sind durchweg
Umnummerierungen und Erweiterungen (7 Schritte → mehr, Muster 2619-HOP2 statt Platzhalter). (c)
**Eine Loeschung ist echt und wurde nicht ersetzt:** `honorarberechnung-sia102` fuehrte
`offertenpruefung` als «Nachgelagert (Gegenstueck: dort werden fremde Honorarofferten anderer
Planer geprueft)»; die neue Zeile nennt stattdessen `honorar-angebotspruefung` und
`kostenkontrolle`. Beide Kanten sind sachlich richtig, aber die alte war es auch — und
`offertenpruefung` nennt `honorarberechnung-sia102` seinerseits nirgends. **Bewusst nicht
repariert:** die Kante wieder einzuziehen waere eine Aussage darueber, ob fremde Honorarofferten
in `offertenpruefung` oder in `honorar-angebotspruefung` gehoeren — ein Zuschnitt-Entscheid, kein
Querverweis. Hier als Beobachtung festgehalten, damit der naechste Lauf an
`honorarberechnung-sia102` sie sieht. Der Merksatz aus Lauf 24 bleibt gueltig, **weil erst die
Pruefung das feststellt**.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-75.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (68), `normen` (11),
`twin` (10), `projekt-lessons` (4), `bauprodukte` (4), `planungsgrundlagen` (3), `koordination`
(3, Eigenrauschen Lauf 28), `auflagebereinigung` (3), `grobkosten` (2), `spec` (1),
`immobilienbewertung` (1). Die Zahlen decken sich mit denen von Lauf 29, weil dessen Fenster im
selben liegt.

**Nicht als eigener Befund gefuehrt, weil bereits erfasst:** der Ausfall der MacBook-Pro-Loops am
05. und 06.09.2026 (null Sessions an beiden Tagen, gegen 18 am 04.09. und 23 am 07.09.). Er ist im
Fristen-Register vom heutigen Morgen dreifach dokumentiert (Radar 07:4x, Hub-Chef 08:0x) samt
Ursache (Station aus oder im Tiefschlaf) und Sofortmassnahme (`launchctl kickstart` des
`synctask-runner`). Ein Aufsichtslauf, der einen bereits gemeldeten Befund als neu fuehrt,
verdoppelt ihn, statt ihn zu schaerfen.
...
Lauf 31: 08.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 07.09.2026 17:10, Fenster 23,8 h,
**119 Commits**). Uhr-Check bestanden: Stationsuhr 17:10 CEST gegen nativen
Synology-Selfcommit-Log 17:00:06. Anschluss-Gegenprobe bestanden: aeltester Commit im Fenster
`7230d0b65` (07.09. 17:15), der Commit davor ist `9a96588b0` — der HEAD von Lauf 30; HEAD dieses
Laufs `0764894b4`. Gemessen im SSD-Klon, dessen HEAD mit dem Synology-Log identisch ist, **nicht**
per `git` ueber den SMB-Mount. Strukturelles Delta: **0 neue** Skills, Agenten, Rules oder
Connectoren, **8 geaenderte** Contract-relevante Dateien in vier Vorgaengen. Frueh-Ausstieg griff
nicht.

**Die vier Vorgaenge und was jeder wert war.** (1) SYN-72-Auslieferung durch Wissens-Chef Lauf 53/54
(`34fe0557a`, drei Bausteine) — nachgefuehrt, kein neuer Befund, aber eine ungedeckte siebte Stelle
gemessen. (2) `offertenpruefung` Z. 16 — das ist der Nachtrag von **Lauf 30 selbst** (SYN-77),
Eigenrauschen, nicht aufgerollt. (3) `wissenscheck` + `wissen/WISSEN-CLAUDE.md` (`f2bf87873`) —
**die Rueckkante ist beidseitig und vorbildlich gesetzt**, der Befund liegt daneben und heisst
SYN-79. (4) `rules/jans-dna-facetten.md` aus dem `twin-fidelity-review` (`641664511`) — der einzige
Vorgang des Tages mit Loeschungen (9+/11-), und aus ihm kommt SYN-78.

**Neu: SYN-78 und SYN-79.** SYN-78 ist der schwerere: der Byte-Riegel der Facetten-Pflichtlektuere
ist erreicht (gemessen an HEAD **7 B** Reserve von 34'000), und die erste erzwungene Verdichtung
hat neun belegte Service-Satz-Gattungen aus dem kompilierten Do/Don't-Block in den Teil von
`stimme.md` verschoben, den `build_dna.py` nach Konstruktion nicht mitnimmt. Der Ersatz-Zeiger
«steht im Artikel» loeste in keinem der beiden Kontexte auf — in der Quelle Selbstverweis, im
Kompilat adresslos. **Das ist die Falle jeder Kompilierung: ein Satz wird aus der Sicht des
Erzeugnisses formuliert und in die Quelle geschrieben, wo er nicht traegt.** Der Zustand war am
18.08.2026 im Script selbst vorhergesagt («die naechste Runde haette Regeln geloescht statt
Doppelungen») und ist nach 21 Tagen eingetreten. Repariert wurde nur der Zeiger (a); die
Riegel-Frage (b) gehoert Raphael. SYN-79 ist die Nebenwirkung einer sonst mustergueltig
verdrahteten Konvention: `links:` in `outputs/` ist ab 08.09. ein Audit-F-Verstoss, und der Hub
fuehrt **85** solche Reports in mindestens zehn KBs, ohne dass die Klarstellung etwas ueber den
Altbestand sagt.

**Nullbefund mit Aussage — Vorgang (3).** `skills/wissenscheck/SKILL.md` nennt
`wissen/WISSEN-CLAUDE.md` als Quelle der `links:`-Definition, und die Meta-Datei nennt
«Pruefung B im Skill `wissenscheck`» zurueck. Beide Richtungen, im selben Commit, mit Anlassbeleg
(PL46-2/PL46-3). Das ist die Kante, deren Fehlen dieser Skill seit SYN-10 am haeufigsten meldet —
hier war sie da, bevor jemand danach gefragt hat. Wert des Nullbefunds: er zeigt, dass die
Rueckkanten-Disziplin im Wissens-Layer inzwischen greift; der Restbefund SYN-79 betrifft die
Reichweite der Regel, nicht ihre Verdrahtung.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-77 mit Ausnahme von
SYN-72 (Beteiligte bewegt, Status nachgefuehrt) und SYN-77 (Eigenrauschen aus Lauf 30, nur als
Vorgang identifiziert).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `twin` (28), `energie` (26),
`baurecht` (19), `normen` (11), `koordination` (9, Eigenrauschen der Laeufe 30/54),
`projekt-lessons` (6), `bauprodukte` (4), `immobilienbewertung` (2), `spec` (1). `twin`,
`immobilienbewertung` und `projekt-lessons` sind ueber SYN-78 bzw. SYN-79 erfasst, nicht ueber
ihre Inhaltsarbeit.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert und
in-place-eindeutig (kein globales Suchen-und-Ersetzen), alle Pfade **absolut** (SYN-67), Umfang
vor und nach **jedem** Schreibvorgang gemessen — `stimme.md` 5552 → 5552 (eine Zeile ersetzt),
`jans-dna-facetten.md` 391 → 391 (eine Zeile, via `build_dna.py` nachgezogen statt von Hand
editiert), Register 1594 → 1596 → mit Fussnote, CHANGELOG gewachsen; **in keiner Datei eine
entfernte Zeile ausser den zwei benannten Ersetzungen**. Nicht per `git` ueber SMB gemessen.

Lauf 32: 09.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 08.09.2026 17:10, Fenster 23,8 h,
115 Commits). Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST, letzter
selfcommit-Eintrag 17:00 — keine Abweichung. Delta nativ im SSD-Klon gemessen, **nicht** per
`git` ueber den SMB-Mount; Anschlussprobe lueckenlos (`0764894b4` 08.09. 17:00 vor dem Fenster,
`92339630e` 08.09. 17:15 als aeltester darin), HEAD `0db388bed` = tatsaechlicher HEAD.

**Strukturelles Delta: kein neuer Baustein, eine geaenderte Rule** — `rules/jans-dna-facetten.md`
in drei Commits, davon einer (`baadd6921`) Eigenrauschen aus Lauf 31. Der Frueh-Ausstieg griff
deshalb nicht, und er haette hier auch nichts gespart: die beiden fremden Commits sind genau der
Vorgang, den SYN-78 gestern als naechsten Schritt vorhergesagt hatte.

**Neu: SYN-80.** Der Befund ist nicht der Riegel selbst — der ist seit dem 02.09. als SYN-69
erfasst —, sondern dass **dieselbe Entscheidungsvorlage inzwischen an vier Stellen liegt und
keine die anderen nennt** (SYN-69, SYN-78 (b), `logbuch/fristen.md` Z. 49, `twin-fidelity-review`
zum fuenften Lauf in Folge). Die Richtungsprobe faellt in alle Richtungen negativ aus, und der
teuerste Fall ist der eigene: **SYN-78 nennt SYN-69 nicht** — der Vorgaengerlauf hat neben einem
bestehenden Riegel-Eintrag einen zweiten angelegt, ohne die Kante zu ziehen. Genau der Fehler,
den dieser Skill seit SYN-10 bei anderen am haeufigsten meldet.

**Die Folge ist gemessen, nicht befuerchtet.** Die SYN-78-Reparatur vom 08.09. 17:21 hielt
**13 Stunden**; `303c7672e` (09.09. 06:00) kuerzte den Zeiger auf denselben Wortlaut zurueck, den
SYN-78 als adresslos beanstandet hatte. Vier weitere belegte Marker wanderten im selben Zug aus der
Pflichtlektuere in den nicht geladenen Artikelteil (je 1 Treffer im Artikel, 0 im Do/Don't-Block),
«kontrollierter Vergleich» in `haltung.md` ganz gestrichen. Reserve **15 B von 34'000** (eigene
Nachrechnung mit der Extraktionslogik von `build_dna.py` Z. 67-74: stimme 7069, denken 2930,
haltung 5991, beziehungsregister 5995, fachsignatur 4978, arbeitsweise 6874). **Eine Reparatur an
einer byte-gedeckelten, taeglich neu kompilierten Datei ist nicht haltbar, solange der Druck
besteht** — das ist die Lehre, die SYN-78 noch nicht kannte.

**Der Nacht-Lauf hat nichts falsch gemacht** — er hat drei neue belegte Regeln gesetzt, die
Verdichtung offen ausgewiesen und den Mechanismus im Ruecklauf
`wissen/claude-code/outputs/2026-09-09_verdichtung-verliert-zuerst-die-bedingung.md` praeziser
beschrieben als beide Registereintraege («der Riegel selektiert gegen Bedingungen und fuer
unbedingte Imperative», Marker im Artikel 38 gegen 1 im Kompilat). Dieser Report hat **0 Treffer**
auf `SYN-`/`koordination` — er kannte die Reparatur nicht, die er einzog. Der Befund richtet sich
gegen den Zustand, nicht gegen den Lauf.

**Status nachgefuehrt:** SYN-69 (Buendelung), SYN-78 ((a) zurueckgefallen), SYN-79 (Bestand
85 → 88; **zwei** der sieben neuen `outputs/`-Reports tragen ein `links:`-Feld, beide **nach** der
Klarstellung vom 08.09. entstanden — die Konvention wirkt nicht einmal auf Neuanlagen),
`logbuch/fristen.md` Z. 49 (Frist verstrichen, Vorhersage eingetreten, Prioritaet mittel → **hoch**).

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-77 mit Ausnahme von
SYN-69 (Beteiligter bewegt).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (17), `normen` (14),
`koordination` (6, Eigenrauschen der Laeufe 31/55), `spec` (2), `planungsgrundlagen` (2),
`architekten-synobsis` (2). `twin` (24), `claude-code` (4) und `projekt-lessons` (2) sind ueber
SYN-80 bzw. SYN-79 erfasst, nicht ueber ihre Inhaltsarbeit.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert und
in-place-eindeutig (`assert count==1` vor jedem Schreibvorgang, kein globales Suchen-und-Ersetzen),
alle Pfade **absolut** bzw. relativ zum abgesicherten NAS-Arbeitsverzeichnis (SYN-67), Umfang vor
und nach **jedem** Schreibvorgang gemessen — Register 1653 → 1654 (eine Zeile SYN-80, danach drei
in-place-Nachtraege ohne Zeilenaenderung), `fristen.md` 5406 → 5406, Ruecklauf-Report 58 → 62
(reiner Anhang); **in keiner Datei eine entfernte Zeile.** Nicht per `git` ueber SMB gemessen.

Lauf 33: 10.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 09.09.2026 17:10, Fenster 24,0 h,
115 Commits). Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST, letzter
selfcommit-Eintrag 17:00:08 — keine Abweichung. Delta nativ im SSD-Klon gemessen, **nicht** per
`git` ueber den SMB-Mount; Anschlussprobe lueckenlos (`0db388bed` 09.09. 17:00 vor dem Fenster,
`73356adc0` 09.09. 17:15 als aeltester darin), HEAD `af5b903dc` = tatsaechlicher HEAD.

**Strukturelles Delta: kein neuer Baustein, vier geaenderte Bausteine** — `rules/jans-dna-facetten.md`
(`efbaafac5`, 18/20, twin-fidelity-review), `rules/anrede-kontakte.md` (`1081dfdd7`, 16/0, Wissens-Chef
Lauf 56), `CLAUDE.md` (`1081dfdd7`, 1/1, WC56-1), `connectors/WEGE.md` (`e397d8950`, 17/0). Dazu ein
geteiltes Werkzeug ausserhalb des Filters: `wissen/tools/datenstand-waechter.py` (`8b0a2b9d6`, 15/0).

**Neu: SYN-81 und SYN-82**, beide gefunden, weil ein bewegter Beteiligter (`datenstand-waechter.py`)
zum Skill fuehrte, der ihn kennen muesste. SYN-82: `wissenscheck` kennt keines der sieben Werkzeuge unter
`wissen/tools/`, und sein Audit E misst Alter mit dem Mass, das der Alterswaechter als ungeeignet
belegt; beidseitige Querverweise gesetzt, Sachfrage offen. SYN-81: der Prompt desselben Monats-Tasks
verlangt in Schritt 4 `git add/commit/push` im NAS-Repo ueber den SMB-Mount, ebenso `immobewertung-training`;
am 01.09. hat die Rule den Prompt uebersteuert, Schaden null. Nicht gesetzt (ausfuehrender Baustein),
Frist vor dem 01.10. in `logbuch/fristen.md`.

**Status nachgefuehrt:** SYN-80 (die Buendelung erreicht die twin-Seite, der Fidelity-Report nennt
SYN-69 und uebernimmt (c); erster belegter Schadensfall im Blindtest; zwei Satzbau-Artefakte der
Verdichtung in der Quelle repariert, Reserve 16 → 15 B; Bedingungsverlust an der SYN-43-Stelle
gegengeprueft und widerlegt), SYN-43 (erster Gegenfluss twin → importierte Rule), `logbuch/fristen.md`
Z. 51 (Nachtrag Schadensfall).

**Nullbefunde mit Aussage.** (1) `CLAUDE.md` WC56-1: die Stand-Angabe «480 Wiki-Artikel» steht jetzt
**mit Messdatum**. Gegen `auto-verbesserungen` 260830 Ziff. 3 geprueft: eine datierte Momentaufnahme
ist keine Falschaussage, nur eine alte — die undatierte Vorfassung war es (16 Tage falsch). Kein
Eintrag. (2) `connectors/WEGE.md` IPTV-Nachtrag: privater Anwendungsfall, der Rueckverweis auf die Zeile
«IPTV-Sender starten» (Z. 63) loest auf, `scripts/iptv-epg.py` liegt vor; kein Baustein im Sinn der
Rollen-Taxonomie, kein Connector-Index-Eintrag noetig. (3) Der Stadelmann-Vermerk in
`rules/anrede-kontakte.md` widerspricht dem Kompilat nicht: das Trajektorie-Beispiel «(Furrer,
Stadelmann)» und «Geschätzter Othmar» im Du (Z. 242) passen zur neuen Lesart.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-79 mit Ausnahme von
SYN-43 (Beteiligter bewegt).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (20), `normen` (10),
`koordination` (6, Eigenrauschen Lauf 32 / Wissens-Chef 56), `planungsgrundlagen` (5), `claude-code` (5),
`spec` (1), `baurecht` (1), `auflagebereinigung` (1). `twin` (23) ist ueber SYN-80 erfasst.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert (`assert count==1`
vor jedem Schreibvorgang, kein globales Suchen-und-Ersetzen), alle Pfade relativ zum mit `|| exit 1`
abgesicherten NAS-Arbeitsverzeichnis (SYN-67), Umfang vor und nach **jedem** Schreibvorgang gemessen —
`wissen/twin/wiki/stimme.md` 5782 → 5782, `arbeitsweise.md` 4655 → 4655, `rules/jans-dna-facetten.md`
390 → 390 (via `build_dna.py`, nicht von Hand), `skills/wissenscheck/SKILL.md` 173 → 179,
`wissen/tools/README.md` 262 → 271 (Anhang), Register 1711 → 1711 (drei in-place-Nachtraege) → 1713
...
(`af5b903dc` 10.09. 17:00 vor dem Fenster, `455862803` 10.09. 17:15 als aeltester darin), HEAD
`de35443d2` = tatsaechlicher HEAD laut selfcommit-Log.

**Strukturelles Delta: kein neuer Skill, Agent, Connector, Command oder keine neue Rule;
geaendert** `skills/logbuch/SKILL.md` (`1917f4cef`, 6/0, Wissens-Chef 57), `rules/jans-dna-facetten.md`
(`9f3371519`, 16/19, twin-fidelity-review), `CLAUDE.md` (`d01cedb27`, 7/4), `connectors/WEGE.md`
(4 Commits, 64/0), `connectors/hub-setup-daten.json` (`9fc5e7c63`, 2/1), `rules/betrieb-chronik.md`
(8 Commits, rein additiv), ausserhalb des Filters `logbuch/rollen/rollen-map.tsv` (+24, 16 Schub-Lanes),
`scripts/rollen-bilanz.sh` (71/21), `scripts/vorgaenge-bilanz.sh` (28/5), `wissen/tools/wiki-konsistenz.sh`
(27/0) und **vier neue Mini-Task-Spiegel** unter `templates/scheduled-tasks/mac-mini/` (`8408bbb4e`, V0+).

**Neu: SYN-83** (Prio 3): `connectors/WEGE.md` fuehrt als Ursache eines Haengers von `m365-graph.mjs`
das fehlende Zertifikat; der Code bricht in diesem Fall sofort ab (Gegenprobe 0 s), die `fetch`-Aufrufe
haben keinen Zeitdeckel. Unterpunkt im WEGE-Nachtrag gesetzt, Werkzeugfix offen.

**Status nachgefuehrt:** **SYN-76 in der Hauptsache widerlegt** — `bauleitung-training` liegt seit
07.09. 16:04 in der Registry des **Mac Mini**; Lauf 30 hatte nur das MacBook gemessen. Die Aktion
Raphael «Task anlegen» im Fristen-Register haette einen Doppellauf erzeugt; berichtigt in
`logbuch/fristen.md` und `rules/betrieb-chronik.md` 260907, Rest-Befund (Lauf 08.09. ohne Spur)
mit Gegenmessung 14.09. **SYN-81 verbreitert** — die «0 auf dem Mini» waren eine Aussage ueber das
Suchmuster; breit gemessen tragen 6 von 12 Mini-Prompts eine git-Anweisung ohne `nas-commit-now`,
relevant vor der zweiten V0+-Welle. **SYN-80** — (d) Satzbau-Check im Review uebernommen, der
SYN-78-Zeiger ist nach Reparatur und Rueckkuerzung nun gestrichen, zweite Schadensmessung an
ungesehenen Golds, siebter Lauf mit Empfehlung 36'000 B.

**Nullbefunde mit Aussage.** (1) `skills/logbuch` «Person und Sache»: erreicht den `logbuch-radar`
ueber dessen Skill-Verweis (Prompt Z. 6); die engere Thread-Paarung im Prompt Z. 26 ist die untere
Ebene, keine Gegenregel; `hub-chef` erbt das Urteil ueber `fristen.md`. (2) `vorgaenge-bilanz.sh`
(«Ball bei JANS» nicht mehr als extern) und die Logbuch-Regel treffen dieselbe Fehlerklasse von zwei
Seiten, Skript und Suchmethode — ergaenzend, nicht redundant. (3) `CLAUDE.md` berichtigt den Vermerk
«Selbsttest hing nie im heartbeat»; kein Registereintrag und keine andere Datei traegt die alte
Aussage (`grep` 0). (4) `rollen-bilanz.sh` misst jetzt aus dem Lauf-Journal und weist die
Messgrenze (App-Tasks nicht erfasst) selbst aus (`logbuch/rollen/260911-rollen-bilanz.md` Z. 149).
(5) `wiki-konsistenz.sh`: die drei neuen Ausnahmen sind im Script-Kopf begruendet; SYN-82 (a) bleibt
gueltig. (6) Die drei neuen Mini-Aufsichts-Tasks tragen den sicheren Commit-Weg bzw. keinen;
`vollgas-chef-radar` Z. 61 woertlich nach Rule 260726.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-82 mit Ausnahme von
SYN-76, SYN-80, SYN-81 (Beteiligte bewegt bzw. Messung widerlegt).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `twin` (26, ueber SYN-80 erfasst),
`energie` (24), `normen` (17), `projekt-lessons` (7), `planungsgrundlagen` (6), `koordination` (6,
Eigenrauschen), `claude-code` (3), je 2 `kunde-bopp`, `grobkosten`, `baurecht`, `auflagebereinigung`,
`architektur-fachwissen`, `tools`, 1 `spec`.

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert (`assert count==1`
bzw. genau eine Tabellenzeile je SYN-ID, kein globales Suchen-und-Ersetzen), alle Pfade absolut
(SYN-67), Umfang vor und nach jedem Schreibvorgang gemessen — Register 1763 → 1764 (SYN-83; drei
in-place-Nachtraege) → mit Fussnote, `logbuch/fristen.md` 5480 → 5480 (drei in-place-Nachtraege),
`rules/betrieb-chronik.md` 3322 → 3332 (Anhang), `connectors/WEGE.md` 1139 → 1146 (Einschub);
**in keiner Datei eine entfernte Zeile.** Ein Assert stoppte den Chronik-Schritt einmal an einer
falsch gezaehlten Soll-Zeilenzahl (+11 statt +10), bevor geschrieben wurde; danach korrekt gefahren.

Lauf 35: 14.09.2026 17:10-17:4x (Tagestakt, Delta-Basis 11.09.2026 17:10, Fenster 72 h, **299 Commits**).
Die Laeufe vom 12. und 13.09. fielen aus: die MacBook-App stand im `session_stale_relogin`-Latch (SYN-85), das
Fenster deckt die drei Tage darum in einem Zug. Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST,
letzter selfcommit-Eintrag 17:00:09 — keine Abweichung. Delta nativ im SSD-Klon gemessen, **nicht** per `git`
ueber den SMB-Mount; Anschlussprobe lueckenlos (`de35443d2` 11.09. 17:00 vor dem Fenster, `f453aaa5a` 11.09. 17:15
als aeltester darin), HEAD `9433590e9` = HEAD laut selfcommit-Log. Gegenprobe der Zahl: 298 Commits beruehren
`logbuch/kontingent`, 293 bzw. 290 die beiden `station-status`-Dateien, also der 15-Minuten-Takt ueber 72 h.

**Strukturelles Delta: kein neuer Skill, Agent, Connector, Command oder keine neue Rule; geaendert**
`rules/auto-verbesserungen.md` (`6b6e45e64`, 9/0, Eintrag 260913 Praesentationen als InDesign),
`rules/betrieb-chronik.md` (3 Commits, rein additiv), `connectors/WEGE.md` (3 Commits, 33/0), ausserhalb des Filters
neu `scripts/papierkorb-dataless-loeschen.py` (`5c61340fe`, `c4bc5ba51`).

**Neu: SYN-84** (Typ 1 · 3): die Praesentations-Regel 260913 erreicht `agents/dokument.md` nicht, und
`dokument-layout-standard` fuehrt PPTX weiter als eigenes Format; dazu der alte Widerspruch «Arial, Dunkelblau» im
Agenten; kein InDesign-Generator im Hub trotz dreier Erzeugnisse vom 14.09. (a)-(c) als Vermerke gesetzt.
**Neu: SYN-85** (Typ 1): die Mac-Mini-App stand vom 12.09. 22:37 bis 14.09. 13:57 im selben OAuth-Latch wie das
MacBook, 11 Fehlstarts, in keinem Register; die Aufsicht mass viermal nur das MacBook-Log.

**Status nachgefuehrt:** **SYN-76** Gegenmessung 14.09. gefahren: `bauleitung-training` wurde 04:26 gestartet und
scheiterte am Mini-Latch, 2 Slots, 0 Laeufe; naechste Gegenmessung 21.09. **P1 MacBook-App** im Fristen-Register als
erledigt 14.09. 13:59 nachgetragen (Beleg `main.log` Z. 23879/23880; dieser Lauf ist der erste bestaetigte Start danach).

**Nullbefunde mit Aussage.** (1) Papierkorb-Eingriff 14.09.: Rule `interaktive-eingriffe` Pkt. 4 erfuellt (Chronik
260914, Fristen-Zeile «9 Dataless-Geisterordner», WEGE-Nachtrag mit drei Sackgassen und dem Script); lokaler Speicher,
keine Ueberschneidung mit `tenant-hygiene` (M365) oder `speicher-waechter` (misst, loescht nicht). (2) Der
WEGE-Zuwachs beruehrt `m365-graph.mjs` nicht; SYN-83 bleibt unbewegt. (3) `templates/scheduled-tasks/` im Fenster
unveraendert; SYN-81 bleibt unbewegt. (4) InDesign ist auf beiden Stationen installiert (MacBook 2026, Mini
2024/2025/2026); der Speicherputz 260814 hatte nur Altversionen entfernt.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-83 mit Ausnahme von SYN-76.

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (32 Dateiaenderungen), `architektur-fachwissen`
(9), `koordination` (4, Eigenrauschen Lauf 34), `normen` (2).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert (`assert count==1`, kein globales
Suchen-und-Ersetzen), alle Pfade relativ zum mit `|| exit 1` abgesicherten NAS-Arbeitsverzeichnis (SYN-67), Umfang
vor und nach jedem Schreibvorgang gemessen — `logbuch/fristen.md` 5494 → 5497 (ERLEDIGT-Absatz) → 5497 (zwei
in-place-Nachtraege), Register 1821 → 1823 (SYN-84/85, SYN-Zeilen 83 → 85; ein in-place-Nachtrag) → mit Fussnote,
`agents/dokument.md` 79 → 89, `rules/dokument-layout-standard.md` 59 → 63, `rules/betrieb-chronik.md` 3350 → 3360;
**in keiner Datei eine entfernte Zeile.**

Lauf 36: 15.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 14.09.2026 17:10, Fenster 24 h, **113 Commits**).
Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST, letzter selfcommit-Eintrag 17:00:06, keine
Abweichung. Delta nativ im SSD-Klon gemessen (`main...origin/main` synchron, HEAD `3cdbfcc65` = HEAD laut
selfcommit-Log); Anschlussprobe: `9433590e9` 14.09. 17:00 vor dem Fenster, `e50dbd81e` 17:15 als aeltester darin.

**Strukturelles Delta: kein neuer Baustein; geaendert** `skills/tenant-hygiene/SKILL.md` (`ea61c2313`, 12/0),
`connectors/WEGE.md` (`3cdbfcc65`, 7/0), `rules/jans-dna-facetten.md` (`cfb2b7d3d`, 20/29, alles im
Auto-Block `BEGIN/END AUTO: facetten`, kompiliert vom `twin-fidelity-review`, gewollt),
`rules/anrede-kontakte.md` (`b3572721b`, 9/0, Vermerk «Konflikt hebt nicht» aus Wissens-Chef Lauf 58,
dort geprueft). `agents/dokument.md`, `rules/dokument-layout-standard.md`, `rules/betrieb-chronik.md` sind
die eigenen Schreibvorgaenge von Lauf 35.

**Neu: SYN-86** (Typ 1 · 3): die Korrektur der Versionsrichtlinie vom 14.09. steht in Report und SKILL, nicht
im Fristen-Register, das weiter die widerlegte Aktion «Tenant auf Automatisch» als Rest fuehrt; der neue
Behebungsweg nennt die Windows-only SPO-Shell, das installierte PnP hat `Set-PnPSiteVersionPolicy`.
(a)-(c) gesetzt. **Neu: SYN-87** (Typ 3): der WEGE-Nachtrag Leitungskataster Stadt Zuerich erreicht den
Wiki-Artikel nicht, auf den WEGE verweist; offene Frage in `planungsgrundlagen` gesetzt, kantonaler
GeoShop als Sackgasse gemessen.

**Status nachgefuehrt:** **SYN-85** Gegenprobe (3) bestanden (Mini `energie-training` 14.09. 22:37:54
bestaetigt, seit 14.09. 12:58 auf beiden Stationen kein Fehlstart); (b) offen.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-84 (SYN-76: naechste Gegenmessung
21.09.; SYN-84: `agents/dokument.md` und `dokument-layout-standard` seit Lauf 35 unbewegt).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `normen` (35 Dateiaenderungen), `twin` (24),
`baurecht` (24, darunter der saubere Ruecklauf `a30be5b94`, in SYN-87 als Gegenbeispiel genannt),
`energie` (23), `koordination` (7, Eigenrauschen), `auflagebereinigung` (5), `architektur-fachwissen` (5),
`planungsgrundlagen`, `immobilienbewertung`, `bauprodukte` (je 2), `spec` (1).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert (`assert count==1`, kein
globales Suchen-und-Ersetzen), NAS-Pfade absolut bzw. mit `|| exit 1` abgesichert (SYN-67), Umfang vor und nach
jedem Schreibvorgang gemessen — `logbuch/fristen.md` 5521 → 5521 (ein in-place-Nachtrag),
`skills/tenant-hygiene/SKILL.md` 85 → 99, `wissen/planungsgrundlagen/wiki/QUESTIONS.md` 2264 → 2287,
`wissen/planungsgrundlagen/CHANGELOG.md` 4106 → 4117, Register 1865 → 1867 (SYN-86/87, SYN-Zeilen 85 → 87;
ein in-place-Nachtrag) → mit Fussnote; **in keiner Datei eine entfernte Zeile.**

Lauf 37: 16.09.2026 17:10-17:4x (Tagestakt, Delta-Basis 15.09.2026 17:10, Fenster 24 h, **116 Commits**).
Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST, letzter selfcommit-Eintrag 17:00:07, keine
Abweichung. Delta nativ im SSD-Klon gegen `origin/main` gemessen (HEAD `222c38f19`); Anschlussprobe sauber:
`3cdbfcc65` 15.09. 17:00 (HEAD von Lauf 36) direkt vor `21b612ab5` 17:15 als aeltestem Commit im Fenster.

**Strukturelles Delta: kein neuer Skill, Agent, Connector, Service, Command oder Rule.** Neu im Baum nur eine
Skill-Referenz (`skills/terminplanung/referenzen/170606 Planungsplanung Ausfuehrung.pdf`, `06eb06ecf`) — sie
stand bereits im `referenzen/README.md` und wurde nur nachtraeglich committet, also kein Verdrahtungsbefund.
**Geaendert:** `rules/jans-dna-facetten.md` (`e78436e8e`, 18/29, vollstaendig im Auto-Block
`BEGIN/END AUTO: facetten` Z. 22-368, kompiliert von `skills/twin/tools/build_dna.py`, gewollt — aber
inhaltlich Ausloeser von SYN-88), `rules/betrieb-chronik.md` (`0a0c15f50` 20/0 neu; `e9525b771` und
`5cd92b900` sind die Schreibvorgaenge von Lauf 36), `skills/tenant-hygiene/SKILL.md` (`8343cf5c7` = Lauf 36
selbst, `ecf9b7a91` = der interaktive Vollzug von SYN-86 (e) um 17:31).

**Neu: SYN-88** (Typ 1): der Auto-Block fuehrt seit 16.09. «die Bitte um ein Zugestaendnis hebt nicht» als
Feinregel, waehrend die importierte `anrede-kontakte.md` an zwei Stellen «eine Bitte hebt» sagt; die
Bringschuld war im Twin-Harness erkannt (QUESTIONS 260916b #1 nennt die Ziel-Rule beim Namen) und blieb
liegen. (a) gesetzt. **Neu: SYN-89** (Typ 3): am 16.09. rissen zwei Werkzeugketten aus demselben Grund,
die DOCX-Pipeline wurde zum heartbeat-Check befoerdert, die Rosetta-Lehre blieb Chronik-Text; kein Check
misst die lokale Werkzeug-Architektur. Bewusst nicht gebaut (Script-Aenderung am Aufsichtsbaustein).

**Status nachgefuehrt:** **SYN-71** Nachtrag «Beteiligter bewegt» (dieselbe Klausel, andere Achse; als
SYN-88 gefuehrt). **SYN-86** (e) durch den interaktiven Lauf vom 15.09. 17:28 erledigt, im Eintrag bereits
verbucht. **Gegengeprueft und NICHT zum Befund erhoben:** die Rosetta-Lehre «gilt auch fuer den Mac Mini»
ist im Fristen-Register bereits per `ssh`-Gegenmessung abgedeckt (Mini hat Rosetta), der Chronik-Eintrag
erfuellt `interaktive-eingriffe` Punkt 1-5 vollstaendig.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-87 ausser SYN-71 und SYN-86
(SYN-76: naechste Gegenmessung 21.09.; SYN-85 (b) unveraendert offen, kein Fehlstart auf beiden Stationen).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `architekten-synobsis` (30 Dateiaenderungen),
`energie` (21), `twin` (19, darunter der Fidelity-Review, der SYN-88 ausloest), `normen` (12),
`planungsgrundlagen` (7), `koordination` (7, Eigenrauschen), `kunde-bopp` (4), `bauprodukte` (3),
`claude-code` und `architektur-fachwissen` (je 2), `spec` (1).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: jeder Edit anker-basiert (`assert count==1`, kein
globales Suchen-und-Ersetzen), NAS-Pfade absolut bzw. mit `|| exit 1` abgesichert (SYN-67), Umfang vor und
nach jedem Schreibvorgang gemessen — `rules/anrede-kontakte.md` 158 → 178 (Klauseltext und Kontaktliste mit
23 Tabellenzeilen unveraendert, vier Altmarker einzeln gegengeprueft), Register 1904 → 1906 (SYN-88/89,
SYN-Zeilen 87 → 89; ein in-place-Nachtrag an SYN-71) → mit Fussnote; **in keiner Datei eine entfernte Zeile.**

Lauf 38: 17.09.2026 17:10-17:5x (Tagestakt, Delta-Basis 16.09.2026 17:10, Fenster 24 h, **127 Commits**).
Uhr gegen die native Synology-Uhr geprueft: Station 17:10 CEST, letzter selfcommit-Eintrag 17:00:06, keine
Abweichung. Delta nativ im SSD-Klon gegen `origin/main` gemessen (HEAD `8a4df7cb7`, lokal und remote gleich);
Anschlussprobe sauber: aeltester Commit im Fenster `37e0065e4` 16.09. 17:15, direkt nach der Basis 17:10.

**Strukturelles Delta: ein neuer Connector** — `connectors/bexio-auth.mjs` (OpenID-Connect-Dauerzugang zu
bexio, Refresh-Token im Schluesselbund, `5e2ef8ab9`). Kein neuer Skill, Agent, Service oder Command. Neu im
Baum ausserdem die schon in Lauf 37 gesehene Skill-Referenz `skills/terminplanung/referenzen/…` (Nachtrag,
kein Verdrahtungsbefund). **Geaendert:** `rules/auto-verbesserungen.md` (3 Commits: Neufassung 260702,
neuer Eintrag 260917b, Selfcommit), `rules/betrieb-chronik.md` (4), `rules/jans-dna-facetten.md` und
`rules/anrede-kontakte.md` (je 1 — beides Nachwirkung von Lauf 37 bzw. des logbuch-radar, nicht neu).

**Verdrahtung des neuen Connectors vollstaendig geprueft und in Ordnung** (das ist bei Tagestakt der Kern):
`connectors/README.md` Z. 26-27 fuehrt beide Connectoren mit Zweck, Flags und Zugangsweg · `connectors/WEGE.md`
Z. 49 nennt OIDC als ersten Weg, PAT als Rueckfall und die Schluesselbund-Sackgasse in ssh-Sessions (rc=36) ·
Spec abgelegt (`wissen/spec/outputs/2026-09-17_bexio-oidc-dauerzugang_spec.md`) · Rueckkante beidseitig:
`bexio.mjs` Z. 46 importiert `holeOidcToken`/`oidcStatus`, der Kopf von `bexio-auth.mjs` nennt `bexio.mjs`.
**Gegengeprueft und NICHT zum Befund erhoben:** (a) fehlende Allowlist-Zeile — `.claude/settings.json` fuehrt
`Bash(*)`, eine connectorspezifische Freigabe ist dort gar nicht die Konvention; (b) fehlende Zeile in
`rollen-map.tsv` — das Register fuehrt die Typen command/loop/parkiert/referenz/script/skill/task, **keine
Connectoren**; der zugehoerige Task `bexio-buchen-monatlich` steht drin (Z. 96, aktiv) und liegt real auf dem
**Mac Mini** (per `ssh mini` gemessen, auf dieser Station fehlt er erwartungsgemaess — zwei Task-Registries).

**Neu: SYN-90** (Typ 1): die neugefasste Buchungsvollmacht 260702 zitiert A9 der `AKTIONS-WHITELIST` als
eigene Grundlage, waehrend A9 denselben Zweifelsfall gegenteilig regelt («Entwurf statt Buchung» gegen
«1090/ABKLAERUNG») und ein 60-Minuten-Widerrufsfenster traegt, das zu einem Monatslauf nicht passt; die
Whitelist wurde im Fenster nicht angefasst. Vollzug gedeckt (der Task-Prompt regelt den Fall selbst),
strukturell offen fuer jeden, der A9 als Vollmacht liest — namentlich `hub-chef`. **Neu: SYN-91** (Typ 1):
die User-Level-CLAUDE.md verbietet unter «Grenzen» woertlich «nie Buchungen ohne ausdrueckliche
Einzelfreigabe», also genau das, was 260702 seit heute erlaubt; durch die Rangklausel Z. 51 formal gedaempft.
**Beide Wortlaute sind Raphaels Vollmachts- bzw. Sicherheitstext — dieser Lauf hat keinen davon geaendert
und auch keinen Vermerk hineingeschrieben** (anders als bei SYN-88, wo es um eine Stil-Rule ging).

**Gegengeprueft und NICHT zum Befund erhoben:** (a) `skills/zahlungsabgleich/SKILL.md` Z. 11 und 72 («bucht,
reconciled und loescht NICHTS automatisch») steht **nicht** im Widerspruch zur Neufassung — diese nimmt
Reconcilen und Zahlungen ausdruecklich aus; (b) der fehlende Tailscale-Waechter auf dem Mac Mini (Rule 260917b,
Chronik 260917b) steht mit exaktem Behebungsbefehl bereits in `logbuch/fristen.md` Z. 4498-4506, also kein
neuer Befund; (c) die vier fossilen `Bash(ssh -o ConnectTimeout=10 mini '…')`-Eintraege in
`.claude/settings.local.json` (Z. 54, 60-62) neben der generischen Zeile 57 sind genau die von Eintrag 260917b
beschriebenen Abdruecke — stationslokal, nicht in Git, harmlos; als Hygiene im Laufbericht vermerkt, nicht
als Synergie-Befund gefuehrt.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-89 (SYN-76: naechste Gegenmessung
21.09.; SYN-85 (b) unveraendert offen; SYN-88 und SYN-89 gestern gesetzt, keine Bewegung im Fenster).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `architekten-synobsis` (28 Dateiaenderungen,
darunter neue Katalog-Eintraege), `twin` (21), `energie` (21, neuer Output-Report run203),
`planungsgrundlagen` (12), `koordination` (8, Eigenrauschen), `spec` (3, davon zwei neue bexio-Specs),
`claude-code` (3), `normen`, `baurecht`, `architektur-fachwissen` (je 2).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: Einfuegung anker-basiert (`assert len(anker)==1`
auf die SYN-89-Zeile, kein globales Suchen-und-Ersetzen), NAS-Pfad absolut und mit `|| exit 1` abgesichert
(SYN-67), Umfang vor und nach jedem Schreibvorgang gemessen — Register 1947 → 1949, SYN-Zeilen 89 → 91,
danach Fussnote; **in keiner Datei eine entfernte Zeile.**

Lauf 39: 18.09.2026 17:10-17:4x (Tagestakt, Delta-Basis 17.09.2026 17:10, Fenster 24 h, **118 Commits**).
Uhr gegen Synology gestellt: Station 17:10, letzter nativer Selfcommit 17:00:04 (`c18c50ae7`), keine Abweichung.
Gegenprobe: aeltester Commit im Fenster `15093e68d` 17.09. 17:15, neuester = HEAD `c18c50ae7`, plausibel.

**Strukturelles Delta:** kein neuer Skill, Agent, Connector, Service, Command und keine neue KB. Geaendert:
`skills/healthcare-wirtschaftlichkeit/SKILL.md` (BWO-Zeiger, Wissens-Chef Lauf 61), `rules/jans-dna-facetten.md`
(Auto-Block, Fidelity 56), `rules/betrieb-chronik.md` (260918, LaunchAgent `ch.jans.macbook-wecker` auf dem Mac
Mini), `connectors/WEGE.md` (5 Commits: Versionsrichtlinie app-only, ssh-Kanonform, Exchange-Kontakt zweimal,
Revit-DWG). Neu ausserhalb der Kernordner: `scripts/macbook-wecken.sh`, `scripts/macbook-wecker/`,
`templates/launchd/ch.jans.macbook-wecker.plist`.

**Geprueft und in Ordnung:** (a) BWO-Zeiger 1.25 % steht gleichlautend an allen drei Verbraucher-Stellen
(`healthcare-wirtschaftlichkeit` Z. 67, `machbarkeit/wissensbasis/02_kennwerte-kosten.md` Z. 89,
`03_wirtschaftlichkeit-residualwert.md` Z. 54); `studio_calc.py` rechnet keinen Referenzzins,
`scaffold_studie.py` Z. 133 nennt ihn ohne Wert; der fuehrende Artikel belegt die BWO-Publikation 01.09.2026
(Z. 172-176) und nennt den Skill (Z. 499). (b) `macbook-wecker`: Zeile in `rollen-map.tsv` (Z. 166, maintainer),
Rueckbau in der Chronik, Plist-Vorlage versioniert; `hub-setup-daten.json` fuehrt keine launchd-Liste, also
keine Konvention verletzt. (c) Exchange-Kontakt-Weg: kein Skill oder Agent legt Kontakte an (`grep`
«CNContactStore|Kontakt anlegen» in skills/agents/scripts: 0), keine Rueckkante noetig.

**Neu: SYN-92** (Typ 3): die Revit-DWG-Sackgasse stand nur in WEGE, obwohl WEGE den Skill `pdf2dwg` zum
kanonischen Ort der CAD-Sackgassen erklaert; Querverweise beidseitig gesetzt (active-with-flagging).
**Bewegt:** SYN-86 (Kern geschlossen, (d) offen), SYN-88 (Auto-Block zweimal nachkompiliert, neue Kanal-Achse
«Preisangebot inline bleibt Hoi», Steinmann-Zeile als Nebenbefund; `anrede-kontakte.md` nicht angefasst).

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-85, SYN-87, SYN-89 bis SYN-91
(SYN-76: naechste Gegenmessung 21.09.).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `energie` (27), `twin` (22), `normen` (11),
`koordination` (11, Eigenrauschen), `planungsgrundlagen` (8), `auflagebereinigung` (7), `immobilienbewertung`,
`grobkosten`, `baurecht` (je 2), `architektur-fachwissen` (1).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: alle Einfuegungen anker-basiert (`assert count==1`),
Statusnachtraege nur am Zeilenende angehaengt, NAS-Pfad absolut mit `|| exit 1` (SYN-67). Umfang: Register
+1 Tabellenzeile (SYN 91 → 92) plus Fussnote, `pdf2dwg/SKILL.md` 98 → 106, `WEGE.md` 1313 → 1319;
**in keiner Datei eine entfernte Zeile.**

Lauf 40: 19.09.2026 17:10-17:4x (Tagestakt, Delta-Basis 18.09.2026 17:10, Fenster 24 h, **114 Commits**).
Uhr gegen Synology gestellt: Station 17:10, letzter nativer Selfcommit 17:00:04 (`d1cccab19`), keine Abweichung.
Gegenprobe: aeltester Commit im Fenster `a2143417a` 18.09. 17:15, neuester = HEAD `d1cccab19`, plausibel.
**Messweg:** ein lesendes `git log` ueber den SMB-Mount lief in den 3-Minuten-Timeout (Rule
`sync-kanonische-quelle`, lesend haengt auch); gemessen wurde im SSD-Klon auf `origin/main` = `d1cccab19`.

**Strukturelles Delta:** kein neuer Skill, Agent, Connector, Service, Command und keine neue KB. Geaendert:
`skills/pdf2dwg/SKILL.md` (`42473400a`, MacBook-Weg; der zweite Commit ist Lauf 39 selbst),
`skills/tenant-hygiene/SKILL.md` (`ab70c7907`, Quota gemessen 1044.00 GB), `skills/studien-generator/tools/md2docx.py`
+ `templates/dokument-layout/BUILD-ANLEITUNG.md` (`d8777c85f`), `rules/jans-dna-facetten.md` (`8ce9e3bb1`, Auto-Block),
`rules/betrieb-chronik.md` (`45af6186c`, stale index.lock dritter Zyklus). `connectors/WEGE.md` nur Eigenrauschen
(Lauf 39). `skills/terminplanung/referenzen/170606 Planungsplanung Ausführung.pdf` wurde von `4dcc94b3b` entfernt
und drei Minuten spaeter von `00bbad3ec` wieder aufgenommen; die Datei liegt unveraendert (268'245 B) auf dem NAS,
kein neuer Baustein.

**Neu: SYN-93** (Typ 3): der MacBook-Weg zu DWG stand in Skill und KB, nicht im Wege-Register; Querverweis in
`WEGE.md` gesetzt (active-with-flagging). **Bewegt:** SYN-27 (Konverter auf Formstandard, Listen-Teil bewusst offen,
Zeilenangabe nachgefuehrt).

**Geprueft und in Ordnung:** (a) Tenant-Quota 1044 GB: kein anderer Skill, Command oder heartbeat-Script nennt ein
Limit (`grep` «StorageQuota|Speicherpool|tenant-hygiene» in skills/*/SKILL.md, commands/, scripts/*.sh: nur der Skill
selbst); die Werte «1'054 GB» in `logbuch/fristen.md` sind datierte Nachtraege eines Append-only-Registers, der
Nachtrag vom 17.09. fuehrt die Korrektur. (b) Facetten-Auto-Block: bewegt «wir/unser» und den Service-Satz, nicht
die Anrede-Achse; kein Widerspruch zu `jans-dna.md` («bei unverbindlicher Einschaetzung neutral»), SYN-88 nicht
beruehrt. (c) md2docx: dritter Generator-Pfad `jans_docx.py` laut BUILD-ANLEITUNG nicht von der Theme-Font-Falle
betroffen, keine Doppelrechnung neu.

**Nicht aufgerollt (Compounding, kein Beteiligter bewegt):** SYN-01 bis SYN-26, SYN-28 bis SYN-92
(SYN-76: naechste Gegenmessung 21.09.).

**Gemessen, aber nicht aufgerollt (reine KB-Inhaltsarbeit):** `architekten-synobsis` (30, Katalog-JSON),
`twin` (20), `energie` (19), `normen` (9), `planungsgrundlagen` (7, darin der SYN-93-Output), `immobilienbewertung` (7),
`koordination` (6, Eigenrauschen), `auflagebereinigung` (5), `baurecht` (4), `claude-code` (2), `grobkosten` (1).

Schreib-Kontrolle nach Rule `auto-verbesserungen` 260811: Einfuegungen anker-basiert (`assert count==1`),
SYN-27-Nachtrag nur am Zeilenende, NAS-Pfad absolut mit `|| exit 1` (SYN-67). Umfang: Register +1 Tabellenzeile
(SYN 92 → 93) plus Fussnote, `WEGE.md` 1319 → 1330; **in keiner Datei eine entfernte Zeile.**
