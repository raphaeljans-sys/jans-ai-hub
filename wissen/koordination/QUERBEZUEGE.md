...
| **Aussenflaechen im Kostenkennwert** (ob ein CHF/m3-GV-Vergleich Balkone/Terrassen/Laubengaenge erfasst) | **normen** (`destillate/sia-416-2003.md`, Kernbefund Run 78, an den Anhangfiguren gemessen): ANF und AVF haben **kein Volumen-Pendant**, sie zaehlen zum AKV mit eigenstaendiger Ziff. 6 | `grobkosten` (`wiki/kennwerte`), `immobilienbewertung` (`wiki/flaechendefinitionen-sia`, `wiki/realwert-sachwert`), `skills/grobkosten-onepager`, `skills/machbarkeit-studio`, `agents/grobkosten-rechner` tragen den Vorbehalt seit 03.09.2026 und verweisen dorthin — NEU Lauf 52. **Nicht ueberdehnen:** adversarial gemessen bleibt das Delta bei mittleren Projekten meist innerhalb der ±25-%-Bandbreite der Grobschaetzung; materiell wird es bei kleinen Projekten und bei balkonlastigen Varianten, also wenn der Aussenflaechenanteil das Vergleichsmerkmal ist |
| **Foerderregime-Kuerzel der PV-Einmalverguetung** (KLEIV/GREIV/**HEIV**) | **energie** (`destillate/pv-einmalverguetung-bund-bfe.md`, `established`) — fuehrt HEIV = **hohe** EIV (Volleinspeiser ohne Eigenverbrauch, 2 bis <150 kW, Deckel 60 %) und die alpine Grossanlagen-Foerderung nach **Art. 71a EnG** («Solarexpress») seit jeher in **getrennten** Tabellenzeilen | `planungsgrundlagen` (`wiki/energie-betriebsenergie-pv-wirtschaftlichkeit`) fuehrte HEIV bis 03.09.2026 als «**Hochalpine** EIV» — eine Verwechslung der beiden Regime, die zufaellig denselben 60-%-Deckel kennen; berichtigt Lauf 52 am Primaerbeleg (pronovo.ch, BFE-Faktenblatt pubdb 7238). **Merkzeichen fuer kuenftige Laeufe:** der Leerstrich in der Leistungsbereich-Spalte war der Hinweis — die Zeile war aus dem Kuerzel heraus geraten, nie an einer Quelle gemessen — NEU Lauf 52 |
| **Waldabstand: Mass gegenueber Linienlage** (was aendert eine laufende Ergaenzungsplan-Revision) | **geteilt, und die Trennung ist der Punkt:** das **Abstandsmass** (§ 66 PBG, 30 m, red. bis 15 m) fuehrt `baurecht`; die **Linienlage** und ihren Revisionsstand fuehrt `planungsgrundlagen` (`wiki/kartenportale-baulinien-abstandslinien-zh`, Connector-Meldung `proj wald`) | Aktueller Fall: Stadt Zuerich, Teilrevision Ergaenzungsplan Waldabstandslinien, oeffentliche Auflage seit **03.12.2025**, OEREB-`docid` 17431 — beidseitig eingetragen 03.09.2026. **Materiell, nicht bloss ein Hinweis:** nach § 234 PBG kann bereits eine **beantragte** Festlegung ein Vorhaben scheitern lassen (negative Vorwirkung). **Der Melder wollte den Vermerk an die 30-m-Zeile in `baurecht` setzen; der Verifikator hat den Zielort korrigiert** — eine Planrevision verschiebt die Linie, nicht das Mass — NEU Lauf 52 |
| **Treppenbreite: Erlass gegenueber Sicherheitsempfehlung** | **baurecht** (§ 305 Abs. 1 PBG ZH, volltextverifiziert) — allein massgebend | `normen` (`destillate/bfu-treppen-2020.md`, bfu 2.007.01:12.2020) fuehrt den **zahlengleichen** Wert (MFH 1,20 m / EFH 0,90 m) als **nachrangige, nicht rechtsverbindliche** Empfehlung; beidseitig verlinkt 03.09.2026. Drei Vorbehalte wandern zwingend mit: § 305 regelt Treppen **und Gaenge** in einer Zahl, die bfu-Zeile nur die Treppe; die **Bezugsgroesse** (lichte Breite / Laufbreite) ist an **keiner** der beiden Stellen definiert; und die Quellen sind **unabhaengig** — SIA 500 traegt in derselben bfu-Tabellenzeile «keine Angabe», die Zahlengleichheit ist damit belegt, aber **unerklaert** — NEU Lauf 52 |
