...
|---|---|---|---|---|---|---|
| `3 EASY STEPS - the Spec` | 34 Screenshots, YT Austin Marchese / Karpathy «The Spec» | `wissen/spec/` (5 Wiki-Artikel, Transkript in raw) | Rule `spec-methode` (Gate, immer aktiv) + Skill `spec` | Task `spec-training` (seit 26.07. Ereignis-Trigger bei realer Spec-Anwendung) | implementiert, **ein Befund** | **2026-09-08 geprueft, ein Befund**: (a) Implementation vollstaendig — `rules/spec-methode.md`, `skills/spec/SKILL.md`, KB `wissen/spec/` mit 5 Wiki-Artikeln (the-spec, 3-schritte-spec, verifier, anwendung-jans, environment-jans-hub) plus INDEX/QUESTIONS; (b) Registry `spec-training` unveraendert enabled=false MIT dokumentiertem Entscheid (Raphael 26.07.) = kein stiller Ausfall; der Statuskopf in `training/PROGRAMM.md` haelt den scheinbaren Widerspruch zum Abschnitt «Takt & Umfang» ausdruecklich auf (Umfang, nicht Takt) — sauber; (c) Liefer-Beleg seit der Verifikation vom 30.07. reichlich: Trainingslauf 39 (24.08., F-ENV1 geloest) sowie Health-Checks 01.08., 03.08. und 01.09., alle mit CHANGELOG-Zeile. **BEFUND — die Ereignis-Definition ist blind fuer Fortschreibungen.** Der Trigger lautet «reale Spec-Anwendung» und wird operationalisiert als juengstes **Dateinamen**-Datum unter `*_spec.md`; Run 39 schloss daraus «kein neuer Fall seit 28.07.» und fuhr als Verifikationslauf. Tatsaechlich lief die ganze Zeit der lehrreichste Fall des Hub: `outputs/2026-06-21_ag-gruendung_spec.md` traegt zwar ein Juni-Datum im Namen, ist mit 74'673 Bytes die mit Abstand groesste Spec und wurde seit dem 30.07. **17-mal** fortgeschrieben (53 Commits gesamt), zuletzt am 08.09.2026 07:48 durch den `ag-gruendung-monitor`. Run 39 erwaehnt sie mit keinem Wort (grep leer). Eine ueber elf Wochen mit dem Vorhaben mitwandernde Spec (Namensblocker, Notariat, UBS-Kapitaleinzahlung) ist fuer einen Lern-Loop mindestens so ergiebig wie eine neue — die Messung nach Dateinamen laesst genau diesen Typ durchfallen und stellt den Ereignis-Trigger faktisch still, ohne dass es jemand entschieden haette. **Kein «liefert nicht»-Befund** (der Loop liefert), sondern ein Befund an der Trigger-Definition. Vorschlag: als Ereignis zaehlt auch die **substanzielle Fortschreibung** einer bestehenden Spec, gemessen ueber `git log --since` statt ueber den Dateinamen. Entscheid bei Raphael (QUESTIONS `wissen/spec` F-TRIG1). **Pflichtpruefungen:** (1) `spec-training` laeuft als App-Scheduled-Task und schreibt nie ins Lauf-Journal — geprueftes Belegsystem war darum CHANGELOG/outputs, nicht `logbuch/laeufe/`; (2) Kontingent-Abbrueche im Fenster belegt (01.-03.08., 08.08.), erklaeren aber keine Luecke, weil danach Liefer-Belege vorliegen; (3) Verdrahtung nicht anwendbar — «auf Zuruf» ist per Entscheid manuell, ein fehlender Auslosemechanismus (grep ueber Hooks/Scripts/Rules leer) ist hier kein Mangel, sondern die beschlossene Bauform |
...
Naechste Zeile in der Rotation: **`Methode SPW`** (Stand 08.09.2026, aelteste Verifikation
unter den «implementiert»-Zeilen, zuletzt 03.08.2026). Zuletzt geprueft: `3 EASY STEPS - the Spec`
am 08.09.2026 **mit Befund** (Ereignis-Definition blind fuer Fortschreibungen), davor
`260725 Archetypen` am 13.08.2026 **mit Befund** (Ueberpruefungs-Stufe nicht verdrahtet),
`260729 Antrophic Lecture` am 13.08.2026 ohne neuen Befund und `Methode SPW` am 03.08.2026.

**Vierte Pflichtpruefung, ergaenzt 08.09.2026** aus dem Befund an `3 EASY STEPS - the Spec`: Wo eine
Zeile einen **Ereignis-Trigger** statt eines Zeittakts nennt, wird nicht nur geprueft, ob der Loop
liefert, sondern **wie das Ereignis gemessen wird**. Ein Trigger, der auf Dateinamen, Ordnernamen oder
einer anderen Namenskonvention beruht, uebersieht jede Fortschreibung eines bestehenden Artefakts und
kann den Loop stillstellen, ohne dass ein Entscheid vorliegt. Gegenprobe: `git log --since=<letzte
Verifikation> -- <pfad>` ueber die Kandidaten-Artefakte — liefert das Commits, waehrend der Loop «kein
neuer Fall» meldete, ist die Trigger-Definition der Befund, nicht der Loop. Gleiche Familie wie
`auto-verbesserungen` 260807 (ein Feld gilt ungeprueft als wirksam) und 260730b Ziff. 1b (ein Datum im
Namen ist keine Aussage ueber den Inhalt).
