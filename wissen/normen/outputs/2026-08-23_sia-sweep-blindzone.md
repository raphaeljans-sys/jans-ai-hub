# SIA-Sweep — Blindzone-Zeilen abgearbeitet (interaktive Session)

- **Datum:** 23.08.2026
- **Auftrag Raphael:** Die im `wiki/REGISTER.md`, Abschnitt A geführten SIA-Zeilen ohne Datei
  im Haus ("Blindzone") abarbeiten. Produktdatenblätter mit gültig-ab/gültig-bis beschaffen,
...
4. **QUESTIONS.md**: neuer Abschnitt mit vier neuen offenen Fragen (N-SIASWEEP-1 bis -4) für
   die Fälle ohne benannten Nachfolger; **N52-5 geschlossen** (SIA 242/274-Beschaffungsposten).
5. **CHANGELOG.md** und **REGISTER.md**-Frontmatter/Kopftabelle nachgeführt.
...
`wege-und-vollmachten.md`, Claude tätigt keine Käufe).

## Wichtigster Nebenbefund

**SIA 2016** und **SIA 2045** — zwei unabhängig recherchierte, sachlich unterschiedliche
Normen — nennen beide **SIA 405:2025** als Nachfolgenorm. Das deutet auf eine Konsolidierung
der gesamten GEO405-Familie (alte SIA 405:1998/2012 Leitungskataster, im Haus vorhanden +
SIA 2015/2016 Datenmodelle + SIA 2045 Geodienste) in einer neuen Norm hin. **Nicht am Original
verifiziert** — bei Beschaffung von SIA 405:2025 als Erstes prüfen, ob sie tatsächlich alle
drei Vorgänger ersetzt (N-SIASWEEP-4).

## Warum keine Destillate

Ein Destillat fasst Norminhalt zusammen. Für alle 65 hier bearbeiteten Zeilen liegt kein
Volltext vor — nur die freie Shop-Metadatenseite. Es gibt nichts zu destillieren; die
Fundstelle steht direkt und vollständig in der Register-Zeile. Gleiches Muster wie die
Präzedenzfälle SIA 493 (Run 25, 04.08.2026) und SIA 380/2 (Run 29, 13.08.2026): Metadaten
gehören ins Register, ein Destillat entsteht erst mit dem Volltext.

## Nicht geleistet / offene Bring-Schulden

- Kein Normtext gekauft oder gelesen (23 + 4 = 27 Positionen mit ungeklärtem oder
  überholtem Bestand bleiben Bring-Schuld Raphael).
- Die vier Fälle ohne benannten Nachfolger (SIA 381/3, 422, 2021, 2025) sind nicht
  weiterverfolgt — externe Sekundärquellen wurden bewusst nicht als Beleg verwendet.
- Kein Fach-Skill wurde auf diese 66 Zeilen abgeglichen (anders als bei den VKF-Runs 58-60,
  wo `brandschutz` aktiv zitierte Fundstellen enthielt) — soweit ersichtlich zitiert derzeit
  kein Skill diese Normen aktiv, daher kein Cross-KB-Nachzug in diesem Lauf nötig.
