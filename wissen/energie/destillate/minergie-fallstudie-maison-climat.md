---
quelle: "Fallstudie «Maison Climat» — Minergie Nachweiskurs 2023 (Teil 2); ergaenzt um das
  Objektbeschrieb-Poster «Maison Climat Seelandweg — Biel/Bienne» (Bauherrenschrieb/Praesentations-
  tafel von Buergi Schaerer Architekten AG, Bern, publiziert auf wissen.minergie.ch, Ordner
  Maison_Climat, hash-benannte Datei; vollstaendig gelesen 2026-08-21)"
herausgeber: Minergie WISSEN / Verein Minergie Schweiz; Objektbeschrieb-Poster von Buergi Schaerer
  Architekten AG, Bern
ausgabe: "25.09.2023, Version 1 (Nachweiskurs); Objektbeschrieb-Poster undatiert, Bautermine darin
  Planung 2020-2021 / Ausfuehrung 2021-2022"
gelesen: 2026-06-17 (S. 1-12, Projektbeschrieb + Aufgabe 1 Nachweis + Aufgabe 2 Optimierung;
  Beilagen/Plaene nicht im Dokument); 2026-08-21 (Objektbeschrieb-Poster, 1 S., vollstaendig)
datenstand: "⚠ Kennzahlen fuer die Uebung teilweise angepasst, nicht durchwegs reale Werte; SIA
  380/1:2016. Nachtrag 2026-08-21 aus dem Objektbeschrieb-Poster: Bauteilaufbauten und Projektfakten
  ohne diesen Vorbehalt, stammen direkt vom Architekturbuero."
status: emerging
last_updated: 2026-08-21
---

> **Querbezug KB `planungsgrundlagen` (gesetzt 21.08.2026, Wissens-Chef Run 39):** Dasselbe Projekt wird
> dort seit dem 13.07.2026 als Referenzprojekt gefuehrt —
> `wissen/planungsgrundlagen/wiki/energie-minergie-referenzprojekt-maison-climat` (status established).
> ⚠ **Die beiden Artikel nennen unterschiedliche Adressen:** dort «Seelandweg 15», hier «Seelandweg 17»
> (gemaess Objektbeschrieb-Poster). Die Abweichung ist bisher in beiden KBs getrennt offen und gehoert
> **einmal** geklaert, nicht zweimal.

# Fallstudie «Maison Climat» — gebautes Plusenergie-MFH (Minergie-A)

## Das Wichtigste in 1 Satz
Ein reales, vorbildliches **Plusenergie-Mehrfamilienhaus** (Minergie-A, Holzbau, 20 Wohnungen,
Ersatzneubau Biel 2022) zeigt am konkreten Nachweis, dass ein klimafreundliches MFH mit
**Erdsonden-WP, Bodenheizung/-kuehlung, voller Dach-PV und hoher Daemmung machbar und bezahlbar** ist.

## Das Objekt
- **«Maison Climat»**, Ersatzneubau **Seelandweg 15, 2503 Biel (BE)**, fertiggestellt **2022**
  (Objektnr. BE-120-A). Architektur **Buergi Schaerer Architekten**, Holzbau **Beer Holzbau**,
  Energie energie³. Mehr: maison-climat.ch / bsarch.ch.
- **Plusenergiegebaeude, Minergie-A zertifiziert, Holzbau** mit maximaler Daemmung und hohem
  Schallschutz — produziert **uebers Jahr mehr Energie als es verbraucht**.
- **20 Mietwohnungen**, soziale/wirtschaftliche/oekologische Aspekte vereint; Innenraumklima/Komfort
  als zentrales Anliegen der Bauherrschaft.
- **Geheizt** mit **Erdsonden-Waermepumpe + Bodenheizung**; im Sommer werden die **Boeden unter
  Raumtemperatur gekuehlt** (reversible WP, Waermeentzug ueber den Boden).

## Eckdaten (Gebaeudeeigenschaften)
| Groesse | Wert |
|---|---|
| Standard / Nachweisversion | Minergie-A / **SIA 380/1:2016** |
| Kanton / Klimastation | Bern / Bern Liebefeld |
| Hoehe ueber Meer | 437 m.ue.M |
| Energiebezugsflaeche **EBF** | **1'732 m²** |
| Geschossflaeche GF | **2'413 m²** |
| Gebaeudehuellzahl A_e/EBF | **1,23** |
| Wohnungen / belueftete Raeume | 20 / 61 |

## Nachweis-Kennzahlen (Aufgabe 1 — Ausgangszustand)
- **Heizwaermebedarf** (Systemnachweis, Standardluftwechsel): Projektwert **Q_h = 96,8 MJ/m²**,
  Grenzwert **Q_h,li = 128,3 MJ/m²** → erfuellt; **Heizlast p_h = 14,2 W/m²**; Verschattungsfaktor
  F_s = 0,58; Summe Waermebruecken l = 1'615 m; Bodenheizung, Auslegung Vorlauf 35 °C.
- **Mit effektivem Luftwechsel** (WRG-Lueftung): **Q_h = 78,9 MJ/m²·a** (deutlich tiefer dank WRG).
- **Sommerlicher Waermeschutz:** Variante 1 **nicht** erfuellbar → Nachweis nach **Variante 2/3 mit
  Klimadaten 2035** noetig.
- **Waermeerzeugung:** **Erdsonden-WP** (hoehere Effizienz als Luft-Wasser-WP), **Erdwaermesonden total
  750 m**; Fernwaerme am Standort nicht moeglich; fossil durch Minergie ausgeschlossen. WP-Typ im
  Erstnachweis mit Standardwerten, spaeter mit **WPesti** nachgerechnet (modulierendes Modell).
- **Eigenstrom (PV):** 60 % der nutzbaren Dachflaeche mit Modulen belegt (Standard-Ertrag/Eigenverbrauch).
- **Monitoring:** >1'000 m² → Pflicht; empfohlen wird ein **zertifiziertes Monitoring-Modul**
  (erfuellt die Anforderung automatisch, ermoeglicht Monitoring+-Betriebsauswertung).
- **E-Mobilitaet:** Tiefgarage 20 Stellplaetze, **alle verkabelt, 5 mit Ladestation**; Ausbaustufen
  nach SIA-Merkblatt 2060 (A Pipe / B Power to building / C1 garage / C2 parking / D ready to charge);
  Pflicht-Anteil auszuruestender Parkplaetze B 60 % · C1/C2 60 % · D 20 %.
- **THGE Erstellung — Eingangsgroessen:** GF 2'413 m²; **keine Weiterverwendung**; Baugrube Boeschung;
  **Flachfundation**; **Haustechnik nicht in die Decke eingelegt**; **kein CO₂-angereicherter Beton**.

## Optimierungshebel (Aufgabe 2 — Richtung Minergie-P)
- **Bessere Daemmung** (tieferer U-Wert): Q_h **96,8 → 86,9 MJ/m²**, p_h **14,2 → 12,1 W/m²**;
  mit eff. Luftwechsel **78,9 → 62 MJ/m²·a** — damit ruecken die strengeren Minergie-P-Anforderungen
  in Reichweite.
- **Lueftung mit WRG / Abluft-WP** konsequent mitplanen; **Kleinlueftungs-Standardberechnung** zulaessig
  fuer Wohnbauten bis 2'000 m² (Verwaltung/Schulen bis 1'000 m²).
- **Effiziente WP** (WPesti, modulierend) → bessere JAZ.
- **Warmwasser:** wenn **≥ 2/3 der Entnahmearmaturen Effizienzklasse A** (topten.ch) → Abminderung
  des WW-Bedarfs anrechenbar; Duschabwasser-WRG, effiziente WW-Produktion.
- **Elektrizitaet:** effiziente Elektrogeraete; im Zweckbau Beleuchtungssteuerung/-nachweis.
- **Eigenproduktion:** mehr Leistung / effizientere Module installieren; **Energiemanagement** einsetzen
  und **Eigenverbrauch erhoehen**.

## Bauherren-Transfer
- **Greifbarer Beweis fuer F8/Netto-Null:** ein 20-Wohnungen-MFH erreicht **Plusenergie** mit
  Standard-Bausteinen — **Holzbau, gute Daemmung, WRG-Lueftung, Erdsonden-WP mit Boden-Heizung/Kuehlung,
  volle Dach-PV** — und bleibt bezahlbar. Das ist die belegbare Antwort auf «geht klimaneutral ueberhaupt?».
- **Reihenfolge wie im Lehrsatz** (vgl. F2): zuerst Huelle/Daemmung (Q_h sinkt von 96,8 auf 86,9),
  dann WRG (auf 62), dann effiziente Technik, zuletzt PV vergroessern/Eigenverbrauch heben.
- **Sommerkuehlung «gratis» mitnehmen:** die ohnehin vorhandene Erdsonden-WP reversibel betreiben und
  ueber den Boden kuehlen — deckt den nach Minergie 2023 verschaerften Hitzeschutz und laeuft v. a. mit
  eigenem PV-Strom (vgl. Nachweiskurs, Kuehlung ≤ 12 W_el/m² pauschal).
- **E-Mobilitaet vorbereiten:** Leerrohre/Verkabelung bis zu allen Stellplaetzen ist heute Standard und
  spaeter nicht nachruestbar zu vernuenftigen Kosten.

## Nachtrag 2026-08-21 — Bauteilaufbauten und Projektfakten (Objektbeschrieb-Poster)

Das Objektbeschrieb-Poster von Buergi Schaerer Architekten AG schliesst genau die im Abschnitt
«Offene Punkte» unten vermerkte Luecke («exakte U-Werte/Aufbauten hier nicht belegt») teilweise:
Schichtaufbauten sind jetzt belegt, ein rechnerischer U-Wert steht aber weiterhin nicht drin.

**Aufbau Aussenwand** (aussen → innen): Schalung vertikal N+K 24 mm · Hinterlueftungsrost 2× 30 mm ·
Fassadenbahn · Flumroc Dissco 60 mm · Konstruktion 180 mm (Gefach gedaemmt) · Flumroc DP1 ·
OSB/3 als Luftdichtigkeitsschicht 15 mm · Installationsrost 50 mm mit Flumroc DP1 dazwischen ·
Gipsfaserplatte 12,5 mm innen. Gedaemmte Dicke der Konstruktionsebene rund 240 mm (60 + 180 mm),
zzgl. 50 mm gedaemmter Installationsebene.

**Aufbau Steildach Mansarde**: Ondapress 36 (Faserzement) mit integrierter PV · Querlattung 60 mm ·
Konterlattung 60 mm · Unterdachfolie · Flumroc Dissco 60 mm zwischen Sparren · Konstruktion 220 mm
mit Daemmung dazwischen · OSB/3 Luftdichtigkeitsschicht 15 mm · Dreischichtplatte 19 mm innen.

**Aufbau Geschossdecke** (Holz-Massivdecke): Bodenbelag 15 mm · Unterlagsboden 80 mm ·
Trittschalldaemmung PS81 20 mm + Mineralwolle 30 mm · gebundene Schuettung 85 mm · Massivholz
140-200 mm.

**Projektfakten**: Architektur Buergi Schaerer Architekten AG, Bern; Bauherrschaft privat;
Adresse gemaess Poster **Seelandweg 17**, 2503 Biel/Bienne (⚠ die Fallstudie oben nennt
Seelandweg 15 — vermutlich dasselbe Bauareal, zwei durch Brand baufaellige Mehrfamilienhaeuser
im Madretsch-Quartier wurden ersetzt; Adresse nicht abschliessend geklaert). Planung 2020-2021,
Ausfuehrung 2021-2022. Label Minergie-A. **Kosten BKP 1-9 rund Fr. 7,64 Mio.** Totalunternehmer/
oertliche Bauleitung BEER HOLZHAUS, Ostermundigen; Bauingenieur Nydegger+Finger, Bern;
Holzbauingenieur/Brandschutz BauHolz Wenger, Laengenbuehl; Energieplaner Energie hoch 3, Bern;
HLS Gruetter Gebaeudetechnik, Boenigen; Heizung/Lueftung Zurfluh Lottenbach, Luzern; Elektro
Fux & Sarbach Engineering, Bern; Landschaft bbz Landschaftsarchitekten, Bern.

**Mobilitaet**: 0,5 Parkplaetze pro Wohnung, alle mit Elektroanschluss ausgeruestet; grosser
ueberdeckter Bereich fuer 50 Velos.

**PV-Integration**: flaechenbuendig auf dem Mansardendach sowie auf Teilen der Suedfassade
integriert (dunkle Faserzementeindeckung der Mansarden und die Farbgebung der PV-Module sind
aufeinander abgestimmt). Ergaenzend: Erdwaermesonde als Waermequelle (deckt sich mit den 750 m
Erdsonden-Laenge oben), Sicker- und retensionsfaehige Boeden im Aussenraum.

**Weiterhin offen**: kein rechnerischer U-Wert der Bauteile im Poster; Zuordnung Seelandweg
15 vs. 17 ungeklaert.

## Offene Punkte / Caveats
- ⚠ Kennzahlen **fuer die Uebung teilweise angepasst** — nicht als reale Projektwerte zitieren, sondern
  als realistische Groessenordnung eines Plusenergie-MFH.
- Beilagen (Plaene, Schnitte/Grundrisse, SoWS-Ergaenzung) liegen auf der Minergie-WISSEN-Plattform, nicht
  im Dokument — exakte U-Werte/Aufbauten daher hier nicht belegt.
- Datum-Diskrepanz in der Quelle (Praeambel «2022», Projektbeschrieb «2020») — Fertigstellung ~2022.

Verwandt: `[[minergie-nachweiskurs-2023-mkz-thge]]` (Anforderungen/MKZ/THGE), `[[graue-energie-betriebsenergie]]`,
`[[clt-bauphysik-stora-enso]]` (Holzbau), `[[werkhof29-zirkulaeres-bauen-stroh-lehm]]` (zweites Praxisbeispiel).
</content>
