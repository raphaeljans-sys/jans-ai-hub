---
title: Graue Energie & THGE — das CO₂ im Bauteil
status: established
last_updated: 2026-07-12
sources: [destillate/graue-energie-betriebsenergie.md, destillate/minergie-besser-planen-bauen.md, destillate/clt-bauphysik-stora-enso.md, destillate/minergie-nachweiskurs-2023-mkz-thge.md, destillate/minergie-fallstudie-maison-climat.md, destillate/kbob-graue-treibhausgasemissionen-2025.md, destillate/muken-2025-verabschiedet.md]
links: [[INDEX]], [[minergie-standards]], [[minergie-nachweiskurs-2023-mkz-thge]], [[daemmstoffe-lambda]], [[u-werte-grenzwerte-ch]], [[holzbau-bauphysik-clt]], [[kbob-graue-treibhausgasemissionen-2025]], [[muken-2025-verabschiedet]]
---

# Graue Energie & THGE

**Graue Energie** = die kumulierte Energie (und das CO₂) für **Herstellung, Transport, Einbau,
Unterhalt und Entsorgung** der Baustoffe — alles ausser dem Betrieb. Mit besser werdender
Gebäudehülle sinkt die Betriebsenergie so stark, dass die graue Energie zum **dominierenden
Hebel** wird.

## Grössenordnung (Bezug 50 Jahre)
- Gut gedämmter **Massivbau**: graue Energie ≈ **20 %** der Betriebsenergie; Total ~**74 MJ/m²·a**.
- **Minergie**-Standard: graue Energie ≈ **30 %** der Betriebsenergie (relativ grösser, weil der
  Betrieb schrumpft).
- Gut gedämmter **Holzleichtbau**: ~**59 MJ/m²·a** — **tiefere** graue Energie als Massiv.

## Wo die graue Energie steckt
| Bauteilgruppe | Massivbau | Holzleichtbau |
|---|---|---|
| **Tragstruktur** | **49 %** | **40 %** |
| Dämmung/Fenster/Türen/Dichtungen | 17 % | 27 % |
| Haustechnik | 18 % | 20 % |
| Innenausbau | 7 % | 9 % |
| Baustelle | 9 % | 4 % |

→ Das **grösste Sparpotenzial liegt in der Tragstruktur/Bauweise**, nicht in der Dämmstärke.

**Holzbau-Pointe:** Massivholz (CLT/Brettsperrholz) **dämmt nicht besser** als Massivbau
(λ 0,12, braucht volle Zusatzdämmung) — der ökologische Vorteil liegt **allein** in der
tieferen grauen Energie der Tragstruktur und der CO₂-Bindung im Holz. Bauphysik/Details →
`[[holzbau-bauphysik-clt]]`.

## Der zentrale Merksatz
**Nicht an der Dämmung sparen, um graue Energie zu sparen.** Mehraufwand in der Hülle ist
energetisch hoch rentabel (Lehrbeispiel: 20 MJ/m²·a mehr graue Energie → 180 MJ/m²·a weniger
Gesamtenergie, ≈ 1 : 9). Quelle/Vorbehalt → `[[graue-energie-betriebsenergie]]`.

## Regulatorischer Anker: THGE-Grenzwerte je Gebäudekategorie (Minergie 2023)
Minergie begrenzt seit der Überarbeitung 09/2023 die **Treibhausgasemissionen der Erstellung
(THGE)** — die erste verbindliche Limite für graue Energie im Schweizer Label-Kontext. Der oft
zitierte Wert **11 kg CO₂-eq/m²·a gilt für Wohn-MFH**; das Produktreglement differenziert je
**Gebäudekategorie** (Basiswert für Anteil EBF/GF = 80 %), gleiche Methodik wie Minergie-ECO:

| Kat | Gebäudekategorie | THGE kg CO₂-eq/m² EBF·a |
|---|---|---|
| I Wohnen MFH | 11 | IV Schulen | 11 |
| II Wohnen EFH | 12 | III Verwaltung | 12 |
| VI Restaurant | 14 | VII/IX/X/XI/XII Versamml./Industrie/Lager/Sport/Hallenbad | 15 |
| **V Verkauf** | **17** | **VIII Spitäler** | **17** |

- **Healthcare-relevant:** Spitäler (Kat. VIII) **17 kg** — technik-/materialintensiver, daher höher
  als Wohnen. Konkretes Planungsziel im JANS-Healthcare-Geschäft.
- **Strenger:** Klimapfad-Norm **SIA 390/1** (in Vernehmlassung) verlangt für Kat. I–VI **9 kg/m²·a
  inkl. PV/Erdsonde/Solarthermie**.
- **Bauliche Reduktionshebel** (grösster → kleinster, ~18 → ~3 kg): geringe **Spannweiten** →
  Materialisierung → tieferer **Fensteranteil** → Gebäudetechnik nicht in die Decke einlegen →
  optimierte **Baugrube/Fundation, wenig/kein UG** → **Neubau ohne UG** → **Weiterverwendung** von
  bestehendem UG/Rohbau. **Kompaktheit + einfache Lastabtragung** sind der Schlüssel. Beleg/Detail →
  `[[minergie-nachweiskurs-2023-mkz-thge]]`.

(Im gesetzlichen Vollzug noch keine THGE-Pflicht; THGE wird über den Minergie-Nachweis erbracht.
**Ausblick:** MuKEn 2025 (EnDK, 29.08.2025) führt erstmals ein eigenes, an die Minergie-Methodik
angelehntes Graue-Energie-Modul (Teil G) ins kantonale Muster-Baurecht ein — sobald ein Kanton
MuKEn 2025 umsetzt, wird die THGE-Grenze vom freiwilligen Label-Extra zur gesetzlichen Auflage.
Noch nicht in Kraft, Umsetzungshorizont 2025-2030 → `[[muken-2025-verabschiedet]]`.)
Standards-Übersicht → `[[minergie-standards]]`.

## Sanieren vor Neubau
Der Erhalt von Tragstruktur und Rohbau spart genau den grössten graue-Energie-Posten (~40–49 %).
Das ist das stärkste ökobilanzielle Argument für den **Umbau** — sofern Substanz, Schadstoffe
und Nutzungstauglichkeit es zulassen (TDD → Skill `ankaufspruefung`). Die KBOB fasst das als
**«Restwert Bestandsbauten»** [kg CO₂-eq/m² EBF]: die im Bestand bereits investierte graue Energie,
die bei Erhalt nicht neu anfällt — bei Wettbewerben/Variantenvergleichen mitrechnen.

## KBOB-Faktenblatt: die 9 Reduktionshebel + die Kennzahl-Logik (Bund, 04.04.2025)
Das verbindliche KBOB-Faktenblatt **«Graue Treibhausgasemissionen bei der Erstellung von Gebäuden»**
liefert den Rahmen (Datenstand 04.04.2025) → `[[kbob-graue-treibhausgasemissionen-2025]]`:
- **Umfang GTHGE** = Module **A + B4 + C** nach SN EN 15804 / **SIA 2032** (Herstellung+Errichtung,
  Ersatz, Entsorgung) — **ohne** Betrieb/Mobilität. Messgrösse **kg CO₂-eq/m² EBF** (absolut oder /Jahr).
- **Datenbasis:** KBOB-**Ökobilanzdaten im Baubereich v8.0 (20.02.2026)** — anerkannte Werte für
  GWP/Primärenergie/UBP je Baustoff; SIA-2032-Vorprojektwerte rechnen sie auf Bauteile herunter.
- **Reduktionspotenzial bis 40 %** (MFH) durch Kombination baulicher Massnahmen. Die **9 Hebel**
  (grösster Bauherren-Effekt zürst): **1 früh Ziele setzen · 2 Suffizienz (weniger bauen, wenig UG/
  Einstellhalle) · 3 Materialeffizienz (kompakt, Leichtbau, wenig Glas) · 4 Langlebigkeit/Umbau statt
  Ersatz · 5 biogene Baustoffe (Holz/Stroh/Lehm) · 6 kreislauffähig verbauen · 7 emissionsarme
  Materialien (Beton/Zement/Stahl) · 8 Kaskadennutzung · 9 NET prüfen.** Kernaussage: **Suffizienz +
  Bestandserhalt schlagen Materialoptimierung am Neubau.**
- **Kommt rechtlich:** Pa.Iv. 20.433 → die Kantone **müssen** künftig (Art. 45 Abs. 3 lit. e EnG)
  **Grenzwerte für graue Energie** festlegen (kommende MuKEn). Heute schon im Vollzug nur über Labels:
  **Minergie-ECO** (GW1/GW2), **SNBS**, **SIA 390/1 Klimapfad** (strengste Zielwerte, inkl. Betrieb).

## Offen
- Konkrete **kg CO₂-eq/m² je Bauteilaufbau** aus KBOB-Ökobilanzdaten v8.0 / SIA-2032-Vorprojektwerten
  belegen (kostenpflichtig/Tool eco-bau; frei zugängliche Bauteil-Liste suchen) — nicht schätzen.
- Konkrete Sanieren-vs-Neubau-Schwelle (Amortisation der grauen Energie) — eigene Quelle nötig.
