---
title: Regenwasserbewirtschaftung / Versickerung — Kanton Zürich
status: established
last_updated: 2026-07-29
datenstand: 2026-06-23 (AWEL-Formular + Hinweise; Normen VSA 2019 / SN 592 000:2012 / AWEL-Richtlinie 2022)
sources: [destillate/regenwasserbewirtschaftung-versickerung-zh.md]
links: [[INDEX]], [[BAUHERREN-FAQ]], [[greenpv-fassade-pv-begruenung-hslu]], [[recht-norm-regenwasser-gewaesserraum-zh]] (KB planungsgrundlagen — Recht/Verfahren GSchG-Kaskade + Gewaesserraum-Festlegung)
---

# Regenwasserbewirtschaftung / Versickerung — Kanton Zürich

> Beim Bauen anfallendes Regenwasser ist im Kt. ZH **vor Ort zu versickern**, nicht in die
> Kanalisation abzuleiten. Eine Versickerungs**anlage** ist meist **bewilligungspflichtig**.
> Querschnitt zu Energie/Klima (Schwammstadt) — Wasserrecht selbst → Skill `baurecht`.

## Das Wichtigste
- **Grundsatz:** Regenwasser von Dach und befestigten Flächen versickert auf der Parzelle.
- **Bewilligungspflicht:** jede unterirdische Versickerung, oberirdische mit **Flächenverhältnis
  Entwässerung:Versickerung > 5:1**, oder mit **Adsorber**. **Bagatell bis 20 m²** über belebte
  Bodenschicht: keine gewässerschutzrechtliche Bewilligung.
- **Zuständig:** i.d.R. die **Gemeinde**; das **AWEL** nur in GW-Schutzzonen/-arealen und bei
  belasteten Standorten. Städte **Winterthur/Zürich**: eigene Formulare.

## Materialwahl-Falle: Metallflächen
- Unbeschichtete **Kupfer/Zink/Zinn/Blei/Messing > 50 m²** → **künstlicher Adsorber obligatorisch**.
- Fläche = alle Niederschlagskontaktflächen; **Fassade/senkrecht × 0,2**.
- Beschichtete Cu/Zn/Pb > 50 m²: Dauerhaftigkeit der Beschichtung belegen. → KBOB-Empfehlung 2001/1.

## Technische Eckwerte
- **Versickerungsversuch** (Boden-k-Wert) zwingend, Fachmann beiziehen; Bemessung nach **VSA-Richtlinie
  «Abwasserbewirtschaftung bei Regenwetter»** + **SN 592 000 «Liegenschaftsentwässerung»** (Kap. 5.7).
- Mind. **1,0 m** natürliche Filterschicht über dem HW10; **keine** Schluckbrunnen/Direkteinleitung;
  Versickerungs- vom Schmutzwasser **vollständig trennen**; Notüberlauf in Kanalisation verboten.
- Einzureichen: Entwässerungsplan, Schnitt Versickerungsanlage, hydrogeologischer Bericht (M 1:50).

## Bezug zu Energie/Klima
- **Schwammstadt / klimaangepasstes Bauen:** Versickerung + Verdunstung entlasten die Kanalisation
  bei Starkregen und kühlen — Pendant zur **Dach-/Fassadenbegrünung** (`[[greenpv-fassade-pv-begruenung-hslu]]`,
  FAQ F20). **Gründach** mindert die Abflussspitze (Retention).
- **PV-Glasflächen** und **Kühlaggregate** werden im Versickerungsgesuch eigens erfasst.

## Bezug
- Bauherren-Antwort: `[[BAUHERREN-FAQ]]` F24. Quelle/Details: `[[regenwasserbewirtschaftung-versickerung-zh]]`
  (Destillat). Wasserrecht (GSchG/GSchV) + Bewilligungsverfahren → Skill `baurecht`.

## Offen
- Bemessungsdetail (Mulden-/Rigolengrösse je l/s, k-Wert-Klassen) — AWEL-Regenwasserrechner/VSA.
- Retentions-/Gründach-Anrechnung quantifizieren; andere Kantone (SZ) abweichend.

## Siehe auch: Regenwassernutzung als Brauchwasser (ergänzt Run 119, 2026-07-29)
Anderes Thema als die Versickerungsbewilligung oben: die **Nutzung** von Regenwasser für
WC-Spülung, Waschmaschine und Garten als Trinkwasserersatz ist nach SVGW-Merkblatt TPW 2001/1
zulässig, sofern Regenwasser- und Trinkwassernetz strikt getrennt und gekennzeichnet sind und
die Anlage dem Wasserwerk gemeldet wird → `[[regenwassernutzung-brauchwasser-trinkwasserersatz-ch]]`,
FAQ F186.

**Kantonale Grundlage ZH (ergänzt Wissens-Chef Run 20, 29.07.2026).** Dieser Artikel kannte bisher
nur die Entsorgungsseite. Die **Nutzungs**seite ist im Kanton Zürich ausdrücklich gesetzlich
geregelt: **§ 97 Abs. 3 WsG** (LS 724.1, Fassung Nachtrag 133, in Kraft seit 01.06.2026) — «Die
Verwendung von Regenwasser als Brauchwasser in getrennten Systemen ist zulässig und gebührenfrei.»
Wortlaut in der führenden KB: `wissen/baurecht/raw/260730_amtlich_zh_wsg.md:1374`; für andere
Kantone nicht geprüft. Die Gebührenfreiheit betrifft nur das genutzte Regenwasservolumen:
Bezugspflicht (§ 97 Abs. 1 WsG) und Anschlusspflicht (§ 178 WsV) an die öffentliche
Wasserversorgung bleiben unberührt, ebenso die Grundgebühr; bei der Abwasser-Grundgebühr ist
sowohl der Schmutz- als auch der Regenwasseranfall zu berücksichtigen (§ 111 Abs. 2 WsV). Die
Zisterne ersetzt den Anschluss also nicht. Recht und Verfahren führt die KB `planungsgrundlagen`
(`[[recht-norm-regenwasser-gewaesserraum-zh]]`), den Erlasswortlaut die KB `baurecht`.

**Status-Hebung 2026-07-25 (Run 88):** auf `established` gehoben — Kernaussagen (Bewilligungs-
schwellen, Metallflächen-Adsorberpflicht, Filterschichttiefe) stammen direkt aus dem amtlichen
AWEL-Gesuchsformular und sind mit zwei Fachnormen (VSA-Richtlinie, SN 592 000) hinterlegt; die
verbleibenden Offen-Punkte sind Bemessungsverfeinerungen, keine Kernfakten-Unsicherheit. Dritte
Health-Check-Empfehlung (01.07./21.07./25.07.) damit umgesetzt.
