...
description: "Wissens-Destillat — erschliesst das ungenutzte Fachwissen auf dem NAS (Archiv, Bauprodukte nach BKP, Bueroprojekte, Referenzenbibliothek) und ueberfuehrt es nach der Karpathy-Spec-Methode in belegte Wiki-Artikel des Wissens-Layers. Vierstufige Pipeline mit dem idealen Modell je Phase (Inventar bash / Triage Haiku / Destillat Sonnet / Verifikation Opus). Diesen Skill verwenden wenn der Benutzer fragt: 'Wissen destillieren', 'Archiv erschliessen', 'NAS-Wissen ins Wiki', 'Bauprodukte-Wissen', 'was liegt bei mir alles brach', 'Korpus destillieren', 'Destillat-Lauf' — und als Dauerauftrag des VOLLGAS-Endlos-Runners."
---
...
- **Nicht-Ziel:** Quelldateien im Archiv veraendern, verschieben oder loeschen. Das Archiv
  ist `raw/` im Sinne der Rule `wissens-bibliothekar` — **nur lesen**.

## Das ideale Modell je Phase

Nicht ein Modell fuer alles, sondern das billigste, das die Stufe sicher traegt
(Rule 260719, «Minimum Viable Model»). Die Stufen 0–2 fahren im Treiber-Script,
Stufe 3 laeuft in **dieser** Session.
...
| 1 | Triage: relevant? welches Thema? welche Prioritaet? | **Haiku 4.5** | Klassifikation aus Pfad/Name, tausendfach, billig |
| 2 | Destillat: Quelle lesen, Artikel schreiben, verlinken | **Sonnet 5** | Fliessarbeit mit Formatvorgabe, gutes Kosten-Qualitaets-Verhaeltnis |
...
`specs/<korpus>-spec.md` — einmal, dann bindend fuer alle Folgelaeufe (Skill `spec`,
Vorlage `wissen/spec/templates/spec-vorlage.md`). Sie beantwortet:

1. **Ziel in einem Satz** — welche Frage soll dieser KB in einem Jahr beantworten koennen,
...
3. **Artikelschnitt** — ein Artikel je *Sache* (Gewerk, Produktgattung, Fehlerbild), nicht
   je Quelldatei. Ein PDF kann drei Artikel speisen, zehn PDFs koennen einen speisen.
4. **Bewertungskriterien vorab** («Sei praezise») — woran ein erstklassiger Artikel gemessen
   wird. Mindestens: jede nicht-triviale Aussage mit Fundstelle; keine Aussage ueber Preise
...
4. **Phase 3 — Kurator (diese Session, Opus).** Das Protokoll und **jeden** in diesem Lauf
   ...
   - **Beleg-Pruefung:** jede nicht-triviale Aussage gegen die im Frontmatter genannte Quelle
     halten. Nicht belegbare Aussage → entfernen oder auf `speculative` zuruecknehmen. Diese
     Pruefung ist der eigentliche Zweck der teuren Stufe — sie wird nie uebersprungen.
   - **Widerspruchs-Pruefung:** gegen bestehende Artikel desselben KB und gegen die
     ...
     und nach `QUESTIONS.md`, nie stillschweigend eine Seite ueberschreiben.
...
5. **Sichern** — genau EINMAL am Schluss:
   ...
   ```
   Keine eigenen git-Befehle ueber den SMB-Mount (Rule 260726).

## Endbedingung je Korpus

Sind alle P1- und P2-Positionen im Inventar destilliert oder begruendet uebersprungen, meldet
der Lauf **«KORPUS KOMPLETT»** im Report, setzt den Korpus in `KORPUS-QUEUE.md` auf `erledigt`
und geht zum naechsten. Ist die ganze Queue erledigt, meldet der Lauf das an die
Vollgas-Fruehwarnung und **bittet um Stilllegung** statt weiterzudrehen (Rule 260727,
Leerlauf-Waechter).

## Leitplanken

- Nichts erfinden. Eine unbelegte Aussage ist schlimmer als eine fehlende — sie vergiftet
  jeden spaeteren Skill, der den KB als Faktenbasis zieht.
