...
- **Inputs:** Gewerk/BKP, Projekt-Stammdaten (Bauherrschaft/Objekt aus `projekte/<projektnr>-*.md`), Grundlage (Spezialplaner-Offerte, Pläne, Bestandsaufnahme, Ausschreibungstext), Submittenten-Shortlist (Output von `unternehmerfindung`), Termine; bei Brandschutz-Losen die Grundlage aus `brandschutz`; Produkt-/Systemwissen zum Gewerk aus der KB `wissen/bauprodukte` (Aufbau, Kennwerte, Einsatzgrenzen, wiederkehrende Fehlerbilder) — Produktnamen erscheinen im LV nur als Beispiel, nie als Vorgabe (Rule 260626, herstellerneutrale Submission). Abgrenzung: `wissen/bauprodukte` liefert das Produkt-/Systemwissen, `wissensbasis/05_knowhow-gewerke.md` die LV-Schreibpraxis (Konditionen, Kapitelzuordnung, Ausschreibungsform). Bei bauökologischen Materialvorgaben zusätzlich die KB `wissen/energie` — `wiki/oekologische-materialvorgaben-beschaffung.md` (Mechanik: Anforderung als Hauptposition statt Variante, ecoBKP-Ausgabe benennen, LV-Position statt pauschaler Verweisung) sowie die Destillate `bauoekologische-submissionsbedingungen.md`, `ahb-devis-und-baustellenkontrolle-2008.md` und die ECO-BKP-Reihe nach BKP-Code (`ecobkp-2026-*`). ⚠ Alterskorpus: Ausgabestand vor Verwendung prüfen, übertragbar ist die Mechanik, nicht der Wortlaut. Bei Grobkosten-Positionen ohne vorliegende Richtofferte zusätzlich die KB `wissen/projekt-lessons` — `wiki/nerves-2015-kv-nebengewerke-unterschaetzt.md` (kleine technische Nebengewerke wie Elektro, Sanitär, Gipser werden bei Umbauten als runde Pauschale mitgeschätzt und lagen im belegten Fall 76 bis 322 % daneben; für diese Positionen wo möglich frühzeitig eine indikative Marktofferte einholen, statt sie erst im Devis auf den Markt treffen zu lassen). ⚠ `status: emerging` (ein Beleg) — die Mechanik ist übertragbar, der dort genannte Zuschlags-Erfahrungswert ist KEIN Kennwert. Ergänzt 18.08.2026, Synergie-Lauf 11 (SYN-35): der Artikel führt `ausschreibung` in seinem Feld `skills:`, Wissens-Chef Run 35 verankerte die Lehre aber nur in `kostenschaetzung` und `kostenkontrolle`.
...
- **Abhaengige Rules:** dokument-layout-standard, mail-formatierung, umlaute-konvention, dateinamen-konvention, bkp-2017-referenz, normen-referenz (SIA 118:1991 samt ABB-Reihe SIA 118/xxx — Fundstelle mit Norm/Ausgabe/Ziffer aus `wissen/normen/destillate/`, und vor dem Zitieren das Feld `status:` des Destillats lesen; ergaenzt 17.08.2026, Synergie-Lauf 10: der Skill ist der **vorgelagerte** Traeger der Vertragsbedingungs- und Garantietexte, die spaeter in `werkvertrag` wandern, fuehrte aber als einziger der Kette G bis I keine Norm-Fundstellenpflicht — siehe dazu den offenen Punkt zur Wissensbasis in SYN-34), identifikatoren-verifizieren (Projektnr./Adresse/Termin), auftrags-dekomposition (Mehr-Los-Versand) + Korrektur-Pflicht (jedes Texterzeugnis vor Ausgabe durch Skill `korrektur`).
