...
| `normen` | `skills/normen/SKILL.md` | **Baunormen-Router** ueber die Normenbibliothek SharePoint `PL - 02_Recht_Norm/02_Normen` (SIA/VKF/DIN/VSS/RAL, Inventar 528 Dateien): beantwortet Norm-Fragen belegt mit Fundstelle (Norm, Ausgabe, Ziffer), klaert Gueltigkeit (Register aus `ersetzte_normen.pdf`), destilliert fehlende Normen beim ersten Zugriff (Compounding). KB `wissen/normen/` (Register + Destillate, Originale bleiben SharePoint); Nacht-Loop `normen-training-nacht` (taeglich 22:00, bis Inventar komplett); Rule `normen-referenz` verpflichtet alle Fach-Skills (ausschreibung/werkvertrag/honorarberechnung-sia102/kostenschaetzung/brandschutz/...) auf diese Fundstellen |
...
| `versandplanung` | `skills/versandplanung/SKILL.md` | **PARKIERT (Entscheid Raphael 11.07.2026 — nicht proaktiv vorschlagen, keine Pendenz).** KI-Versand- und Routenplanung Onlineshop: plant den kompletten Versandtag automatisch — Bestellungen aus dem Shop (Adapter WooCommerce/Shopify/CSV), Trennung Post-Versand ↔ Pickup-Route, Post-Etiketten via DCAPI «Barcode», Packzettel + Pickup-Point-Liste als Druck-PDF (CUPS: Labeldrucker + bizhub), optimierte Tagesroute in Circuit for Teams (Fahrer drückt nur noch «Start»). Connectoren `connectors/versand/`; Spec (parkiert, Entscheide E1–E7 unbestätigt) in `wissen/spec/outputs/2026-07-04_versandplanung-onlineshop_spec.md`; Wiedereinstieg bei E1 |
...
| `synergie-orchestrator` | `skills/synergie-orchestrator/SKILL.md` | **Struktur-Aufsicht des Hub** (15.07.2026): prueft alle Prozesse/Harnesse GEGENSEITIG auf Synergien ueber 6 Pruef-Fragen (Ketten-Luecke, Doppelrechnung, Compounding-Luecke, Service-Kombination, Entwurfsprozess-Potenzial, Redundanz), pflegt das kompoundierende Register `wissen/koordination/SYNERGIE-REGISTER.md` und leitet Geschaeftsmodell-/Service-Vorschlaege ab (ENTWUERFE fuer `services/KATALOG.md`, nie Selbst-Aktivierung). Referenzdokument `docs/konzepte/260715-Geschaeftsmodell-KI-Architekturbuero/` (GM1–GM6). Abgrenzung: `hub-chef` = operatives Tagesgeschaeft, `wissens-chef` = Cross-KB-Konsistenz, `masterclass/harness-review` = reaktive Korrektur-Cluster — hier die STRUKTUR (Prozess-Synergien + Geschaeftsmodell). Monatlich/auf Zuruf; waehrend Drossel-Phasen kein Scheduled Task ohne Freigabe |
