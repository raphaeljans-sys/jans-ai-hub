...
| GUI-App steuern, die AppleScript-Klicks nicht annimmt (Catalyst/Mac-Catalyst) | **App-eigener `defaults`-Schluessel statt GUI-Klick** — belegt 29.08.2026 (GSE SMART IPTV: `NSDEFAULT_USECACHEMAIN`); trifft die Einstellung direkt, ohne Fenster | MCP `computer-use` (`mcp__computer-use__*`, Zugriff je App einzeln freizugeben) | **`CGEvent` mit gesetztem `mouseEventClickState` (echter Klick/Doppelklick) — Helfer seit 08.09.2026 im Hub: `scripts/cgevent-klick.swift`**, bauen mit `swiftc -O -o /tmp/klick scripts/cgevent-klick.swift`, Aufruf `klick <x> <y> [anzahl]` in Bildschirmpunkten (Retina: Screenshot-Pixel halbieren); belegt 08.09.2026 an GSE SMART IPTV (Suchfeld klicken, Sender doppelklicken, Vollbild). **Tastatureingabe in ein so fokussiertes Textfeld geht danach per `System Events keystroke`** (die Sackgasse betrifft nur Klicks/Navigation, nicht das Tippen). Der erste Helfer vom 29.08.2026 lag nur im Scratchpad und ging verloren (SYN-59) | ~~`System Events` `click`/`keystroke` zum Navigieren~~ **Sackgasse** — von Catalyst-Apps nicht angenommen (belegt 29.08.2026) |
| IPTV-Sender starten (Fussball, Live-Event) | **Raphael will GSE SMART IPTV, nicht VLC (08.09.2026):** GSE oeffnen → Xtream-codes API → Ste2022 → LIVE → «Alle Kanaele Gruppe», Suchfeld per CGEvent-Klick fokussieren (Zeile darueber), Sendername per `keystroke` tippen, Treffer per CGEvent-Doppelklick starten, gruener Fensterknopf = Vollbild. Sender-IDs vorher per `scripts/iptv-sender-kategorien.py` ermitteln; ⚠ der Anbieter liefert fuer einen ausgefallenen Sender ein **Platzhalter-Video «Kanal nicht erreichbar»** als gueltigen TS-Stream (HTTP 200, Sync-Byte 0x47) — ein curl-Probe beweist also nur die Verbindung, nicht das Programm; dann die HD-Variante oder Blue Sport 1 Live nehmen. Nur als Fallback ohne GUI: `bash scripts/iptv-start.sh <stream_id>` (VLC) — liest das Xtream-Konto zur Laufzeit aus der GSE-DB (`xepgsource.db`, Tabelle `apitv_source`, Eintrag Ste2022), fragt `player_api.php` **je Kategorie** ab (ohne `category_id` liefert das Panel 0 Sender) mit User-Agent `VLC/3.0.21` (Python-Default-UA gibt HTTP 461) und startet VLC mit 10 s Puffer; Sender suchen mit `scripts/iptv-sender-kategorien.py` (belegt 08.09.2026, BVB–Villarreal auf «Amazon Prime Sport FHD» 836879) | GSE SMART IPTV von Hand: Xtream-codes API → Ste2022 → Suche | ⚠ VLC-Sparkle-Update mitten im Stream: Dialog «Installieren und erneut starten» beendet die Wiedergabe — `defaults write org.videolan.vlc SUEnableAutomaticChecks -bool false` ist seit 08.09.2026 gesetzt | ~~`plservers`/`plitems1` in `gseplaylist2.db`~~ leer, das Xtream-Konto steht nur in `xepgsource.db` |
