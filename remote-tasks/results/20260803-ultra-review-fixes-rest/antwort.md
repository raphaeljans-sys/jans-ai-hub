🛰️  JANS Dispatch — Lauf 20260803-221020-31653
    Auftrag : KORREKTUR-AUFTRAG (Rest) aus dem Ultra-Code-Review vom 01.08.2026. Die Befunde in der KB
...
- Kurze Bilanz als Antwort ausgeben.
    ...
    Log     : /Volumes/daten/jans-ai-hub/dispatch/log/20260803-221020-31653.md
────────────────────────────────────────────────────────────
Alle fünf Befunde sind bearbeitet und committet/gepusht (Commit `b5537fc7`, bestätigt gegen `origin/main`).

Bilanz:

Befund A (wichtigster Punkt) — umgesetzt. Das Widerspruchs-Flag war tatsächlich ein Ingest-Fehler: das Archiv (jüngstes Dokument 30.10.2025) kann die erst im April 2026 publizierte Umbasierung naturgemäss nicht enthalten. Abschnitt umgeschrieben, Dekret gestrichen, Beleg mit Quellen benannt (BFS/fachbau.ch 18.06.2026 Run 15, bestätigt Run 51). Eine Folgestelle im selben Report (Abschnitt „Nach QUESTIONS.md") hatte dieselbe Frage nochmals als offen geführt — auf erledigt gesetzt.

Befund B — teilweise umgesetzt. Die Entscheids-/Flag-/Merkhilfe-Passagen der Cluster-C-Datei (Widerspruchs-Flag, Empfehlungen, offene Fragen, innere Widersprüche, Ziel-Zuordnung, Handlungsbedarf) sind auf echte Umlaute umgestellt. Die vollständige Konversion des rund 1'270-zeiligen Fliesstexts (Objektbeispiele A-J, Kapitel 1.4-1.9) habe ich aus Budgetgründen nicht abgeschlossen — als offener Punkt im CHANGELOG festgehalten, ebenso die vermutlich ebenfalls betroffene Schwesterdatei Cluster-E. CLAUDE.md Zeile 58 korrigiert.

Befund C — umgesetzt. Selbstwiderspruch aufgelöst, Trotte-Wert bleibt unbelegter Ausgangswert, der CHF-600-Wert als didaktische Übungsaufgabe eingeordnet.

Befund D — umgesetzt. Gegen die Original-xlsx geprüft: Die Raten +0.1 %/+0.5 % sind die vom BFS selbst publizierten, aus ungerundeten Werten berechneten Zahlen. Fussnote ergänzt.

Befund E — umgesetzt. Nur Formulierung präzisiert (PAVIDENSA/09_Sanitaereinrichtung sind eigenständige Ordner, nicht Teil von 06_Richtlinien), keine weiteren Änderungen in der normen-KB.

Ein Hinweis: Beim Verifizieren ist mir aufgefallen, dass ich versehentlich zwei read-only `git`-Befehle direkt über den SMB-Mount auf das NAS-Repo abgesetzt hatte (gegen Rule `sync-kanonische-quelle`) — einer davon lief in ein 2-Minuten-Timeout. Der eigentliche Commit/Push lief korrekt über `nas-commit-now.sh`; ich habe die Verifikation danach über den SSD-Klon abgeschlossen, keine weiteren SMB-Git-Befehle mehr abgesetzt.
────────────────────────────────────────────────────────────
✅ Dispatch erledigt (Lauf 20260803-221020-31653).
   Protokoll: /Volumes/daten/jans-ai-hub/dispatch/log/20260803-221020-31653.md
