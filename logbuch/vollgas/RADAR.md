...
**NACHTRAG 15:50 — Ad-hoc-Schub auf dem Mac Mini, Auftrag Raphael.** Auf die Frage, ob das Restkontingent heute noch auf 100 % gebracht werden kann, ist die Antwort gemessen **nein**: die fehlenden 114 Mio in 21 h verlangen 5.2 Mio/h über beide Stationen, das **16-fache des Wochendurchschnitts** (0.32 Mio/h) und rund das Doppelte der besten je gemessenen Stunde (2.71 Mio/h, und die war ein nächtlicher Ausschlag). Bindend sind das rollierende 5-h-Fenster und der Gate-Deckel (2 MacBook / 3 Mini, bei Druck je einer weniger). Realistisches Ziel heute rund 50 %. Auf Raphaels Anweisung «ja machen» sind **drei gegatete Läufe auf dem Mini gestartet** (Budget je 40 USD, `acceptEdits`): `sia-sweep` (die im Register geführten SIA-Zeilen ohne Datei im Haus — laut Run 60 der grösste unbearbeitete Block der KB), `projekt-triage` (Korpus `buero-projekte`, Triage Phase 1, 807 offene Positionen) und `energie-fragen` (101 offene QUESTIONS). **Diese drei Läufe im Journal sind gewollt, keine Fremdlast und keine Waisen.** Der Mini trägt sie (Druck 1, rund 12 GB frei); das **MacBook wurde bewusst ausgespart** — Druck 2, Swap 3.22/4.10 GB, dort wäre nur ein Lauf drin gewesen und das Risiko grösser als der Ertrag. **Zwei Werkzeug-Lehren aus dem Start:** (1) die nicht-interaktive SSH-Shell hat `PATH=/usr/bin:/bin:/usr/sbin:/sbin` — ohne Homebrew, deshalb brach der erste Versuch mit «claude nicht gefunden» ab; `CLAUDE_BIN=/opt/homebrew/bin/claude` explizit setzen und `~/.jans-dispatch.env` laden. Gleiche Familie wie der KISPI-Befund vom 09.08.: **ein Weg, der nicht im PATH liegt, ist trotzdem ein Weg.** (2) Das Homebrew-Binary des **Mini** antwortet einwandfrei (rc=0), obwohl das des **MacBook** seit 32 h gewedgt ist — die Stationen sind unabhängig zu prüfen, ein Wedge auf der einen sagt nichts über die andere. **Struktureller Punkt für den Montag:** der Unterverbrauch ist kein Betriebsfehler, sondern Folge der beiden stehenden Entscheide «gleichmässig über die Woche» (03.08.) und «Runner bleibt ausgebaut» (30.07.). Wer die Woche ausschöpfen will, hebt die Grundtaktung ab Montag — der Sonntagabend ist der falsche Hebel. Die Runner-Frage ist Raphael vorgelegt und **nicht** als beauftragt gewertet.
