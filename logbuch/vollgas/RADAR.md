...
**Der Slot ist gefeuert, und der Lauf war erfolgreich.** Belegt im Transcript `a463151c-3bd8-46ff-b8b9-7afe063833ab.jsonl`: enqueue **07.09. 10:57:18 UTC** mit dem Wrapper `<scheduled-task name="vollgas-chef-radar">`, 73 Zeilen, sauberer Abschluss um 13:00 mit einem vollstaendigen P1-Bericht. Er fand das **NAS diskstation918 tot** (kein Ping auf 192.168.1.10 vom Mac Mini aus, SMB 445 und DSM 5001 zu, Tailscale offline) und mailte den Blocker um **13:01:43** an rj@ — Betreff «P1 Blocker: NAS diskstation918 offline seit heute Morgen», in Gesendet nachgemessen. Was fehlte, war allein der **Eintrag in dieser Datei**: sie liegt auf dem NAS, und das NAS war weg.

> **Ergaenzung zur Pruefvorschrift (eigener Befund, dritter Ausfalltyp).** Der Auftrag kennt zwei Faelle: «gar nicht gefeuert» (Diskriminator `kern.boottime`, seit 07.09.) und «gefeuert und versagt» (Diskriminator `lastRunAt`). Dieser Lauf gehoert in keinen von beiden: **gefeuert, gearbeitet, gemeldet — und trotzdem kein Eintrag**, weil das AUSGABEMEDIUM ausgefallen war, nicht der Lauf. Wer nur die Eintragsluecke sieht, verbucht einen erfolgreichen P1-Lauf faelschlich als Leerlauf. Der billige Diskriminator ist das **Transcript des betroffenen Slots**: `find ~/.claude/projects -name "*.jsonl" -newermt "<slot-start>" ! -newermt "<slot-ende>"`, dann `grep -o 'scheduled-task name="[a-z0-9-]*"'` auf den Dateikopf. Gleiche Familie wie Rule `auto-verbesserungen` 260730b und 260807 und wie der Vorlauf-Befund zur Gesendet-Mailbox: **ein leeres Ergebnis ist zuerst eine Aussage ueber das Werkzeug oder den Kanal, nicht ueber die Sache.**

**NAS-Ausfall 07.09., Fenster gemessen: 08:45 bis 13:30, rund 4 h 45 min.** Letzter Selbst-Commit vor der Luecke 08:45, erster danach **13:30:03**; dazwischen kein einziger der sonst lueckenlosen 15-Minuten-Commits. Zwei P1-Mails sind in dem Fenster rausgegangen (09:00:24 «Der NAS ist seit 08:45 weg, beide Stationen ohne Hub-Schicht», 13:01:43 dieser Radar). Seit 13:30 laeuft der Selbst-Committer wieder lueckenlos bis 22:45; Mount, LAN-Ping auf 192.168.1.10 und Lesezugriff sind jetzt in Ordnung. **Die Ursache ist von dieser Station aus nicht ermittelbar** — es existiert kein SSH-Alias `nas`, nur der native Committer-Weg. Bleibt offen, ob es ein Neustart, ein Stromunterbruch oder ein DSM-Absturz war.

**Keine Entwarnungsmail.** Mail-Disziplin (b) erlaubte eine, der Blocker ist geloest. Sie unterbleibt trotzdem: die Aufloesung liegt elf Stunden zurueck, und der Hub-Chef sendet in siebeneinhalb Stunden sein Tagesbriefing. Eine Mail um 01:00 nachts ueber ein seit gestern Mittag behobenes Problem waere Rauschen. Der Befund steht hier und geht ueber das Logbuch in das eine Tagesbriefing.

**Fensterprobe [FREI].** `/opt/homebrew/bin/claude -p ... < /dev/null` antwortet «OK», **rc=0 in 8 s**. Fuenfte Bestaetigung in Folge (03.09. 13 s · 04.09. 7 s · 04.09. 6 s · 07.09. 9 s · heute 8 s). Die PATH-Probe bleibt der Regelweg, der Rueckfallweg ueber die app-gebuendelte CLI bleibt Rueckfall. Keine Waisen (`ps` auf `claude -p` leer), Watchdog nicht eingegriffen. Speicher unauffaellig: Druckstufe 1, rund 6.6 GB frei+inaktiv+purgeable.

**Wochenbudget: 5.4 % von 167 Mio bei 7.7 % verstrichener Woche, Rueckstand 2.4 Punkte.** Ampel **FREI**, beide Stationsdateien frisch (MacBook 4.14 Mio, Mini 4.82 Mio). Neue Woche seit dem Reset gestern Montag 12:00; der Verbrauch laeuft dem Zeitverlauf leicht hinterher, was nach dem stehenden Entscheid vom 03.08. (gleichmaessig ueber die Woche) genau richtig ist. Bemerkenswert ist die Verteilung: der Mac Mini liegt erstmals seit Wochen **vor** dem MacBook — Folge des NAS-Ausfalls, der am MacBook den halben Vormittag gekostet hat.

**Liefer-Delta (git, seit 07.09. 08:00): kein Loop ohne Ertrag.** 62 Commits, davon **11 Sachcommits**. Geaenderte Dateien je KB: **energie 12**, **baurecht 12**, koordination 6, bauprodukte 4, projekt-lessons 3, normen 3, twin 2, immobilienbewertung 2. Getragen von energie Run 185, baurecht Buch-Run 142 (RPG 2 seit 1.1.2026 in Kraft, Destillat sagte «anstehend» — ein echter Fassungsfehler, gefunden und korrigiert), normen-training Run 80, wissens-chef Lauf 54, synergie-lauf 30, bauprodukte HC0901-1. Dazu ein neuer Baustein ausserhalb der KBs: **Bauadministrations-Raster bbase.ch** mit Audit ueber fuenf Skills und dem Loop `bauleitung-training`. **Keine Ruecktakt- oder Stilllegungs-Kandidaten**, keine Delta-Null-Serie.

**Nachhol-Luecke des Vorlaufs ist geschlossen.** Die beiden am 07.09. durchgefallenen Tasks sind gelaufen: `konversations-log` 06:23, `vollgas-fruehwarnung` 06:27. Der Radar arbeitet damit wieder auf frischem Destillat. Heute Abend 19:01 feuert ausserdem `methoden-radar` — erster Lauf seit dem 13.08., monatlicher Takt, planmaessig.

**Feuermechanismen: unveraendert, keine Drift.** MacBook geladen: claude-alwayson, claude-autoupdate, screensaver-idle, speicher-waechter, synctask-runner, transcript-rotation, widerruf-queue, wissens-trigger. Mac Mini geladen: nachtschicht, claude-autoupdate, speicher-waechter, synctask-runner, transcript-rotation, wissens-trigger. **Beide `vollgas-*.plist.disabled-260729` liegen unangetastet**, kein Runner wiederbelebt, keine STOP-Datei entfernt.

**P1** — offen und unveraendert seit dem 23.08.: der **Abo-Auslastungs-Connector auf dem Mac Mini misst nichts**, `/login` im Terminal des Mini ist der Handgriff, nur Raphael kann ihn tun. **P2** — Ursache des NAS-Ausfalls vom 07.09. (4 h 45 min) ungeklaert; ein SSH-Weg zur Synology fehlt in diesem Kontext, waere aber fuer genau diese Diagnose der billigste Hebel. **P3** — die Pruefvorschrift dieses Radars sollte den dritten Ausfalltyp aufnehmen (siehe Kasten oben).

---

## 2026-09-07 08:21 — [FREI] **Selbstkontrolle NICHT bestanden: 67 h Lücke, fünf Läufe verpasst. Ursache gemessen und harmlos — der MacBook Pro war ausgeschaltet (Boot heute 07:53, Uptime 29 Min). Der Mac Mini trug die Woche allein und lieferte durchgehend. Ein Befund des Mac Mini ist dabei widerlegt worden: die als unzustellbar geführte Alarm-Mail vom 06.09. liegt in Gesendet.**

**Selbstkontrolle: nicht bestanden.** Letzter Eintrag **04.09. 12:57**, dieser Lauf **07.09. 08:21** — **67 h 24 min** bei 15 h Toleranz (Takt 12 h + 3 h). Verpasst sind fünf Slots: 05.09. 00:50 und 12:50, 06.09. 00:50 und 12:50, 07.09. 00:50.

**Ausfalltyp: gar nicht gefeuert.** Der Diskriminator des Auftrags («liegt `lastRunAt` nach dem letzten Eintrag, hat der Lauf gefeuert und versagt») trägt hier nicht: die Registry hält nur den *letzten* Wert, und der ist dieser Lauf selbst (2026-09-07T06:21:31Z). Entschieden hat stattdessen der Boot-Zeitstempel: `kern.boottime` = **Mo 07.09.2026 07:53:42**, `uptime` 29 Minuten. Die Station war die ganze Lücke über aus. Kein Kontingent-Blocker, kein Absturz, kein Leerlauf — und nichts, was zu beheben wäre.

> **Ergänzung zur Prüfvorschrift (eigener Befund, für den nächsten Ausfall):** Bei einem Ausfall über mehr als einen Takt taugt `lastRunAt` nicht mehr zur Trennung der beiden Ausfalltypen, weil der laufende Lauf den Wert bereits überschrieben hat. Der billige und eindeutige Diskriminator ist **`sysctl -n kern.boottime` plus `uptime`**: liegt der Boot innerhalb der Lücke, war die Station aus und es hat nichts gefeuert. Erst wenn die Station durchlief, lohnt die Suche im Lauf-Journal und in den Transcripts. Ein zweites, ebenso billiges Signal ist der **Nachhol-Sturm** in der Registry (siehe unten) — er entsteht nur beim Wiederanlauf.

**Fensterprobe [FREI].** `/opt/homebrew/bin/claude -p ... < /dev/null` antwortet «OK», **rc=0 in 9 s**. Vierte Bestätigung in Folge nach 03.09. (13 s), 04.09. 00:57 (7 s) und 04.09. 12:57 (6 s). Die Wedge-Episode vom 29.08. bleibt geschlossen, die PATH-Probe bleibt der Regelweg, der Rückfallweg über die app-gebündelte CLI bleibt Rückfall. Keine Waisen (`ps` auf `claude -p` leer), der Watchdog musste nicht eingreifen.

**Wochenbudget: 33.4 % von 167 Mio bei 97.8 % verstrichener Woche, Rückstand 64.4 Punkte.** Ampel FREI, beide Stationsdateien frisch (MacBook 43.24 Mio, Mini 12.61 Mio). Der Reset fällt auf **heute Montag 12:00**, die Woche ist damit gelaufen und der Rückstand nicht mehr aufholbar. Er ist auch nicht zu beklagen: nach dem stehenden Entscheid vom 03.08. soll der Verbrauch gleichmässig über die Woche laufen, nicht am Ende hochgerissen werden. Festzuhalten bleibt die Grösse — **drei von sieben Tagen ohne die Arbeitsstation** sind der teuerste einzelne Posten dieser Woche, und die Ursache liegt ausserhalb des Hub.

**Liefer-Delta (git, seit 04.09. 12:00): kein Loop ohne Ertrag, trotz Stationsausfall.** 294 Commits, davon **23 Sachcommits** (der Rest `nas-selfcommit`/`station-status`, zählt nicht). Geänderte Dateien je KB: **energie 66**, **twin 10**, **normen 9**, **bauprodukte 4**, koordination 3, auflagebereinigung 3, projekt-lessons 2, spec 1, immobilienbewertung 1. Getragen hat das allein der Mac Mini (Uptime 6 d 17 h): energie Runs 179 bis 184, normen-training Run 80, bauprodukte HC0901-1. Alle Nachtschicht-Slots im Journal mit **rc=0**, Kosten 0.46 bis 4.61 USD je Lauf. **Keine Rücktakt- oder Stilllegungs-Kandidaten** — kein Loop zeigt eine Delta-Null-Serie, und die MacBook-Loops hatten keine Gelegenheit zu liefern, was nach der Regel ausdrücklich kein Leerlauf ist.

**Nachhol-Sturm beim Wiederanlauf, und er ist unvollständig.** Zwischen **07:42 und 08:21** hat die App dreizehn überfällige Tasks in dichter Folge nachgeholt (twin-mail, twin-fidelity, logbuch-radar, behoerden-zh, ag-gruendung, tenant-hygiene, hub-chef, normen-training, wissens-chef, mahnwesen, zahlungsabgleich, wettbewerbs-dna, dieser Radar). **Zwei blieben liegen** und stehen mit `nextRunAt` erst auf morgen:

- **`konversations-log`** (06:10), letzter Lauf 04.09. Das ist die **Vorstufe des logbuch-radar** — der Radar lief heute 07:45 also auf einem Konversations-Destillat vom 04.09. und hat drei Tage Sessions nicht gesehen.
- **`vollgas-fruehwarnung`** (07:15), letzter Lauf 04.09. Fällt heute ganz aus.

Beide laufen morgen regulär wieder; der Schaden ist auf einen Tag begrenzt und ein Eingriff lohnt nicht. Festgehalten wird es, weil daraus eine Regel folgt: **ein Nachhol-Sturm ist kein vollständiger Nachhol-Sturm.** Wer nach einem Stationsausfall prüft, ob die Loops wieder greifen, darf sich nicht damit begnügen, dass «die meisten» gelaufen sind — die Reihenfolge ist nicht die des Cron, und was durchfällt, fällt still durch. `heartbeat-daily` (09:40) und `synergie-lauf-taeglich` (17:00) kommen heute noch regulär.

**Befund am Mac Mini: der zweite blinde Fleck des Abo-Checks existiert so nicht.** Der Lauf vom 06.09. schrieb `UNZUSTELLBAR.md` mit dem Befund, seine Eskalationsmail sei im Entwurf liegen geblieben (Entwürfe 149 → 150, in Gesendet nicht auffindbar). Gegenmessung von dieser Station auf demselben Konto: die Nachricht **«Abo-Auslastung blind seit 2 Läufen — Terminal, claude, /login» liegt in Gesendet, Zeitstempel 06.09. 18:09:07**, also exakt der Sendezeitpunkt. Sie ist angekommen. Versagt hat nicht der Versand, sondern die **Erfolgskontrolle auf dem Mini** — eine Prüfung unmittelbar nach `send` gegen eine dort möglicherweise nicht synchrone Gesendet-Mailbox. Nachtrag mit dieser Messung ist in `logbuch/abo-check/UNZUSTELLBAR.md` angehängt (Original unangetastet), damit der Lauf am 13.09. nicht auf der falschen Annahme aufsetzt und den Meldekanal eskaliert. Dieselbe Familie wie Rule `auto-verbesserungen` 260730b und 260807: **ein leeres Suchergebnis ist zuerst eine Aussage über das Werkzeug.** Der **erste** blinde Fleck bleibt offen und unverändert P1 für Raphael: der Connector misst seit dem 23.08. nichts, `/login` auf dem Mac Mini ist der Handgriff.

**Feuermechanismen: unverändert, keine Drift.** MacBook geladen: claude-alwayson (PID 958), claude-autoupdate, screensaver-idle, speicher-waechter, synctask-runner, transcript-rotation, widerruf-queue, wissens-trigger. Mac Mini geladen: nachtschicht, claude-autoupdate, speicher-waechter, synctask-runner, transcript-rotation, wissens-trigger, cockpit, git-auto-sync, nas-mount, station-status. **Beide `vollgas-*.plist.disabled-260729` liegen unangetastet**, kein Runner wiederbelebt, keine STOP-Datei entfernt. Mini-Registry unverändert bei acht Tasks; `energie-training` trägt weiterhin `enabled: false` in der Frontmatter und feuert trotzdem täglich und liefert (Runs 179 bis 184) — das ist der bekannte Dokumentation-gegen-Live-Widerspruch und wird ausdrücklich **nicht** «geheilt».

**Speicher:** frei + inactive + purgeable = **3.45 GB**, `kern.memorystatus_vm_pressure_level` = **2** (warning). Bei 29 Minuten Uptime und dreizehn nachgeholten Tasks in vierzig Minuten ist das erwartbar und kein Handlungsbedarf; der `speicher-waechter` ist geladen.

**Mail: keine.** Kein Anlass nach der Mail-Disziplin. Der Ausfall selbst ist bereits gemeldet — der Hub-Chef hat heute **08:09** gemailt («Frist Thalwil 12:00, und heute frueh hat Dich kein Loop erreicht»), und der Abo-Check-P1 ging am **06.09. 18:09** raus. Eine zweite Meldung desselben Befunds wäre Wiederholung.

**P1 — `/login` auf dem Mac Mini, unverändert seit dem 30.08.** Der Abo-Auslastungs-Connector misst seit dem 23.08. nichts (Refresh-Token durch Rotation überholt, `invalid_grant`, dann 401). Damit fehlt seit **fünfzehn Tagen** die einzige direkte Sicht auf Prozentstand, Reset-Zeitpunkt und Extra-Usage des Abos; das hier gemessene Wochenbudget ist ein Eigenbau aus Transcript-Summen und kalibrierten 167 Mio, kein ausgewiesener Wert. Nur Raphael kann das lösen: auf dem Mac Mini im Terminal `claude` starten, `/login`, Methode «Claude account with subscription». Bereits gemailt am 06.09., wird von diesem Radar nicht erneut gemailt.

**P2 — der Ausfall dreier Tage ist ein Stations-, kein Hub-Problem, und der Hub hat ihn gut überstanden.** Der Mac Mini hat als Always-On-Station genau das getan, wofür die Rollentrennung gebaut wurde: durchgelaufen, geliefert, sauber protokolliert. Kein Eingriff nötig. Zu beobachten bleibt allein, ob das Muster wiederkehrt — dann wäre zu prüfen, ob die drei MacBook-only-Loops mit Aussenwirkung (konversations-log als Radar-Vorstufe, vollgas-fruehwarnung, heartbeat) auf dem Mini eine Zweitheimat brauchen. **Heute keine Empfehlung**, ein einzelnes Wochenende trägt sie nicht.

**P3 — Takt und Toleranz stimmen.** 12-h-Takt, Toleranz 15 h, `nextRunAt` 07.09. 12:57. Keine Anpassung. Dieser Lauf blieb inline und schlank (rund zehn Werkzeugaufrufe, keine Delegation) — die Vertiefung beschränkte sich auf den einen Befund, der sie verlangte (die widerlegte Unzustellbarkeit).

---

## 2026-09-04 12:57 — [FREI] **Regellauf ohne Befund. Die PATH-Probe antwortet zum dritten Mal in Folge (6 s, rc=0), der beste Wert der Reihe. Der einzige Befund ist eine Messfalle im Prüfwerkzeug selbst — und die Unterauslastung, die sich weiter öffnet: das Wochenkontingent läuft dem Zeitverlauf jetzt um 30.5 Punkte hinterher.**

**Selbstkontrolle: bestanden.** `lastRunAt` dieser Task **04.09. 12:57** (2026-09-04T10:57:40Z), letzter Eintrag **04.09. 00:57** — exakt **12 h 0 min** bei 12-h-Takt, Toleranz 15 h (Takt + 3 h). Kein Lauf fehlt; der zweite Ausfalltyp ist nicht zu prüfen.

**Fensterprobe [FREI].** `/opt/homebrew/bin/claude -p ... < /dev/null` antwortet «OK», **rc=0 in 6 s**. Dritte Bestätigung in Folge nach dem 03.09. (13 s) und dem 04.09. 00:57 (7 s); die Wedge-Episode vom 29.08. bleibt geschlossen, der Rückfallweg über die app-gebündelte CLI bleibt Rückfall.

**⚠ MESSFALLE IM EIGENEN PRÜFWERKZEUG (eigener Befund, für die Latenz-Reihe wichtig).** Die erste Messung dieses Laufs meldete **180 s** — das ist **nicht** die Latenz der Probe, sondern die Laufzeit des Wrappers. Ursache: der Watchdog `( sleep 180; kill ... ) &` hält die Pipeline des Scripts offen, auch nachdem `wait $CPID` längst zurück ist und `kill $WPID` die Subshell beendet hat — der geerbte `sleep` läuft weiter und hält stdout. Wer die Wanduhr um `bash probe.sh` legt, misst deshalb **immer die Watchdog-Grenze**, nie die Antwortzeit. Die saubere Messung (Zeitstempel unmittelbar um `wait $CPID`, danach `kill -9 $WPID` plus `pkill -P $WPID sleep`) ergab **6 s**, Faktor 30 Unterschied.
**Warum das zählt:** ein 180-s-Wert wäre als Ausreisser in die Latenz-Reihe eingegangen (7 · 13 · 7 · **180**) und hätte exakt den Trend vorgetäuscht, der am 19.08. schon einmal fälschlich als Degradation gelesen wurde — diesmal aber ohne Gegenbeleg, weil die Zahl an der Watchdog-Grenze klebt und damit reproduzierbar aussieht. **Regel für künftige Läufe: die Latenz IM Probe-Script messen, nie um das Script herum.** Gleiche Familie wie die `find -newermt`-Falle (20.08.) und wie Rule `auto-verbesserungen` 260807: eine Zahl, die das Werkzeug erzeugt, ist zuerst eine Aussage über das Werkzeug.

**Wochenbudget: FREI, aber die Lücke wächst.** `kontingent-budget.sh --json`: **45.5 von 167 Mio teure Token (27.2 %)** bei **57.7 %** verstrichener Woche, **Vorsprung −30.5 Punkte**. Stationen: MacBook Pro 37.33, Mac Mini 8.16, beide frisch. Zum Vergleich der Vorlauf: **04.09. 00:57 −27.1 Punkte**, jetzt **−30.5** — der Rückstand hat in zwölf Stunden um 3.4 Punkte zugenommen, er schliesst sich also nicht, er öffnet sich. Linear fortgeschrieben endet die Woche bei rund **79 Mio**, also **47 % des Budgets**; gut 88 Mio blieben ungenutzt. Das ist keine Verletzung des stehenden Entscheids vom 03.08. («gleichmässig über die Woche») — gleichmässig ist der Verlauf durchaus. Es verfehlt aber das Ziel dieses Auftrags, das Kontingent PRODUKTIV auszuschöpfen. **Keine Massnahme aus eigenem Antrieb**: die Regimefrage ist entschieden und wird nicht erneut vorgelegt; ein Tagesdeckel-Mechanismus in die Gegenrichtung (Auffüllen) existiert nicht und wäre ein eigener Auftrag.

**Liefer-Delta: kräftig, kein Leerlauf-Kandidat.** Gemessen über git (nicht über `find`), Basis `76fc3b5a` bis HEAD, Fenster 14 h: **72 Commits**, davon **19 substanziell** (53 sind automatische `nas-selfcommit`-Läufe des 15-Min-Takts und zählen nicht als Arbeit). Geänderte Dateien je KB: **normen 14 · twin 11 · planungsgrundlagen 6 · bauprodukte 5 · koordination 3 · immobilienbewertung 3 · baurecht 3 · grobkosten 2 · energie 2 · architektur-fachwissen 2**. Ausserhalb `wissen/` gearbeitet: `logbuch/fristen.md`, `logbuch/LOGBUCH.md`, `rules/betrieb-chronik.md`, `rules/jans-dna-facetten.md`, `scripts/lauf.sh`, `skills/wissens-destillat/tools/inventar.sh`, `docs/konzepte/260830-Laufzeitschicht-Umbau/`. Die operativen Loops haben alle geliefert (logbuch-radar 06:55, mahnwesen 08:05, ag-gruendung 07:46, hub-chef 08:39, twin-fidelity 05:45, vollgas-fruehwarnung 07:15). **Kein Loop steht bei drei oder mehr Delta-Null-Läufen. Keine Rücktaktung, keine Stilllegung.**

**Feuermechanismen: Sollstand auf beiden Stationen.** MacBook Pro: `ch.jans.vollgas-supervisor` und `ch.jans.vollgas-monitor` unverändert als `*.disabled-260729` abgelegt und **nicht geladen**; geladen sind nur die regulären Jobs (synctask-runner, speicher-waechter, transcript-rotation, nas-keepalive, git-auto-sync, station-status, cockpit-server, claude-alwayson, wissens-trigger, widerruf-queue, screensaver-idle). Mac Mini: `ch.jans.nachtschicht` **geladen** — der einzige Lern-Taktgeber, wie vorgesehen; `vollgas-supervisor` dort ebenfalls nur als `.disabled-260729` vorhanden und nicht geladen. Kein Doppelfeuern, keine wiederbelebte plist. Registry MacBook: 36 Tasks, die deaktivierten sind durchgehend die per Entscheid auf Ereignis-Trigger gestellten (immobewertung-training, spec-training, synergie-lauf-monatlich) plus abgelaufene One-Time-Tasks — **keine Task hängt aus einer früheren Drossel und wartet auf Wiederscharfschaltung.**

**Speicher: unauffällig.** frei+inaktiv+purgeable rund **3.9 GB**, `kern.memorystatus_vm_pressure_level: 1` (normal). Keine `claude -p`-Waisen (`ps` leer), der Watchdog dieses Laufs hat sauber abgeräumt.

**P1 — keiner.** Kein Blocker, keine Mail (kein neuer P1, kein gelöster P1, kein erschöpftes Wochenkontingent).
**P2 — Unterauslastung, zweiter Lauf in Folge zunehmend.** 45.5 von 167 Mio teuren Token bei 57.7 % Wochenverlauf. Zur Kenntnis Raphaels, ohne Massnahme dieses Radars; falls das Kontingent aktiv aufgefüllt werden soll, braucht es einen eigenen Auftrag (Gegenstück zum nie gebauten Tagesdeckel).
**P3 — Latenz-Messung im Script.** Ab dem nächsten Lauf im Probe-Script messen, nicht um es herum (siehe Messfalle oben). Kein Werkzeug-Umbau nötig, nur die Messstelle.

---

## 2026-09-04 00:57 — [FREI] **Regellauf ohne Befund. Die PATH-Probe antwortet zum zweiten Mal in Folge (7 s, rc=0) — die gestern beschlossene Pflicht-Gegenmessung ist gemacht und faellt positiv aus, die Wedge-Episode ist damit geschlossen. Auffaellig ist heute die andere Richtung: das Wochenkontingent laeuft dem Zeitverlauf um 27 Punkte HINTERHER.**

**Selbstkontrolle: bestanden.** `lastRunAt` dieser Task **04.09. 00:57** (2026-09-03T22:57:44Z), letzter Eintrag **03.09. 12:57** — exakt **12 h 0 min** bei 12-h-Takt, Toleranz 15 h (Takt + 3 h). Kein Lauf fehlt, der zweite Ausfalltyp ist nicht zu pruefen.

**Fensterprobe [FREI].** `/opt/homebrew/bin/claude -p ... < /dev/null` antwortet «OK», **rc=0 in 7 s** — der beste Wert der ganzen Messreihe, gleichauf mit den 7 bis 8 s vom 19./20.08. Damit ist der gestrige Befund unabhaengig bestaetigt: die Fassung 2.1.236 laeuft, sie hat nie einen Nachschub gebraucht, und der Rueckfallweg ueber die app-gebuendelte CLI bleibt Rueckfall. **Die daraus abgeleitete Regel hat ihren ersten Regellauf ueberstanden**: die Gegenmessung kostete 7 Sekunden und haette, waere sie am 29.08. gemacht worden, elf Laeufe lang einen falschen Betriebszustand verhindert.

**P2 — Das Wochenkontingent laeuft dem Zeitverlauf deutlich hinterher, nicht voraus.** Gemessen ohne Modellaufruf: **39.24 von 167 Mio teuren Token = 23.5 %**, bei **50.6 % verstrichener Woche**. Vorsprung **-27.1 Punkte**, Ampel FREI. Aufteilung MacBook Pro 31.96 Mio, Mac Mini 7.28 Mio, beide Stationsdateien frisch (0.0 h alt). Bei linearer Fortschreibung schliesst die Woche bei rund 46 % des Budgets. Das ist die **Gegenrichtung** zum Vorfall vom 01.–03.08., der die Budgetmessung ueberhaupt erst ausgeloest hat, und es verletzt den stehenden Entscheid vom 03.08. («gleichmaessig ueber die Woche») nicht — gleichmaessig heisst nicht ausgeschoepft. Der Auftrag dieses Radars nennt aber ausdruecklich das produktive Ausschoepfen als Ziel, deshalb steht der Befund hier statt in der Fussnote. **Keine Massnahme in diesem Lauf**: es gibt keinen gedrosselten Loop zurueckzudrehen (alle Lern-Loops sind scharf), und ein Hochtakten aus Budgetgruenden allein waere genau der Laufzaehler-Fehler, den Rule 260830 verbietet. Wenn die Unterauslastung bestehen bleibt, ist der richtige Hebel neues Material fuer eine hungrige KB, nicht ein hoeherer Takt fuer eine gesaettigte.

**Liefer-Delta: alle Loops liefern, kein Leerlauf-Kandidat.** Gemessen ueber `git log --name-only` (nicht ueber `find -newermt`, Werkzeug-Falle vom 20.08.), Fenster 14 h: planungsgrundlagen 9 Dateien · energie 9 · koordination 6 · baurecht 6 · wettbewerbs-dna 3 · normen 3 · immobilienbewertung 3 · kunde-bopp 2 · grobkosten 2 · architektur-fachwissen 2. Substanzielle Commits im Fenster: **energie Run 176** (EIV-Leistungsbeitraege am Pronovo-Tarifrechner gemessen, dritte Stufe ab 100 kWp erstmals erfasst, Cross-KB-Korrektur nach planungsgrundlagen), **wissens-chef Lauf 52** (SIA-416-Aussenflaechenbefund an sechs Konsumenten ausgeliefert, zwei Befunde widerlegt), **synergie-lauf 27** (SYN-70/SYN-71, zwei Querverweise gesetzt), **Nachtschicht Laufzeitschicht-Umbau Phase 0 und 1** (`scripts/lauf.sh` gebaut, additiv), **tenant-hygiene** (903.30 GB / 85.70 %, Restlaufzeit 52 Tage). Keine drei aufeinanderfolgenden Delta-Null-Laeufe bei irgendeinem Loop, also keine Ruecktaktung faellig.

**Feuermechanismen: alle drei Orte stimmen ueberein.** (i) Lokale Registry 34 Tasks, Sollstand. (ii) launchd beide Stationen: `ch.jans.vollgas-supervisor` und `ch.jans.vollgas-monitor` sind **nicht geladen**, beide plists tragen weiterhin `.disabled-260729` — der stehende Entscheid vom 30.07. ist unverletzt. `ch.jans.nachtschicht` auf dem Mini geladen. (iii) Mini-Registry acht Tasks, unveraendert gegen den Sollstand vom 06.08. **Keine Doppelfeuerung.**

**Speicher und Waisen: sauber.** `ps -eo pid,ppid,command | grep "claude -p"` zeigt **nichts** — der Watchdog dieses Laufs musste nicht eingreifen, es liegen keine Waisen mit PPID 1 im Lauf-Gate. Frei (free + inactive + purgeable) **3.79 GB**, `kern.memorystatus_vm_pressure_level: 1` (normal). Lauf-Journal 03.09.: fuenf Mac-Mini-Laeufe, **alle rc=0**, kein Fehlstart.

**P3 — `speicher-hygiene-quartal` hat noch nie gefeuert.** Die Task traegt in der Registry **kein `lastRunAt`-Feld** bei Cron `0 7 1 2,5,8,11 *`; der faellige Termin war der **01.08.2026** und fiel damit mitten in den 47-Stunden-Wochenlimit-Ausfall vom 01.–03.08. Naechster regulaerer Termin 01.11.2026. Kein Handlungsbedarf, aber notiert, damit die Luecke beim naechsten Quartalstermin nicht als Neubefund auftaucht.

**Mail: keine.** Kein P1-Blocker, kein geloester P1, kein erschoepftes Wochenkontingent. **Turns: schlank**, alle Messungen inline und parallel, kein Subagent (Regellauf delegiert nicht, Korrektur vom 08.08.).
...
- **P1 — unveraendert der Sync-Stau, jetzt im fuenften Tag und weiter wachsend.** Nativ auf der Synology gegengemessen (`/volume2/daten/jans-ai-hub`, nicht aus dem Vorlauf uebernommen): **Versuch 790**, Divergenz **698 lokal / 267 remote** (Vorlauf 12 h zuvor: 634/267, also **+64**, rund 5 je Stunde — exakt dieselbe Rate wie im Vorlauf). 211 Logzeilen allein heute. Das Repo ist dabei sauber: Kopf `2652236f1` (12:45), **kein** `MERGE_HEAD`, **kein** `rebase-merge`, **null** unmergte Dateien — das Abbruch-Verhalten des Scripts traegt weiterhin. `wissen/architektur-fachwissen/wiki/` fuehrt auf dem NAS **99** Artikel, auf `github/main` **455**; beide Zahlen nativ gezaehlt, unveraendert seit gestern. Neu gemessen: `git merge-tree` weist **13 Konfliktstellen** aus. **Achtung bei der Einordnung** — der Vorlauf nannte «vier Konfliktdateien»; das ist eine andere Einheit (Dateien gegen Konfliktstellen), die beiden Zahlen widersprechen sich nicht und duerfen nicht als Wachstum gelesen werden. Der vorgelegte Weg bleibt unveraendert und nur nach Sichtung der Konfliktdateien: `ssh raphaeljans@diskstation918.tail8265aa.ts.net "cd /volume2/daten/jans-ai-hub && git merge github/main"`. Es bleibt ein Urteil ueber fremde Arbeit und damit Raphaels Entscheid; kein Loop und kein Radar fasst es an. Ohne die Behebung von **B6** (`dispatch-run.sh` Zeile 33 pinnt das Arbeitsverzeichnis auf den SSD-Klon, Spec `docs/konzepte/260830-Laufzeitschicht-Umbau/SPEC.md`) kehrt der Stau in Tagen wieder.
  ...
  **Nachtrag aus dem eigenen Sicherungsversuch (13:01, derselbe Lauf) — der Konfliktsatz WAECHST.** Der Eintrag oben ist als `c2fc2358d` nativ auf der Synology committet; Arbeitsbaum sauber, **kein** `MERGE_HEAD`, **null** unmergte Dateien, das Abbruch-Verhalten des Scripts traegt zum wiederholten Mal. Der Push scheiterte erwartungsgemaess — und die Merge-Ausgabe korrigiert meine eigene Vorsichtsformulierung von oben: es sind **sieben** Konfliktdateien, nicht vier. **Kein eigener Fund** — die Nachtschicht des Mac Mini hat dieselben sieben heute um 02:3x gemessen und im Fristen-Register festgehalten; mein Beitrag ist die Bestaetigung, dass der Satz seit elf Stunden **stabil** bei sieben liegt, und die Einordnung ihrer Art. Die vier bekannten — `wissen/normen/CHANGELOG.md`, `wissen/baurecht/CHANGELOG.md`, `wissen/baurecht/training/KORPUS-QUEUE-thalwil-reglemente.md`, `wissen/grobkosten/CHANGELOG.md` — und **drei neue, alle in `architektur-fachwissen`**: `raw/inventar/archiv-fachwissen__10_Protomodern.md`, `wiki/QUESTIONS.md` und `wiki/protofunktional-begriff-abgrenzung-funktionalismus.md`. Die letzte ist ein **`add/add`**-Konflikt: beide Seiten haben unabhaengig voneinander einen Artikel gleichen Namens angelegt. **Das ist genau der Doppelarbeits-Schaden, den der Eintrag vom 02.09. 00:57 vorhergesagt hat**, und er ist jetzt eingetreten — die KB mit der Bestandsluecke (99 gegen 455) produziert Artikel, die drueben schon existieren. Der Aufwand fuer die Aufloesung waechst damit von vier auf sieben Urteile, und die drei neuen sind nicht mehr blosses Aneinanderhaengen: bei `add/add` ist inhaltlich zu entscheiden, welche Fassung gilt. Das ist beim Vorlegen an Raphael zu nennen — **jeder weitere Tag Stau verteuert die Aufloesung messbar, nicht nur die Sicherungslage.**
...
**Keine Mail.** Kein neuer Blocker, kein geloester P1, kein erschoepftes Wochenkontingent — der Sync-Befund ist derselbe Vorgang, fuer den eine Wiederholungsmail seit dem 30.08. ausdruecklich ausgeschlossen ist. Er erreicht Raphael um 06:55 ueber das Fristen-Register und um 08:39 im Tagesbriefing des Hub-Chefs. **Regellauf: keine Delegation, Messungen inline und parallel.** Die Messphase brauchte 19 Werkzeugaufrufe, der Sicherungs-, Nachtrags- und Korrekturzyklus weitere 13 — **zusammen 32**. Die Zahl «15», die hier zunaechst stand, war beim Schreiben richtig und danach falsch: sie zaehlte den Zyklus nicht mit, der dem Schreiben des Eintrags erst folgt. **Der naechste Lauf zaehlt am Ende, nicht in der Mitte.** Inhaltlich bleibt der Lauf im Sollmass — die 13 Zusatzaufrufe sind Sicherung und zwei Selbstkorrekturen, keine Tiefenuntersuchung.
...
**Nachtrag aus dem eigenen Sicherungsversuch (01:0x, derselbe Lauf).** Der Eintrag oben ist als `e12419a4a` nativ auf der Synology committet, Arbeitsbaum sauber, keine Merge-Marker, Divergenz danach 635/267. Der Push scheiterte erwartungsgemaess — und die Fehlerausgabe **praezisiert den P1**: es sind **vier** Konfliktdateien, nicht eine. `wissen/normen/CHANGELOG.md` (bisher allein benannt), dazu `wissen/baurecht/CHANGELOG.md`, `wissen/baurecht/training/KORPUS-QUEUE-thalwil-reglemente.md` und `wissen/grobkosten/CHANGELOG.md`. Alle vier sind Dateien, in denen beide Seiten **oben** angefuegt haben; `wissen/baurecht/training/PROGRAMM.md` und `wissen/baurecht/wiki/QUESTIONS.md` merged git dagegen automatisch. Die Aufloesung bleibt mechanisch einfach, betrifft aber **vier** Urteile ueber fremde Arbeit statt einem — das ist beim Vorlegen an Raphael zu nennen, damit der Aufwand nicht unterschaetzt wird.
...
**Keine Mail.** Kein geloester P1, kein erschoepftes Wochenkontingent. Der Sync-Befund ist derselbe Vorgang, fuer den eine Wiederholungsmail seit dem 30.08. ausdruecklich ausgeschlossen ist — neu ist seine Praezisierung, nicht sein Bestehen. Er erreicht Raphael morgen um 06:55 ueber das Fristen-Register und um 08:39 im Tagesbriefing des Hub-Chefs; der Nachtrag im Register ist gesetzt und nennt neu den konkreten Konflikt-Commit. **Regellauf: keine Delegation, Messungen inline und parallel — aber mit rund 33 Werkzeugaufrufen deutlich ueber dem Sollmass, und der Grund gehoert festgehalten.** Vier Aufrufe gingen an Fehlversuche beim Zugriffsweg, weil zwei Angaben aus den Vorlaeufen nicht stimmten: (1) der SSD-Klon kennt **keinen** Remote `github` — er heisst `origin`; `git log github/main` scheitert dort mit «mehrdeutiges Argument» und sieht wie ein kaputtes Repo aus. (2) Das Repo auf der Synology liegt unter **`/volume2/daten/jans-ai-hub`**, nicht unter `/volume1`; letzteres liefert ein blosses «No such file or directory». (3) `ssh mini` fuehrt zum **Mac Mini**, nicht zur Synology — der SSH-Weg dorthin ist `raphaeljans@diskstation918.tail8265aa.ts.net`, ablesbar aus `scripts/nas-commit-now.sh`. Gleiche Familie wie die bekannten Messfallen: **ein Fehlschlag am Zugriffsweg sieht aus wie ein Sachbefund.** Die drei Angaben stehen jetzt hier, damit der naechste Lauf sie nicht erneut sucht. Die uebrigen Zusatzaufrufe gingen an den Selfcommit-Log, also an genau den einen Befund, fuer den der Auftrag eine Tiefenpruefung vorsieht.
...
**Nachtrag aus dem eigenen Sicherungsversuch (01:24) — der P1 hat sich beim Schreiben dieses Eintrags selbst bestaetigt.** `nas-commit-now.sh` meldete `rc=1` samt vier Konflikten (`baurecht` CHANGELOG + KORPUS-QUEUE-thalwil, `grobkosten` CHANGELOG, `normen` CHANGELOG) — ausnahmslos append-at-top-Dateien, wie in allen Vorlaeufen. Nativ auf der Synology gegengeprueft: **der Commit selbst ist durchgegangen** (`9abf0988e`, 01:24), **kein** `MERGE_HEAD`, **null** unmergte Dateien, **null** Konfliktmarker in `RADAR.md` und `fristen.md`, Arbeitsbaum bis auf eine Zeile sauber. Gescheitert ist wieder allein der Push nach GitHub. Wer nur den `rc` liest, haelt eine gelungene Sicherung faelschlich fuer gescheitert.
...
**P1 — die NAS/GitHub-Divergenz besteht fort und ist deutlich gewachsen.** Beim Sichern dieses Eintrags meldete `nas-commit-now.sh` erneut `rc=1` samt fuenf Merge-Konflikten. Nativ gegengeprueft auf der Synology: **der Commit selbst ging durch**, Arbeitsbaum sauber (`git status --porcelain` = 0), **kein** `MERGE_HEAD`, **keine** Konfliktmarker in den genannten Dateien. Gescheitert ist allein der Abgleich mit GitHub, wie an allen Versuchen der Vortage. Der `rc=1` bezeichnet also den Push, nicht den Commit; wer nur den rc liest, haelt eine gelungene Sicherung faelschlich fuer gescheitert.

**Messung nach frischem Fetch (13:15):** `git rev-list --left-right --count github/main...HEAD` ergibt **267 / 452** — 267 Commits nur bei GitHub, 452 nur auf dem NAS. Reihe der bisherigen Messungen: 3/50 → 31/13 → 42/19 → 68/34 → **267/452**. **Die Divergenz waechst auf beiden Seiten und beschleunigt.** Ursache unveraendert wie benannt: zwei konkurrierende Schreibwege auf denselben Branch (`com.jans.git-auto-sync` auf dem Mac Mini pusht direkt nach GitHub, waehrend `nas-selfcommit.sh` denselben Bestand auf der Synology committet). **Kein Datenverlust** — beide Staende existieren vollstaendig, sie sind nur nicht vereinigt; GitHub traegt einen Kopf von **heute 06:12**, ist also nicht tot, sondern auf einem eigenen Zweig.

**Nicht aufgeloest, und zwar bewusst.** Eine Zusammenfuehrung ueber 719 divergierende Commits mit ueberlappenden Wissens-Dateien ist genau der Fall, den Rule `auto-verbesserungen` 260811 und Rule `interaktive-eingriffe` ausschliessen: unbeaufsichtigtes Urteil ueber fremde, unbestaetigte Arbeit. Ein Force-Push ist ohnehin verboten. Der Befund steht seit mehreren Tagen als P1 im Register und in `rules/betrieb-chronik.md`; er braucht Raphaels Entscheid darueber, **welcher Schreibweg kuenftig fuehrt** — solange beide parallel pushen, kann kein Rebase aufholen.

**Keine Mail.** Kein NEUER P1, kein geloester P1, kein erschoepftes Wochenkontingent. Die Divergenz ist derselbe Befund wie an den Vortagen und steht im Register; eine Wiederholungsmail dafuer ist ausdruecklich ausgeschlossen. Neu ist allein die Groessenordnung, und die gehoert ins Register, nicht ins Postfach.

**P1b — zwei SMB-Mount-Stalls waehrend dieses einen Laufs.** Der Mount fiel zweimal weg (beim ersten und beim zweiten Schreibversuch). Der erste Stall liess sich mit `ensure-nas-mounted.sh` beheben, beim **zweiten schlug der Guard fehl** und erst `osascript mount volume` half — mit dem Nebeneffekt eines verwaisten Zweitmounts **`/Volumes/daten-1`**. Zwei Stalls in rund zwanzig Minuten sind mehr als das bekannte Idle-Muster; die Haertung (nsmb.conf, Keepalive, Inline-Guard) greift nicht mehr zuverlaessig. `/Volumes/daten` funktioniert, der Zweitmount ist unangetastet geblieben (Unmounten waere ein Eingriff der heiklen Klasse und ist unbeaufsichtigt nicht meine Sache). Fuer den naechsten Lauf: pruefen, ob `daten-1` noch existiert, und den Guard-Fehlschlag nachmessen.
...
**Fensterzustand [FREI].** Probe über die app-gebündelte CLI, neu **2.1.247** (heute 05:27 installiert, 2.1.246 ist aus dem Ordner verschwunden): «OK», rc=0, in **77 Sekunden**. Das ist ein Ausreisser gegen die Reihe 5 s (26.08. 12:57) · 5 s (27.08. 00:57) · 6 s (27.08. 12:57) · 6 s (28.08. 00:57) · 7 s (28.08. 12:57) · 7 s (29.08. 00:57) · **77 s (jetzt)**. Die naheliegende Erklärung ist der **Kaltstart des frisch installierten Binaries** — dieselbe Grössenordnung wie die 76 s am 18.08., die ebenfalls unmittelbar auf einen Versionswechsel folgten. Nach der geltenden Vorschrift ist ein einzelner langsamer Lauf **kein Befund**: erst eine Reihe, die mehrfach in Folge über 150 s bleibt, rechtfertigt einen eigenen Befund-Lauf. Der nächste Lauf misst gegen dieselbe 2.1.247 und entscheidet damit, ob es der Kaltstart war. Keine Waisen (`ps` auf `claude -p` leer). Speicher unauffällig: Druckstufe 1 (normal), rund 5.8 GB frei+inaktiv+purgeable.

**Werkzeug-Befund: die Homebrew-Wedge hält an, jetzt in der dritten Fassung.** Homebrew hat den Symlink `/opt/homebrew/bin/claude` heute um 05:15 auf **2.1.236** umgehängt. Gegenmessung in diesem Lauf: `--version` kam nach **45 Sekunden nicht zurück** (rc=137, Watchdog). Damit ist die Reihe der gewedgten Homebrew-Fassungen **2.1.224 (15./16.08.) · 2.1.231 (22.08.) · 2.1.236 (heute)** — dasselbe Symptom, drei Versionsnummern. Die app-gebündelte CLI ist im selben Lauf sauber durchgelaufen, das Kontingent war also frei; der Fehler liegt weiterhin allein am Binary. **Die Probe bleibt bis auf Weiteres auf dem app-gebündelten Weg**; die PATH-Probe wird gar nicht erst mit `-p` versucht, weil sie 180 s kostet und einen rc=137 liefert, der wie eine Kontingentsperre aussieht und keine ist. Der billige Discriminator ist `--version` mit kurzem Watchdog (45 s), nicht der volle Prompt — so kostet die Gegenprobe eine Dreiviertelminute statt drei.

**Wochenbudget: 50.58 von 167 Mio teuer, 30.3 % bei 72.0 % verstrichener Woche — Ampel FREI, Vorsprung -41.7 Punkte.** Die Reserve wächst zum siebten Mal in Folge (-33.4 → -37.4 → -41.7). Stationen: MacBook Pro 40.25 Mio, Mac Mini 10.33 Mio, beide Stationsdateien frisch. Die Woche läuft deutlich unter Plan, der stehende Entscheid «gleichmässig über die Woche» ist eingehalten, eine Drossel ist nicht angezeigt und keine Lern-Task steht wegen Kontingent auf `enabled=false`.

**Liefer-Delta im Fenster 00:57–12:57 (gemessen über git, nicht über `find -newermt`): drei KBs, alle plausibel.** `wissen/twin` 11 Dateien, `wissen/normen` 7, `wissen/architekten-synobsis` 2; dazu `logbuch/` (LOGBUCH, Fristen, Konversationen, Arbeits-Weiche, Kontingent) und `rules/betrieb-chronik.md`. Gegenprobe gegen die Loops, die in diesem Fenster überhaupt feuern durften: `normen-training-nacht` 01:28 → normen ✓ · `twin-mail-training` 03:40 und `twin-fidelity-review` 05:45 → twin ✓ (Commit `5d3bf733f`) · Mac-Mini-Nachtschicht 02:30 und 05:30 → architekten-synobsis, Sammelfrage Charge 1 mit 14/14 abgeschlossen (Commit `73cd4ee89`, beide Läufe rc=0 im Lauf-Journal) · `konversations-log` 06:13, `logbuch-radar` 06:55 (`f7ca658b6`), `vollgas-fruehwarnung` 07:15 (`5ad519007`), `zahlungsabgleich-check` 08:23, `hub-chef-taeglich` 08:39 (`ea349d57f`) ✓. **Kein Loop ohne Liefer-Delta.** `heartbeat-daily`, `mahnwesen-verzugscheck` und `ag-gruendung-monitor` sind Mo–Fr getaktet und hatten heute, an einem Samstag, keinen Lauf — das ist kein Leerlauf. Die drei KBs sind kein Rückgang gegenüber den zehn vom 00:57-Lauf: jenes Fenster enthielt die volle Nachtschicht plus die Abendläufe, dieses hier nur die zweite Nachthälfte.

**Feuermechanismen: unverändert regelkonform, alle drei Orte geprüft.** MacBook Pro: `ch.jans.vollgas-supervisor` und `ch.jans.vollgas-monitor` weiterhin als `*.disabled-260729` abgelegt und nicht geladen, `com.jans.aihub.runner` als `*.disabled-260728`. Mac Mini: `ch.jans.vollgas-supervisor` ebenfalls `*.disabled-260729`, `ch.jans.nachtschicht` geladen und liefernd. Beide Registries geprüft — die lokale mit 33 Einträgen, die des Mini per `ssh` mit **acht** Tasks, exakt im dokumentierten Sollstand (`arbeits-weiche-review`, `baurecht-buch-training`, `claude-abo-auslastung`, `energie-training`, `grobkosten-training`, `normen-training-mini`, `planungsgrundlagen-training`, `synobsis-batch-nacht`). Kein Loop wird von zwei Mechanismen gefeuert, kein stillgelegter Job ist wieder aktiv.

**P3 (unverändert, keine Eskalation): Wildcard-Allowlist-Regel in `.claude/settings.local.json`.** Steht seit dem 28.08. Die Zeile zeigt auf eine einzelne, vergängliche Tool-Result-Datei einer Session vom Sommer und hat keinen Nutzen mehr; ihr `*` steht vor dem Rest des Befehls und genehmigt damit auch eingeschobene Optionen ohne Rückfrage. Das Aufräumen ist ein Eingriff in die Berechtigungskonfiguration und damit kein Radar-Geschäft — Vorschlag an den Hub-Chef bzw. an Raphael: Regel ersatzlos streichen. Kein Sendegrund nach Rule `auto-verbesserungen` 260803 (Hub-Internum ohne Aussenwirkung).

**Massnahmen: keine.** Regellauf, 11 Werkzeugaufrufe inline in drei parallelen Blöcken, keine Delegation. Keine Mail: kein neuer P1-Blocker, kein gelöster, kein erschöpftes Wochenkontingent. Die Homebrew-Wedge ist ein Hub-Internum ohne Aussenwirkung und arbeitsfähig umgangen, also ebenfalls kein Sendegrund.
...
**Fensterzustand [FREI].** Probe über die app-gebündelte CLI, weiterhin **2.1.246**: «OK», rc=0, in **6 Sekunden**. Reihe unverändert stabil: 5 s (25.08. 00:57) · 6 s (25.08. 12:57) · 6 s (26.08. 00:57) · 5 s (26.08. 12:57) · 5 s (27.08. 00:57) · 6 s (jetzt). Keine Waisen (`ps` auf `claude -p` leer), Speicherdruck normal (`kern.memorystatus_vm_pressure_level: 1`).

**Die Homebrew-Wedge steht seit fünf vollen Tagen.** `/opt/homebrew/bin/claude` zeigt unverändert auf **2.1.231** (Symlink vom 22.08. 05:15), das Caskroom führt keine neuere Fassung. Die PATH-Probe wurde erneut gar nicht erst versucht. Zum Vergleich: am 15./16.08. dauerte die Nachlieferung rund zwei Tage, hier ist sie mehr als doppelt so lange aus. Der Rückfallweg über die app-gebündelte CLI trägt ohne einen einzigen Ausfall und ist faktisch der Normalweg; ein Handlungsbedarf entsteht daraus nicht, weil beide Wege unabhängig voneinander aktualisiert werden.

**Wochenbudget: Reserve wächst weiter.** `kontingent-budget.sh --json`: Ampel **FREI**, 30.88 von 167 Mio teuren Token = **18.5 %**, bei **43.4 %** verstrichener Woche. Vorsprung **-24.9 Punkte**, der Verbrauch läuft dem Zeitverlauf also noch deutlicher hinterher als heute Nacht (-21.3) und gestern Mittag (-16.3). Beide Stationsdateien frisch (MacBook Pro 23.94 Mio, Mac Mini 6.94 Mio, 0.0 bzw. 0.5 h alt). Kein Drossel-Anlass; der stehende Entscheid vom 03.08. («gleichmässig über die Woche») ist eingehalten, mit Luft nach oben statt nach unten.

**Feuermechanismen: alle drei Orte stimmen, beide Registries geprüft.** (i) Registry MacBook Pro: **jeder** aktive Task mit `lastRunAt` vom 27.08. — `logbuch-radar` 06:54, `konversations-log` 06:13, `vollgas-fruehwarnung` 07:15, `mahnwesen-verzugscheck` 08:05, `zahlungsabgleich-check` 08:22, `ag-gruendung-monitor` 07:46, `hub-chef-taeglich` 08:39, `heartbeat-daily` 07:40, `normen-training-nacht` 01:27, die Twin-Loops 03:39 und 05:44. Keine ausgefallene. (ii) Registry Mac Mini per `ssh mini`: **acht** Tasks, exakt der dokumentierte Sollstand, unverändert. (iii) launchd: `ch.jans.vollgas-monitor` und `ch.jans.vollgas-supervisor` liegen auf beiden Stationen als `*.disabled-260729` und sind nicht geladen; auf dem Mac Mini ist `ch.jans.nachtschicht` geladen mit **Exit 0**. Der stehende Entscheid vom 30.07. («nicht wieder beleben») ist gewahrt. Kein Loop wird doppelt gefeuert.

**Liefer-Delta über die letzten 13 h, gemessen per `git diff --name-only` gegen die Basis** (nicht per `find -newermt`, Werkzeug-Falle vom 20.08.): `twin` 11 Dateien · `normen` 8 · `architekten-synobsis` 2 · `spec` 1. Dazu Register- und Werkzeugpflege (`logbuch/fristen.md`, `logbuch/LOGBUCH.md`, `connectors/WEGE.md`, `rules/jans-dna-facetten.md`, `skills/honorarberechnung-sia102/SKILL.md`, `skills/twin/tools/stilmetrik.py`). 61 Commits im Fenster, davon der Grossteil `nas-selfcommit`. Die inhaltlichen Träger sind `normen-training-nacht` (01:27), `twin-mail-training` (03:39), `twin-fidelity-review` (05:44) samt Neukompilat der DNA-Facetten sowie ein Dispatch-Lauf auf dem Mac Mini um 05:35 (57 Turns, rc=0), der sechs offene Identitätsfragen der KB `architekten-synobsis` per Web-Recherche geklärt hat. **Kein Loop auf Delta Null, keine Rücktaktung fällig.**

**Zur Vollständigkeit, damit nichts falsch gelesen wird:** `energie-training` und `wissens-chef` erscheinen nicht in dieser Liste, weil ihre Läufe vom 26.08. (22:45 bis 23:11) **vor** der Messbasis liegen. Ihr Delta wurde im 00:57-Eintrag erfasst. Fenstereffekt, kein Leerlauf — dieselbe Konstellation wie heute Nacht bei den Twin-Loops, nur mit vertauschten Rollen.

### P3 — Der Ordner `token-drosselung-100810` existiert weiterhin, die Registry kennt ihn nicht

Beim Abgleich der beiden Registries fiel auf: `~/.claude/scheduled-tasks/` enthält ein Verzeichnis `token-drosselung-100810`, `list_scheduled_tasks` führt dazu aber **keinen Eintrag** — weder aktiv noch deaktiviert, während alle übrigen erledigten One-Time-Tasks (`masterclass-woche1`, `token-messung-140716`, `syn02-spec-anstoss`, `wettbewerbs-dna-reaktivierung`, `wettbewerbs-layer-nachbrenner`) sehr wohl als `enabled: false` gelistet sind. Damit ist die Feststellung des Radar-Headers vom 30.07.2026 zum dritten Mal bestätigt: **es kommt von dort keine Drossel.** Der Ordner ist ein Karteileichnam.

Der praktische Schaden ist heute null, der potenzielle liegt in der Zukunft: wer die Ordnerliste als Registry liest — ein naheliegender Griff, gerade beim Vergleich mit dem Mac Mini, wo genau dieser Weg der vorgeschriebene ist — sieht eine scharfe Drossel-Task, die keine ist. Der Umkehrschluss gilt ebenso und ist der gefährlichere: **auf dem Mac Mini wird die Ordnerliste laut Auftrag als Registry gelesen**, weil `list_scheduled_tasks` dort nicht greift. Ein Karteileichnam derselben Art würde dort als aktive Task gezählt. Kein Eingriff in diesem Lauf: einen Task-Ordner zu löschen ist nicht umkehrbar im Sinne von Rule `interaktive-eingriffe`, und dieser Lauf ist unbeaufsichtigt. Vorschlag zur Entscheidung durch Raphael: den Ordner nach `~/.claude/scheduled-tasks/_erledigt/` verschieben statt löschen, damit die Namenszuordnung der Rollen-Bilanz erhalten bleibt (Rule `rollen-taxonomie`).

### P3 (unverändert, aus dem Eintrag 00:57) — Permission-Regel mit Wildcard
