...
## 2026-09-04 12:57 — [FREI] **Regellauf ohne Befund. Die PATH-Probe antwortet zum dritten Mal in Folge (6 s, rc=0), der beste Wert der Reihe. Der einzige Befund ist eine Messfalle im Pruefwerkzeug selbst — und die Unterauslastung, die sich weiter oeffnet: das Wochenkontingent laeuft dem Zeitverlauf jetzt um 30.5 Punkte hinterher.**

**Selbstkontrolle: bestanden.** `lastRunAt` dieser Task **04.09. 12:57** (2026-09-04T10:57:40Z), letzter Eintrag **04.09. 00:57** — exakt **12 h 0 min** bei 12-h-Takt, Toleranz 15 h (Takt + 3 h). Kein Lauf fehlt; der zweite Ausfalltyp ist nicht zu pruefen.

**Fensterprobe [FREI].** `/opt/homebrew/bin/claude -p ... < /dev/null` antwortet «OK», **rc=0 in 6 s**. Dritte Bestaetigung in Folge nach dem 03.09. (13 s) und dem 04.09. 00:57 (7 s); die Wedge-Episode vom 29.08. bleibt geschlossen, der Rueckfallweg ueber die app-gebuendelte CLI bleibt Rueckfall.

**⚠ MESSFALLE IM EIGENEN PRUEFWERKZEUG (eigener Befund, fuer die Latenz-Reihe wichtig).** Die erste Messung dieses Laufs meldete **180 s** — das ist **nicht** die Latenz der Probe, sondern die Laufzeit des Wrappers. Ursache: der Watchdog `( sleep 180; kill ... ) &` haelt die Pipeline des Scripts offen, auch nachdem `wait $CPID` laengst zurueck ist und `kill $WPID` die Subshell beendet hat — der geerbte `sleep` laeuft weiter und haelt stdout. Wer die Wanduhr um `bash probe.sh` legt, misst deshalb **immer die Watchdog-Grenze**, nie die Antwortzeit. Die saubere Messung (Zeitstempel unmittelbar um `wait $CPID`, danach `kill -9 $WPID` plus `pkill -P $WPID sleep`) ergab **6 s**, Faktor 30 Unterschied.
**Warum das zaehlt:** ein 180-s-Wert waere als Ausreisser in die Latenz-Reihe eingegangen (7 · 13 · 7 · **180**) und haette exakt den Trend vorgetaeuscht, der am 19.08. schon einmal faelschlich als Degradation gelesen wurde — diesmal aber ohne Gegenbeleg, weil die Zahl an der Watchdog-Grenze klebt und damit reproduzierbar aussieht. **Regel fuer kuenftige Laeufe: die Latenz IM Probe-Script messen, nie um das Script herum.** Gleiche Familie wie die `find -newermt`-Falle (20.08.) und wie Rule `auto-verbesserungen` 260807: eine Zahl, die das Werkzeug erzeugt, ist zuerst eine Aussage ueber das Werkzeug.

**Wochenbudget: FREI, aber die Luecke waechst.** `kontingent-budget.sh --json`: **45.5 von 167 Mio teuer (27.2 %)** bei **57.7 %** verstrichener Woche, **Vorsprung −30.5 Punkte**. Stationen: MacBook Pro 37.33, Mac Mini 8.16, beide frisch. Zum Vergleich der Vorlauf: **04.09. 00:57 −27 Punkte**, jetzt **−30.5** — der Rueckstand hat in zwoelf Stunden um 3.5 Punkte zugenommen, er schliesst sich also nicht, er oeffnet sich. Linear fortgeschrieben endet die Woche bei rund **79 Mio**, also **47 % des Budgets**; gut 88 Mio blieben ungenutzt. Das ist keine Verletzung des stehenden Entscheids vom 03.08. («gleichmaessig ueber die Woche») — gleichmaessig ist der Verlauf durchaus. Es verfehlt aber das Ziel dieses Auftrags, das Kontingent PRODUKTIV auszuschoepfen. **Keine Massnahme aus eigenem Antrieb**: die Regimefrage ist entschieden und wird nicht erneut vorgelegt; ein Tagesdeckel-Mechanismus in die Gegenrichtung (Auffuellen) existiert nicht und waere ein eigener Auftrag.

**Liefer-Delta: kraeftig, kein Leerlauf-Kandidat.** Gemessen ueber git (nicht ueber `find`), Basis `76fc3b5a` bis HEAD, Fenster 14 h: **72 Commits**, davon substanziell. Geaenderte Dateien je KB: **normen 14 · twin 11 · planungsgrundlagen 6 · bauprodukte 5 · koordination 3 · immobilienbewertung 3 · baurecht 3 · grobkosten 2 · energie 2 · architektur-fachwissen 2**. Ausserhalb `wissen/` gearbeitet: `logbuch/fristen.md`, `logbuch/LOGBUCH.md`, `rules/betrieb-chronik.md`, `rules/jans-dna-facetten.md`, `scripts/lauf.sh`, `skills/wissens-destillat/tools/inventar.sh`, `docs/konzepte/260830-Laufzeitschicht-Umbau/`. Die operativen Loops haben alle geliefert (logbuch-radar 06:55, mahnwesen 08:05, ag-gruendung 07:46, hub-chef 08:39, twin-fidelity 05:45, vollgas-fruehwarnung 07:15). **Kein Loop steht bei drei oder mehr Delta-Null-Laeufen. Keine Ruecktaktung, keine Stilllegung.**

**Feuermechanismen: Sollstand auf beiden Stationen.** MacBook Pro: `ch.jans.vollgas-supervisor` und `ch.jans.vollgas-monitor` unveraendert als `*.disabled-260729` abgelegt und **nicht geladen**; geladen sind nur die regulaeren Jobs (synctask-runner, speicher-waechter, transcript-rotation, nas-keepalive, git-auto-sync, station-status, cockpit-server, claude-alwayson, wissens-trigger, widerruf-queue, screensaver-idle). Mac Mini: `ch.jans.nachtschicht` **geladen** — der einzige Lern-Taktgeber, wie vorgesehen; `vollgas-supervisor` dort ebenfalls nur als `.disabled-260729` vorhanden und nicht geladen. Kein Doppelfeuern, keine wiederbelebte plist. Registry MacBook: 36 Tasks, die deaktivierten sind durchgehend die per Entscheid auf Ereignis-Trigger gestellten (immobewertung-training, spec-training, synergie-lauf-monatlich) plus abgelaufene One-Time-Tasks — **keine Task haengt aus einer frueheren Drossel und wartet auf Wiederscharfschaltung.**

**Speicher: unauffaellig.** frei+inaktiv+purgeable rund **3.9 GB**, `kern.memorystatus_vm_pressure_level: 1` (normal). Keine `claude -p`-Waisen (`ps` leer), der Watchdog dieses Laufs hat sauber abgeraeumt.

**P1 — keiner.** Kein Blocker, keine Mail (kein neuer P1, kein geloester P1, kein erschoepftes Wochenkontingent).
**P2 — Unterauslastung, zweiter Lauf in Folge zunehmend.** 45.5 von 167 Mio bei 57.7 % Wochenverlauf. Zur Kenntnis Raphaels, ohne Massnahme dieses Radars; falls das Kontingent aktiv aufgefuellt werden soll, braucht es einen eigenen Auftrag (Gegenstueck zum nie gebauten Tagesdeckel).
**P3 — Latenz-Messung im Script.** Ab dem naechsten Lauf im Probe-Script messen, nicht um es herum (siehe Messfalle oben). Kein Werkzeug-Umbau noetig, nur die Messstelle.
