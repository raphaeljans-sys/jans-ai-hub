...
## 260709 — Logbuch-Radar: Mailverkehr WIRKLICH lesen und jeden offenen Punkt gegen den neuesten Thread verifizieren, bevor er weitergeschleppt wird
- **Auslöser:** "mir erscheinen die erwähnten log's nicht wirklich sinnhaft und mit dem aktuellen mailverkehr abgeglichen: A) RE-00088 ist bezahlt; keine Erwähnung mehr wert B) Rechnung Ersatzabgabe wurde von KISPI bezahlt ... wenn du dies tun würdest [Mailverkehr + Anhänge anschauen] wäre es klar dass diese rechnung bezahlt wurde. Könntest du bitte einrichten das der Mailverkehr besser gelesen wird und die infos korrekter in den log einfliessen, ansonsten macht das mit dem logbuch radar kein sinn." (Der Radar hatte RE-00088 und die Lenggstrasse-30-Schutzräume-/Ersatzabgaben-Rechnung tagelang als «offen/dringend» geführt, obwohl beide belegt bezahlt waren — die Zahlung der Ersatzabgabe war schon 06.07. veranlasst, «Die Rechnung wird sofort überwiesen».)
- **Regel:** Der Logbuch-Radar (Scheduled Task `logbuch-radar`, Skill `logbuch`) darf einen bestehenden Register-Punkt NICHT erneut als «offen/dringend/nachfassen» melden, ohne den zugehörigen Mail-Thread WIRKLICH gelesen zu haben — Body via read_resource, bei Bedarf die Anhänge/Rechnungen öffnen, nicht nur Betreffzeilen scannen. Pflicht-Schritt 2b «Gegen-Verifikation»: je offenem Punkt den NEUESTEN Thread auf Erledigungs-/Zahlungssignale prüfen und bei Beleg schliessen (→ Archiv). Erledigt-Signale u.a.: «wird (sofort) überwiesen» / «bezahlt» / «Zahlung veranlasst» / Weiterleitung an die Kreditoren; bei Debitoren zusätzlich bexio `--verzug` (nicht mehr im Verzug = bezahlt). Fremd-Rechnungen (Bauherr/KISPI zahlt, nicht JANS) sind keine offene JANS-Aktion, sobald die Zahlung dort veranlasst ist. Kein tagelanges unverändertes Wiederholen desselben Punkts ohne Bestätigung durch den neuesten Thread; im Zweifel den Beleg zitieren (Absender + Datum + Kernsatz) statt raten.
- **Gilt für:** Scheduled Task `logbuch-radar`, Skill `logbuch`, und jede Register-/Fristen-Pflege (auch `hub-chef`, `/morgen`, `mahnwesen`/`zahlungsabgleich`-Aggregation), alle Stationen.
