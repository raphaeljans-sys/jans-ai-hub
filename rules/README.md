...
- `interaktive-eingriffe.md` — **NOCH NICHT AKTIV** (kein @-Import, Stand 12.08.2026: Raphael zur Durchsicht vorgelegt). Spielraum interaktiver Sessions bei heiklen Eingriffen (Git-Interna, Cloud-Sync/FileProvider, Keychain, Systemdienste, Persistenz): vorher im Chat ankuendigen, nur umkehrbar handeln (`mv` statt `rm`, kein Force), Unumkehrbares und Unbeaufsichtigtes ueber die Freigabe-Queue, danach Einzeiler in die `betrieb-chronik`. Schliesst die Luecke zwischen Freigabe-Schwelle (unbeaufsichtigt) und Aktions-Whitelist (umkehrbare Arbeit); Anlassfall 260812
...
- `rollen-taxonomie.md` — jeder Baustein (Skill/Agent/Loop) trägt eine der fünf Rollen prototyper/builder/sweeper/grower/maintainer im Register `logbuch/rollen/rollen-map.tsv`; vor jedem neuen Skill und vor jeder Takt-/Drosselentscheidung die Rollen-Bilanz lesen; Instrumente `rollen-bilanz.sh`, `nutzungs-radar.sh`, `schutzmechanik-selbsttest.sh`
...
- `wege-und-vollmachten.md` — vor «geht nicht» das Wege-Register `connectors/WEGE.md` lesen und ALLE Wege durchprobieren; Sackgassen dort eintragen; Vollmacht nach `logbuch/AKTIONS-WHITELIST.md` v2 (umkehrbare Arbeit wird getan, Aussenwirkung ueber Widerrufsfenster, Zahlungen ausgenommen); Klassifikator und Freigabe-Schwelle werden nie umgangen; Anlassfall KISPI 260809
...
- `wissens-ruecklauf.md` — Pflicht-Schlussschritt nach jedem substanziellen Deliverable: «Was ist verallgemeinerbar?» → belegter `outputs/`-Report in die passende KB + QUESTIONS-Eintrag; nie rohe Chat-Antworten (Echo-Schutz-Logik); Anlassfall StrAV/2414 Thalwil 260731
